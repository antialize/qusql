//! Contains the `Keyword` enum and related functions.
// This file is generated by keywords.py. Do not edit manually.

/// A SQL keyword.
#[derive(Debug, Clone, Copy, PartialEq, PartialOrd, Eq, Ord, Hash)]
#[allow(non_camel_case_types, clippy::upper_case_acronyms)]
#[derive(Default)]
pub enum Keyword {
    #[default]
    NOT_A_KEYWORD,
    QUOTED_IDENTIFIER,
    ABS,
    ACCESSIBLE,
    ACCOUNT,
    ACOS,
    ACTION,
    ADD,
    ADDDATE,
    ADDTIME,
    ADD_MONTHS,
    ADMIN,
    AFTER,
    AGAINST,
    AGGREGATE,
    ALGORITHM,
    ALL,
    ALTER,
    ALWAYS,
    ANALYZE,
    AND,
    ANY,
    AS,
    ASC,
    ASCII,
    ASENSITIVE,
    ASIN,
    AT,
    ATAN,
    ATAN2,
    ATOMIC,
    AUTHORIZATION,
    AUTHORS,
    AUTO,
    AUTOEXTEND_SIZE,
    AUTO_INCREMENT,
    AVG,
    AVG_ROW_LENGTH,
    BACKUP,
    BEFORE,
    BEGIN,
    BETWEEN,
    BIGINT,
    BIN,
    BINARY,
    BINLOG,
    BIT,
    BIT_LENGTH,
    BLOB,
    BLOCK,
    BLOOM,
    BODY,
    BOOL,
    BOOLEAN,
    BOTH,
    BPCHAR_PATTERN_OPS,
    BRIN,
    BTREE,
    BY,
    BYPASSRLS,
    BYTE,
    BYTEA,
    CACHE,
    CALL,
    CASCADE,
    CASCADED,
    CASE,
    CAST,
    CATALOG_NAME,
    CEIL,
    CEILING,
    CHAIN,
    CHANGE,
    CHANGED,
    CHANNEL,
    CHAR,
    CHARACTER,
    CHARACTER_LENGTH,
    CHARSET,
    CHAR_LENGTH,
    CHECK,
    CHECKPOINT,
    CHECKSUM,
    CHR,
    CIPHER,
    CLASS,
    CLASS_ORIGIN,
    CLIENT,
    CLOB,
    CLOSE,
    COALESCE,
    CODE,
    COLLATE,
    COLLATION,
    COLUMN,
    COLUMNS,
    COLUMN_ADD,
    COLUMN_CHECK,
    COLUMN_CREATE,
    COLUMN_DELETE,
    COLUMN_GET,
    COLUMN_NAME,
    COMMENT,
    COMMIT,
    COMMITTED,
    COMMUTATOR,
    COMPACT,
    COMPLETION,
    COMPRESSED,
    COMPRESSION,
    CONCAT,
    CONCAT_WS,
    CONCURRENT,
    CONCURRENTLY,
    CONDITION,
    CONFLICT,
    CONNECTION,
    CONSISTENT,
    CONSTRAINT,
    CONSTRAINT_CATALOG,
    CONSTRAINT_NAME,
    CONSTRAINT_SCHEMA,
    CONTAINS,
    CONTEXT,
    CONTINUE,
    CONTRIBUTORS,
    CONV,
    CONVERT,
    CONVERT_TZ,
    COPY,
    COS,
    COSTS,
    COT,
    COUNT,
    CPU,
    CRC32,
    CRC32C,
    CREATE,
    CREATEDB,
    CREATEROLE,
    CROSS,
    CUBE,
    CURDATE,
    CURRENT,
    CURRENT_DATE,
    CURRENT_POS,
    CURRENT_ROLE,
    CURRENT_TIME,
    CURRENT_TIMESTAMP,
    CURRENT_USER,
    CURSOR,
    CURSOR_NAME,
    CURTIME,
    CYCLE,
    DATA,
    DATABASE,
    DATABASES,
    DATAFILE,
    DATE,
    DATEDIFF,
    DATETIME,
    DATE_ADD,
    DATE_FORMAT,
    DATE_SUB,
    DAY,
    DAYNAME,
    DAYOFMONTH,
    DAYOFWEEK,
    DAYOFYEAR,
    DAY_HOUR,
    DAY_MICROSECOND,
    DAY_MINUTE,
    DAY_SECOND,
    DEALLOCATE,
    DEC,
    DECIMAL,
    DECLARE,
    DEFAULT,
    DEFINER,
    DEGREES,
    DELAYED,
    DELAY_KEY_WRITE,
    DELETE,
    DELETE_DOMAIN_ID,
    DELIMITER,
    DESC,
    DESCRIBE,
    DES_KEY_FILE,
    DETERMINISTIC,
    DIAGNOSTICS,
    DIRECTORY,
    DISABLE,
    DISCARD,
    DISK,
    DISTINCT,
    DISTINCTROW,
    DIV,
    DO,
    DOMAIN,
    DOUBLE,
    DO_DOMAIN_IDS,
    DROP,
    DUAL,
    DUMPFILE,
    DUPLICATE,
    DYNAMIC,
    EACH,
    ELSE,
    ELSEIF,
    ELSIF,
    ELT,
    EMPTY,
    ENABLE,
    ENCLOSED,
    ENCRYPTED,
    ENCRYPTION,
    END,
    ENDS,
    ENGINE,
    ENGINES,
    ENGINE_ATTRIBUTE,
    ENUM,
    ERROR,
    ERRORS,
    ESCAPE,
    ESCAPED,
    EVENT,
    EVENTS,
    EVERY,
    EXAMINED,
    EXCEPT,
    EXCEPTION,
    EXCHANGE,
    EXCLUDE,
    EXCLUSIVE,
    EXECUTE,
    EXISTS,
    EXIT,
    EXP,
    EXPANSION,
    EXPIRE,
    EXPLAIN,
    EXPORT,
    EXPORT_SET,
    EXTENDED,
    EXTENSION,
    EXTENT_SIZE,
    EXTRACT,
    EXTRACTVALUE,
    FALSE,
    FAMILY,
    FAST,
    FAULTS,
    FEDERATED,
    FETCH,
    FIELD,
    FIELDS,
    FILE,
    FIND_IN_SET,
    FIRST,
    FIXED,
    FLOAT,
    FLOAT4,
    FLOAT8,
    FLOOR,
    FLUSH,
    FOLLOWING,
    FOLLOWS,
    FOR,
    FORCE,
    FOREIGN,
    FORMAT,
    FOUND,
    FROM,
    FROM_BASE64,
    FROM_DAYS,
    FROM_UNIXTIME,
    FULL,
    FULLTEXT,
    FUNCTION,
    GENERAL,
    GENERATED,
    GET,
    GET_FORMAT,
    GIN,
    GIST,
    GLOBAL,
    GOTO,
    GRANT,
    GRANTS,
    GREATEST,
    GROUP,
    GROUP_CONCAT,
    HANDLER,
    HARD,
    HASH,
    HASHES,
    HAVING,
    HELP,
    HEX,
    HIGH_PRIORITY,
    HISTORY,
    HNSW,
    HOST,
    HOSTS,
    HOUR,
    HOUR_MICROSECOND,
    HOUR_MINUTE,
    HOUR_SECOND,
    ID,
    IDENTIFIED,
    IDENTITY,
    IF,
    IFNULL,
    IGNORE,
    IGNORED,
    IGNORE_DOMAIN_IDS,
    IGNORE_SERVER_IDS,
    IMMEDIATE,
    IMPORT,
    IN,
    INCLUDE,
    INCREMENT,
    INDEX,
    INDEXES,
    INET4,
    INET6,
    INFILE,
    INHERIT,
    INHERITS,
    INITIAL_SIZE,
    INNER,
    INOUT,
    INPLACE,
    INSENSITIVE,
    INSERT,
    INSERT_METHOD,
    INSTALL,
    INSTANT,
    INSTR,
    INT,
    INT1,
    INT2,
    INT2_OPS,
    INT3,
    INT4,
    INT4_OPS,
    INT8,
    INT8_OPS,
    INTEGER,
    INTERSECT,
    INTERSECTA,
    INTERVAL,
    INTO,
    INVISIBLE,
    INVOKER,
    IO,
    IO_THREAD,
    IPC,
    IS,
    ISOLATION,
    ISOPEN,
    ISSUER,
    ITERATE,
    JOIN,
    JSON,
    JSON_ARRAY,
    JSON_ARRAYAGG,
    JSON_ARRAY_APPEND,
    JSON_ARRAY_INSERT,
    JSON_ARRAY_INTERSECT,
    JSON_COMPACT,
    JSON_CONTAINS,
    JSON_CONTAINS_PATH,
    JSON_DEPTH,
    JSON_DETAILED,
    JSON_EQUALS,
    JSON_EXISTS,
    JSON_EXTRACT,
    JSON_INSERT,
    JSON_KEYS,
    JSON_LENGTH,
    JSON_LOOSE,
    JSON_MERGE,
    JSON_MERGE_PATCH,
    JSON_MERGE_PRESERVE,
    JSON_NORMALIZE,
    JSON_OBJECT,
    JSON_OBJECTAGG,
    JSON_OBJECT_FILTER_KEYS,
    JSON_OBJECT_TO_ARRAY,
    JSON_OVERLAPS,
    JSON_PRETTY,
    JSON_QUERY,
    JSON_QUOTE,
    JSON_REMOVE,
    JSON_REPLACE,
    JSON_SCHEMA_VALID,
    JSON_SEARCH,
    JSON_SET,
    JSON_TABLE,
    JSON_TYPE,
    JSON_UNQUOTE,
    JSON_VALID,
    JSON_VALUE,
    KEY,
    KEYS,
    KEY_BLOCK_SIZE,
    KILL,
    LAG,
    LANGUAGE,
    LAST,
    LASTVAL,
    LAST_DAY,
    LAST_VALUE,
    LCASE,
    LEAD,
    LEADING,
    LEAST,
    LEAVE,
    LEAVES,
    LEFT,
    LEFTARG,
    LENGTH,
    LENGTHB,
    LESS,
    LEVEL,
    LIKE,
    LIMIT,
    LINEAR,
    LINES,
    LIST,
    LN,
    LOAD,
    LOAD_FILE,
    LOCAL,
    LOCALTIME,
    LOCALTIMESTAMP,
    LOCATE,
    LOCK,
    LOCKED,
    LOCKS,
    LOG,
    LOG10,
    LOG2,
    LOGFILE,
    LOGIN,
    LOGS,
    LONG,
    LONGBLOB,
    LONGTEXT,
    LOOP,
    LOWER,
    LOW_PRIORITY,
    LPAD,
    LTRIM,
    MAKEDATE,
    MAKETIME,
    MAKE_SET,
    MASTER,
    MASTER_CONNECT_RETRY,
    MASTER_DELAY,
    MASTER_GTID_POS,
    MASTER_HEARTBEAT_PERIOD,
    MASTER_HOST,
    MASTER_LOG_FILE,
    MASTER_LOG_POS,
    MASTER_PASSWORD,
    MASTER_PORT,
    MASTER_SERVER_ID,
    MASTER_SSL,
    MASTER_SSL_CA,
    MASTER_SSL_CAPATH,
    MASTER_SSL_CERT,
    MASTER_SSL_CIPHER,
    MASTER_SSL_CRL,
    MASTER_SSL_CRLPATH,
    MASTER_SSL_KEY,
    MASTER_SSL_VERIFY_SERVER_CERT,
    MASTER_USER,
    MASTER_USE_GTID,
    MATCH,
    MATERIALIZED,
    MAX,
    MAXVALUE,
    MAX_CONNECTIONS_PER_HOUR,
    MAX_QUERIES_PER_HOUR,
    MAX_ROWS,
    MAX_SIZE,
    MAX_STATEMENT_TIME,
    MAX_UPDATES_PER_HOUR,
    MAX_USER_CONNECTIONS,
    MEDIUM,
    MEDIUMBLOB,
    MEDIUMINT,
    MEDIUMTEXT,
    MEMBER,
    MEMORY,
    MERGE,
    MERGES,
    MESSAGE_TEXT,
    MICROSECOND,
    MID,
    MIDDLEINT,
    MIGRATE,
    MIN,
    MINUS,
    MINUTE,
    MINUTE_MICROSECOND,
    MINUTE_SECOND,
    MINVALUE,
    MIN_ROWS,
    MOD,
    MODE,
    MODIFIES,
    MODIFY,
    MONITOR,
    MONTH,
    MONTHNAME,
    MUTEX,
    MYSQL,
    MYSQL_ERRNO,
    NAME,
    NAMES,
    NATIONAL,
    NATURAL,
    NATURAL_SORT_KEY,
    NCHAR,
    NEGATOR,
    NESTED,
    NEVER,
    NEW,
    NEXT,
    NEXTVAL,
    NO,
    NOBYPASSRLS,
    NOCACHE,
    NOCREATEDB,
    NOCREATEROLE,
    NOCYCLE,
    NODEGROUP,
    NOINHERIT,
    NOLOGIN,
    NOMAXVALUE,
    NOMINVALUE,
    NONE,
    NOREPLICATION,
    NOSUPERUSER,
    NOT,
    NOTFOUND,
    NOTHING,
    NOTICE,
    NOW,
    NOWAIT,
    NO_WAIT,
    NO_WRITE_TO_BINLOG,
    NULL,
    NULLIF,
    NULLS,
    NUMBER,
    NUMERIC,
    NVARCHAR,
    NVL,
    NVL2,
    OCT,
    OCTET_LENGTH,
    OF,
    OFFSET,
    OLD_PASSWORD,
    ON,
    ONE,
    ONLINE,
    ONLY,
    OPEN,
    OPERATOR,
    OPTIMIZE,
    OPTIMIZER,
    OPTIMIZER_COSTS,
    OPTION,
    OPTIONALLY,
    OPTIONS,
    OR,
    ORD,
    ORDER,
    ORDINALITY,
    OTHERS,
    OUT,
    OUTER,
    OUTFILE,
    OVER,
    OVERLAPS,
    OWNED,
    OWNER,
    PACKAGE,
    PACK_KEYS,
    PAGE,
    PAGE_CHECKSUM,
    PARSER,
    PARSE_VCOL_EXPR,
    PARTIAL,
    PARTITION,
    PARTITIONING,
    PARTITIONS,
    PASSWORD,
    PATH,
    PERIOD,
    PERIOD_ADD,
    PERIOD_DIFF,
    PERSISTENT,
    PHASE,
    PI,
    PLPGSQL,
    PLUGIN,
    PLUGINS,
    PORT,
    PORTION,
    POSITION,
    POW,
    POWER,
    PRECEDES,
    PRECEDING,
    PRECISION,
    PREPARE,
    PRESERVE,
    PREV,
    PREVIOUS,
    PRIMARY,
    PRIVILEGES,
    PROCEDURE,
    PROCESS,
    PROCESSLIST,
    PROFILE,
    PROFILES,
    PROXY,
    PURGE,
    QUARTER,
    QUERY,
    QUICK,
    QUOTE,
    RADIANS,
    RAISE,
    RAND,
    RANGE,
    RAW,
    READ,
    READS,
    READ_ONLY,
    READ_WRITE,
    REAL,
    REBUILD,
    RECOVER,
    RECURSIVE,
    REDOFILE,
    REDO_BUFFER_SIZE,
    REDUNDANT,
    REFERENCES,
    REF_SYSTEM_ID,
    REGEXP,
    RELAY,
    RELAYLOG,
    RELAY_LOG_FILE,
    RELAY_LOG_POS,
    RELAY_THREAD,
    RELEASE,
    RELOAD,
    REMOVE,
    RENAME,
    REORGANIZE,
    REPAIR,
    REPEAT,
    REPEATABLE,
    REPLACE,
    REPLAY,
    REPLICA,
    REPLICAS,
    REPLICATION,
    REPLICA_POS,
    REQUIRE,
    RESET,
    RESIGNAL,
    RESTART,
    RESTORE,
    RESTRICT,
    RESUME,
    RETURN,
    RETURNED_SQLSTATE,
    RETURNING,
    RETURNS,
    REUSE,
    REVERSE,
    REVOKE,
    RIGHT,
    RIGHTARG,
    RLIKE,
    ROLE,
    ROLLBACK,
    ROLLUP,
    ROUND,
    ROUTINE,
    ROW,
    ROWCOUNT,
    ROWNUM,
    ROWS,
    ROWTYPE,
    ROW_COUNT,
    ROW_FORMAT,
    RPAD,
    RTREE,
    RTRIM,
    SAVEPOINT,
    SCHEDULE,
    SCHEMA,
    SCHEMAS,
    SCHEMA_NAME,
    SEARCH,
    SECOND,
    SECONDARY,
    SECONDARY_ENGINE,
    SECONDARY_ENGINE_ATTRIBUTE,
    SECOND_MICROSECOND,
    SECURITY,
    SEC_TO_TIME,
    SELECT,
    SENSITIVE,
    SEPARATOR,
    SEQUENCE,
    SERIAL,
    SERIALIZABLE,
    SERVER,
    SESSION,
    SESSION_USER,
    SET,
    SETVAL,
    SFORMAT,
    SHARE,
    SHARED,
    SHOW,
    SHUTDOWN,
    SIGN,
    SIGNAL,
    SIGNED,
    SIMPLE,
    SIN,
    SKIP,
    SLAVE,
    SLAVES,
    SLAVE_POS,
    SLEEP,
    SLOW,
    SMALLINT,
    SNAPSHOT,
    SOCKET,
    SOFT,
    SOME,
    SONAME,
    SOUNDEX,
    SOUNDS,
    SOURCE,
    SPACE,
    SPATIAL,
    SPECIFIC,
    SQL,
    SQLEXCEPTION,
    SQLSTATE,
    SQLWARNING,
    SQL_BIG_RESULT,
    SQL_BUFFER_RESULT,
    SQL_CACHE,
    SQL_CALC_FOUND_ROWS,
    SQL_NO_CACHE,
    SQL_SMALL_RESULT,
    SQL_THREAD,
    SQL_TSI_DAY,
    SQL_TSI_HOUR,
    SQL_TSI_MINUTE,
    SQL_TSI_MONTH,
    SQL_TSI_QUARTER,
    SQL_TSI_SECOND,
    SQL_TSI_WEEK,
    SQL_TSI_YEAR,
    SQRT,
    SSL,
    STAGE,
    START,
    STARTING,
    STARTS,
    STARTS_WITH,
    STATEMENT,
    STATS_AUTO_RECALC,
    STATS_PERSISTENT,
    STATS_SAMPLE_PAGES,
    STATUS,
    STDIN,
    STOP,
    STORAGE,
    STORED,
    STRAIGHT_JOIN,
    STRCMP,
    STRFTIME,
    STRICT,
    STRING,
    STR_TO_DATE,
    SUBCLASS_ORIGIN,
    SUBDATE,
    SUBJECT,
    SUBPARTITION,
    SUBPARTITIONS,
    SUBSTR,
    SUBSTRING,
    SUBSTRING_INDEX,
    SUBTIME,
    SUM,
    SUPER,
    SUPERUSER,
    SUSPEND,
    SWAPS,
    SWITCHES,
    SYSDATE,
    SYSID,
    SYSTEM,
    SYSTEM_TIME,
    TABLE,
    TABLES,
    TABLESPACE,
    TABLE_CHECKSUM,
    TABLE_NAME,
    TAN,
    TEMPORARY,
    TEMPTABLE,
    TERMINATED,
    TEXT,
    TEXT_PATTERN_OPS,
    THAN,
    THEN,
    THREADS,
    TIES,
    TIME,
    TIMEDIFF,
    TIMESTAMP,
    TIMESTAMPADD,
    TIMESTAMPDIFF,
    TIMESTAMPTZ,
    TIME_FORMAT,
    TIME_TO_SEC,
    TIME_ZONE,
    TINYBLOB,
    TINYINT,
    TINYTEXT,
    TO,
    TO_BASE64,
    TO_CHAR,
    TO_DAYS,
    TO_SECONDS,
    TRAILING,
    TRANSACTION,
    TRANSACTIONAL,
    TRIGGER,
    TRIGGERS,
    TRUE,
    TRUNCATE,
    TYPE,
    TYPES,
    UCASE,
    UNBOUNDED,
    UNCOMMITTED,
    UNCOMPRESSED_LENGTH,
    UNDEFINED,
    UNDO,
    UNDOFILE,
    UNDO_BUFFER_SIZE,
    UNHEX,
    UNICODE,
    UNINSTALL,
    UNION,
    UNIQUE,
    UNIX_TIMESTAMP,
    UNKNOWN,
    UNLOCK,
    UNSIGNED,
    UNTIL,
    UPDATE,
    UPDATEXML,
    UPGRADE,
    UPPER,
    USAGE,
    USE,
    USER,
    USER_RESOURCES,
    USE_FRM,
    USING,
    UTC_DATE,
    UTC_TIME,
    UTC_TIMESTAMP,
    VALID,
    VALUE,
    VALUES,
    VARBINARY,
    VARBIT,
    VARCHAR,
    VARCHAR2,
    VARCHARACTER,
    VARCHAR_PATTERN_OPS,
    VARIABLES,
    VARYING,
    VERSION,
    VERSIONING,
    VIA,
    VIEW,
    VIRTUAL,
    VISIBLE,
    WAIT,
    WARNINGS,
    WEEK,
    WEEKDAY,
    WEEKOFYEAR,
    WEIGHT_STRING,
    WHEN,
    WHERE,
    WHILE,
    WINDOW,
    WITH,
    WITHIN,
    WITHOUT,
    WORK,
    WRAPPER,
    WRITE,
    X509,
    XA,
    XML,
    XOR,
    YEAR,
    YEARWEEK,
    YEAR_MONTH,
    ZEROFILL,
    ZONE,
    _LIST_,
}

impl From<&str> for Keyword {
    fn from(v: &str) -> Self {
        let mut cs = v.chars();
        match cs.next() {
            Some('A' | 'a') => match cs.next() {
                Some('B' | 'b') => {
                    if matches!(cs.next(), Some('S' | 's')) {
                        Keyword::ABS
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('C' | 'c') => match cs.next() {
                    Some('C' | 'c') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('B' | 'b'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::ACCESSIBLE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::ACCOUNT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('S' | 's')) {
                            Keyword::ACOS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::ACTION
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('D' | 'd') => match cs.next() {
                    Some('D' | 'd') => match cs.next() {
                        None => Keyword::ADD,
                        Some('D' | 'd') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::ADDDATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('T' | 't') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::ADDTIME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('_') => {
                            if matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('H' | 'h'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::ADD_MONTHS
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::ADMIN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('F' | 'f') => {
                    if matches!(cs.next(), Some('T' | 't'))
                        && matches!(cs.next(), Some('E' | 'e'))
                        && matches!(cs.next(), Some('R' | 'r'))
                    {
                        Keyword::AFTER
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('G' | 'g') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::AGAINST
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::AGGREGATE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('L' | 'l') => match cs.next() {
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('M' | 'm'))
                        {
                            Keyword::ALGORITHM
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => Keyword::ALL,
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::ALTER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('W' | 'w') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::ALWAYS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('N' | 'n') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                            && matches!(cs.next(), Some('Z' | 'z'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::ANALYZE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('D' | 'd') => Keyword::AND,
                    Some('Y' | 'y') => Keyword::ANY,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('S' | 's') => match cs.next() {
                    None => Keyword::AS,
                    Some('C' | 'c') => match cs.next() {
                        None => Keyword::ASC,
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('I' | 'i')) {
                                Keyword::ASCII
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('V' | 'v'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::ASENSITIVE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('N' | 'n')) {
                            Keyword::ASIN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('T' | 't') => match cs.next() {
                    None => Keyword::AT,
                    Some('A' | 'a') => match cs.next() {
                        Some('N' | 'n') => match cs.next() {
                            None => Keyword::ATAN,
                            Some('2') => Keyword::ATAN2,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('C' | 'c'))
                        {
                            Keyword::ATOMIC
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('U' | 'u') => match cs.next() {
                    Some('T' | 't') => match cs.next() {
                        Some('H' | 'h') => match cs.next() {
                            Some('O' | 'o') => match cs.next() {
                                Some('R' | 'r') => match cs.next() {
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('Z' | 'z'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                        {
                                            Keyword::AUTHORIZATION
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => Keyword::AUTHORS,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('O' | 'o') => match cs.next() {
                            None => Keyword::AUTO,
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('X' | 'x'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('D' | 'd'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('Z' | 'z'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::AUTOEXTEND_SIZE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('M' | 'm'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::AUTO_INCREMENT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('V' | 'v') => match cs.next() {
                    Some('G' | 'g') => match cs.next() {
                        None => Keyword::AVG,
                        Some('_') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('W' | 'w'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('H' | 'h'))
                            {
                                Keyword::AVG_ROW_LENGTH
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('B' | 'b') => match cs.next() {
                Some('A' | 'a') => {
                    if matches!(cs.next(), Some('C' | 'c'))
                        && matches!(cs.next(), Some('K' | 'k'))
                        && matches!(cs.next(), Some('U' | 'u'))
                        && matches!(cs.next(), Some('P' | 'p'))
                    {
                        Keyword::BACKUP
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('E' | 'e') => match cs.next() {
                    Some('F' | 'f') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::BEFORE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::BEGIN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('W' | 'w'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::BETWEEN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::BIGINT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => match cs.next() {
                        None => Keyword::BIN,
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                            {
                                Keyword::BINARY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('G' | 'g'))
                            {
                                Keyword::BINLOG
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::BIT,
                        Some('_') => {
                            if matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('H' | 'h'))
                            {
                                Keyword::BIT_LENGTH
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('L' | 'l') => match cs.next() {
                    Some('O' | 'o') => match cs.next() {
                        Some('B' | 'b') => Keyword::BLOB,
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('K' | 'k')) {
                                Keyword::BLOCK
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('M' | 'm')) {
                                Keyword::BLOOM
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    Some('D' | 'd') => {
                        if matches!(cs.next(), Some('Y' | 'y')) {
                            Keyword::BODY
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => match cs.next() {
                        Some('L' | 'l') => match cs.next() {
                            None => Keyword::BOOL,
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                {
                                    Keyword::BOOLEAN
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('H' | 'h')) {
                            Keyword::BOTH
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => {
                    if matches!(cs.next(), Some('C' | 'c'))
                        && matches!(cs.next(), Some('H' | 'h'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('_'))
                        && matches!(cs.next(), Some('P' | 'p'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('T' | 't'))
                        && matches!(cs.next(), Some('T' | 't'))
                        && matches!(cs.next(), Some('E' | 'e'))
                        && matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('N' | 'n'))
                        && matches!(cs.next(), Some('_'))
                        && matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('P' | 'p'))
                        && matches!(cs.next(), Some('S' | 's'))
                    {
                        Keyword::BPCHAR_PATTERN_OPS
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('R' | 'r') => {
                    if matches!(cs.next(), Some('I' | 'i')) && matches!(cs.next(), Some('N' | 'n'))
                    {
                        Keyword::BRIN
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('T' | 't') => {
                    if matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('E' | 'e'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::BTREE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('Y' | 'y') => match cs.next() {
                    None => Keyword::BY,
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::BYPASSRLS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::BYTE,
                            Some('A' | 'a') => Keyword::BYTEA,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('C' | 'c') => {
                match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('H' | 'h'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::CACHE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('L' | 'l')) {
                                Keyword::CALL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => match cs.next() {
                            Some('C' | 'c') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('D' | 'd') => match cs.next() {
                                        Some('E' | 'e') => match cs.next() {
                                            None => Keyword::CASCADE,
                                            Some('D' | 'd') => Keyword::CASCADED,
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('E' | 'e') => Keyword::CASE,
                            Some('T' | 't') => Keyword::CAST,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('T' | 't') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::CATALOG_NAME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => match cs.next() {
                        Some('I' | 'i') => match cs.next() {
                            Some('L' | 'l') => match cs.next() {
                                None => Keyword::CEIL,
                                Some('I' | 'i') => {
                                    if matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('G' | 'g'))
                                    {
                                        Keyword::CEILING
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('H' | 'h') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('N' | 'n')) {
                                    Keyword::CHAIN
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('N' | 'n') => match cs.next() {
                                Some('G' | 'g') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        None => Keyword::CHANGE,
                                        Some('D' | 'd') => Keyword::CHANGED,
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('N' | 'n') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                    {
                                        Keyword::CHANNEL
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('R' | 'r') => match cs.next() {
                                None => Keyword::CHAR,
                                Some('A' | 'a') => match cs.next() {
                                    Some('C' | 'c') => match cs.next() {
                                        Some('T' | 't') => match cs.next() {
                                            Some('E' | 'e') => match cs.next() {
                                                Some('R' | 'r') => match cs.next() {
                                                    None => Keyword::CHARACTER,
                                                    Some('_') => {
                                                        if matches!(cs.next(), Some('L' | 'l'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                            && matches!(cs.next(), Some('N' | 'n'))
                                                            && matches!(cs.next(), Some('G' | 'g'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('H' | 'h'))
                                                        {
                                                            Keyword::CHARACTER_LENGTH
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('S' | 's') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::CHARSET
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('_') => {
                                    if matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('G' | 'g'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('H' | 'h'))
                                    {
                                        Keyword::CHAR_LENGTH
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('E' | 'e') => match cs.next() {
                            Some('C' | 'c') => match cs.next() {
                                Some('K' | 'k') => match cs.next() {
                                    None => Keyword::CHECK,
                                    Some('P' | 'p') => {
                                        if matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                        {
                                            Keyword::CHECKPOINT
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => {
                                        if matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                        {
                                            Keyword::CHECKSUM
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('R' | 'r') => Keyword::CHR,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::CIPHER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('S' | 's') => match cs.next() {
                                Some('S' | 's') => match cs.next() {
                                    None => Keyword::CLASS,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('G' | 'g'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                        {
                                            Keyword::CLASS_ORIGIN
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::CLIENT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => match cs.next() {
                            Some('B' | 'b') => Keyword::CLOB,
                            Some('S' | 's') => {
                                if matches!(cs.next(), Some('E' | 'e')) {
                                    Keyword::CLOSE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::COALESCE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('D' | 'd') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::CODE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('L' | 'l') => match cs.next() {
                            Some('L' | 'l') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        Some('E' | 'e') => Keyword::COLLATE,
                                        Some('I' | 'i') => {
                                            if matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                            {
                                                Keyword::COLLATION
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('U' | 'u') => match cs.next() {
                                Some('M' | 'm') => match cs.next() {
                                    Some('N' | 'n') => match cs.next() {
                                        None => Keyword::COLUMN,
                                        Some('S' | 's') => Keyword::COLUMNS,
                                        Some('_') => match cs.next() {
                                            Some('A' | 'a') => {
                                                if matches!(cs.next(), Some('D' | 'd'))
                                                    && matches!(cs.next(), Some('D' | 'd'))
                                                {
                                                    Keyword::COLUMN_ADD
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('C' | 'c') => match cs.next() {
                                                Some('H' | 'h') => {
                                                    if matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('C' | 'c'))
                                                        && matches!(cs.next(), Some('K' | 'k'))
                                                    {
                                                        Keyword::COLUMN_CHECK
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('R' | 'r') => {
                                                    if matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('A' | 'a'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                    {
                                                        Keyword::COLUMN_CREATE
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('D' | 'd') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('L' | 'l'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                {
                                                    Keyword::COLUMN_DELETE
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('G' | 'g') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                {
                                                    Keyword::COLUMN_GET
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('N' | 'n') => {
                                                if matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('M' | 'm'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                {
                                                    Keyword::COLUMN_NAME
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('M' | 'm') => match cs.next() {
                            Some('M' | 'm') => match cs.next() {
                                Some('E' | 'e') => {
                                    if matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::COMMENT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('I' | 'i') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        None => Keyword::COMMIT,
                                        Some('T' | 't') => {
                                            if matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('D' | 'd'))
                                            {
                                                Keyword::COMMITTED
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('U' | 'u') => {
                                    if matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                    {
                                        Keyword::COMMUTATOR
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('P' | 'p') => match cs.next() {
                                Some('A' | 'a') => {
                                    if matches!(cs.next(), Some('C' | 'c'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::COMPACT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('L' | 'l') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                    {
                                        Keyword::COMPLETION
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('R' | 'r') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        Some('S' | 's') => match cs.next() {
                                            Some('S' | 's') => match cs.next() {
                                                Some('E' | 'e') => {
                                                    if matches!(cs.next(), Some('D' | 'd')) {
                                                        Keyword::COMPRESSED
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('I' | 'i') => {
                                                    if matches!(cs.next(), Some('O' | 'o'))
                                                        && matches!(cs.next(), Some('N' | 'n'))
                                                    {
                                                        Keyword::COMPRESSION
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('N' | 'n') => match cs.next() {
                            Some('C' | 'c') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        None => Keyword::CONCAT,
                                        Some('_') => {
                                            if matches!(cs.next(), Some('W' | 'w'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                            {
                                                Keyword::CONCAT_WS
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('U' | 'u') => match cs.next() {
                                    Some('R' | 'r') => match cs.next() {
                                        Some('R' | 'r') => match cs.next() {
                                            Some('E' | 'e') => match cs.next() {
                                                Some('N' | 'n') => match cs.next() {
                                                    Some('T' | 't') => match cs.next() {
                                                        None => Keyword::CONCURRENT,
                                                        Some('L' | 'l') => {
                                                            if matches!(cs.next(), Some('Y' | 'y'))
                                                            {
                                                                Keyword::CONCURRENTLY
                                                            } else {
                                                                Keyword::NOT_A_KEYWORD
                                                            }
                                                        }
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                {
                                    Keyword::CONDITION
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('F' | 'f') => {
                                if matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::CONFLICT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('N' | 'n') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                {
                                    Keyword::CONNECTION
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => {
                                match cs.next() {
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                        {
                                            Keyword::CONSISTENT
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('T' | 't') => {
                                        match cs.next() {
                                            Some('R' | 'r') => {
                                                match cs.next() {
                                                    Some('A' | 'a') => match cs.next() {
                                                        Some('I' | 'i') => match cs.next() {
                                                            Some('N' | 'n') => match cs.next() {
                                                                Some('T' | 't') => {
                                                                    match cs.next() {
                                                                        None => Keyword::CONSTRAINT,
                                                                        Some('_') => {
                                                                            match cs.next() {
                                                                                                    Some('C' | 'c') => {
                                                                                                        if matches!(cs.next(), Some('A' | 'a'))
                                                                                                          && matches!(cs.next(), Some('T' | 't'))
                                                                                                          && matches!(cs.next(), Some('A' | 'a'))
                                                                                                          && matches!(cs.next(), Some('L' | 'l'))
                                                                                                          && matches!(cs.next(), Some('O' | 'o'))
                                                                                                          && matches!(cs.next(), Some('G' | 'g')) {
                                                                                                            Keyword::CONSTRAINT_CATALOG
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    Some('N' | 'n') => {
                                                                                                        if matches!(cs.next(), Some('A' | 'a'))
                                                                                                          && matches!(cs.next(), Some('M' | 'm'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e')) {
                                                                                                            Keyword::CONSTRAINT_NAME
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    Some('S' | 's') => {
                                                                                                        if matches!(cs.next(), Some('C' | 'c'))
                                                                                                          && matches!(cs.next(), Some('H' | 'h'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('M' | 'm'))
                                                                                                          && matches!(cs.next(), Some('A' | 'a')) {
                                                                                                            Keyword::CONSTRAINT_SCHEMA
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                }
                                                                        }
                                                                        _ => Keyword::NOT_A_KEYWORD,
                                                                    }
                                                                }
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                }
                            }
                            Some('T' | 't') => match cs.next() {
                                Some('A' | 'a') => {
                                    if matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                    {
                                        Keyword::CONTAINS
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('E' | 'e') => {
                                    if matches!(cs.next(), Some('X' | 'x'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::CONTEXT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('I' | 'i') => {
                                    if matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::CONTINUE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('R' | 'r') => {
                                    if matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('B' | 'b'))
                                        && matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                    {
                                        Keyword::CONTRIBUTORS
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('V' | 'v') => match cs.next() {
                                None => Keyword::CONV,
                                Some('E' | 'e') => match cs.next() {
                                    Some('R' | 'r') => match cs.next() {
                                        Some('T' | 't') => match cs.next() {
                                            None => Keyword::CONVERT,
                                            Some('_') => {
                                                if matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('Z' | 'z'))
                                                {
                                                    Keyword::CONVERT_TZ
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('P' | 'p') => {
                            if matches!(cs.next(), Some('Y' | 'y')) {
                                Keyword::COPY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => match cs.next() {
                            None => Keyword::COS,
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('S' | 's')) {
                                    Keyword::COSTS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('T' | 't') => Keyword::COT,
                        Some('U' | 'u') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::COUNT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('U' | 'u')) {
                            Keyword::CPU
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => match cs.next() {
                        Some('C' | 'c') => match cs.next() {
                            Some('3') => match cs.next() {
                                Some('2') => match cs.next() {
                                    None => Keyword::CRC32,
                                    Some('C' | 'c') => Keyword::CRC32C,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('E' | 'e') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        None => Keyword::CREATE,
                                        Some('D' | 'd') => {
                                            if matches!(cs.next(), Some('B' | 'b')) {
                                                Keyword::CREATEDB
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('R' | 'r') => {
                                            if matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('L' | 'l'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                            {
                                                Keyword::CREATEROLE
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::CROSS
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => {
                        match cs.next() {
                            Some('B' | 'b') => {
                                if matches!(cs.next(), Some('E' | 'e')) {
                                    Keyword::CUBE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('R' | 'r') => match cs.next() {
                                Some('D' | 'd') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::CURDATE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('R' | 'r') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        Some('N' | 'n') => match cs.next() {
                                            Some('T' | 't') => match cs.next() {
                                                None => Keyword::CURRENT,
                                                Some('_') => match cs.next() {
                                                    Some('D' | 'd') => {
                                                        if matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                        {
                                                            Keyword::CURRENT_DATE
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('P' | 'p') => {
                                                        if matches!(cs.next(), Some('O' | 'o'))
                                                            && matches!(cs.next(), Some('S' | 's'))
                                                        {
                                                            Keyword::CURRENT_POS
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('R' | 'r') => {
                                                        if matches!(cs.next(), Some('O' | 'o'))
                                                            && matches!(cs.next(), Some('L' | 'l'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                        {
                                                            Keyword::CURRENT_ROLE
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('T' | 't') => {
                                                        match cs.next() {
                                                            Some('I' | 'i') => match cs.next() {
                                                                Some('M' | 'm') => match cs.next() {
                                                                    Some('E' | 'e') => {
                                                                        match cs.next() {
                                                                                                            None => Keyword::CURRENT_TIME,
                                                                                                            Some('S' | 's') => {
                                                                                                                if matches!(cs.next(), Some('T' | 't'))
                                                                                                                  && matches!(cs.next(), Some('A' | 'a'))
                                                                                                                  && matches!(cs.next(), Some('M' | 'm'))
                                                                                                                  && matches!(cs.next(), Some('P' | 'p')) {
                                                                                                                    Keyword::CURRENT_TIMESTAMP
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                        }
                                                                    }
                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                },
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        }
                                                    }
                                                    Some('U' | 'u') => {
                                                        if matches!(cs.next(), Some('S' | 's'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                            && matches!(cs.next(), Some('R' | 'r'))
                                                        {
                                                            Keyword::CURRENT_USER
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('S' | 's') => match cs.next() {
                                    Some('O' | 'o') => match cs.next() {
                                        Some('R' | 'r') => match cs.next() {
                                            None => Keyword::CURSOR,
                                            Some('_') => {
                                                if matches!(cs.next(), Some('N' | 'n'))
                                                    && matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('M' | 'm'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                {
                                                    Keyword::CURSOR_NAME
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('T' | 't') => {
                                    if matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('M' | 'm'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::CURTIME
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        }
                    }
                    Some('Y' | 'y') => {
                        if matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::CYCLE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                }
            }
            Some('D' | 'd') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('T' | 't') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            None => Keyword::DATA,
                            Some('B' | 'b') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('S' | 's') => match cs.next() {
                                        Some('E' | 'e') => match cs.next() {
                                            None => Keyword::DATABASE,
                                            Some('S' | 's') => Keyword::DATABASES,
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('F' | 'f') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::DATAFILE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::DATE,
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                {
                                    Keyword::DATEDIFF
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('M' | 'm'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::DATETIME
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => match cs.next() {
                                Some('A' | 'a') => {
                                    if matches!(cs.next(), Some('D' | 'd'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                    {
                                        Keyword::DATE_ADD
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('F' | 'f') => {
                                    if matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('M' | 'm'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::DATE_FORMAT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => {
                                    if matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('B' | 'b'))
                                    {
                                        Keyword::DATE_SUB
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('Y' | 'y') => match cs.next() {
                        None => Keyword::DAY,
                        Some('N' | 'n') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::DAYNAME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => match cs.next() {
                            Some('F' | 'f') => match cs.next() {
                                Some('M' | 'm') => {
                                    if matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('H' | 'h'))
                                    {
                                        Keyword::DAYOFMONTH
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('W' | 'w') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('K' | 'k'))
                                    {
                                        Keyword::DAYOFWEEK
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('Y' | 'y') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                    {
                                        Keyword::DAYOFYEAR
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('_') => match cs.next() {
                            Some('H' | 'h') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                {
                                    Keyword::DAY_HOUR
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('M' | 'm') => match cs.next() {
                                Some('I' | 'i') => match cs.next() {
                                    Some('C' | 'c') => {
                                        if matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::DAY_MICROSECOND
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('N' | 'n') => {
                                        if matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::DAY_MINUTE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('S' | 's') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('D' | 'd'))
                                {
                                    Keyword::DAY_SECOND
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::DEALLOCATE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        None => Keyword::DEC,
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                            {
                                Keyword::DECIMAL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::DECLARE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('F' | 'f') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::DEFAULT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::DEFINER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::DEGREES
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('Y' | 'y') => match cs.next() {
                                Some('E' | 'e') => {
                                    if matches!(cs.next(), Some('D' | 'd')) {
                                        Keyword::DELAYED
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('_') => {
                                    if matches!(cs.next(), Some('K' | 'k'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('Y' | 'y'))
                                        && matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('W' | 'w'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::DELAY_KEY_WRITE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('E' | 'e') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    None => Keyword::DELETE,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('D' | 'd'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::DELETE_DOMAIN_ID
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::DELIMITER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => match cs.next() {
                        Some('C' | 'c') => match cs.next() {
                            None => Keyword::DESC,
                            Some('R' | 'r') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('B' | 'b'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::DESCRIBE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('_') => {
                            if matches!(cs.next(), Some('K' | 'k'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('F' | 'f'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::DES_KEY_FILE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('C' | 'c'))
                        {
                            Keyword::DETERMINISTIC
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::DIAGNOSTICS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                        {
                            Keyword::DIRECTORY
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('B' | 'b'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::DISABLE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::DISCARD
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('K' | 'k') => Keyword::DISK,
                        Some('T' | 't') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('N' | 'n') => match cs.next() {
                                    Some('C' | 'c') => match cs.next() {
                                        Some('T' | 't') => match cs.next() {
                                            None => Keyword::DISTINCT,
                                            Some('R' | 'r') => {
                                                if matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('W' | 'w'))
                                                {
                                                    Keyword::DISTINCTROW
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('V' | 'v') => Keyword::DIV,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    None => Keyword::DO,
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::DOMAIN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('B' | 'b'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::DOUBLE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('_') => {
                        if matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('_'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::DO_DOMAIN_IDS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('R' | 'r') => {
                    if matches!(cs.next(), Some('O' | 'o')) && matches!(cs.next(), Some('P' | 'p'))
                    {
                        Keyword::DROP
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('U' | 'u') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('L' | 'l')) {
                            Keyword::DUAL
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('F' | 'f'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::DUMPFILE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::DUPLICATE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('Y' | 'y') => {
                    if matches!(cs.next(), Some('N' | 'n'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('M' | 'm'))
                        && matches!(cs.next(), Some('I' | 'i'))
                        && matches!(cs.next(), Some('C' | 'c'))
                    {
                        Keyword::DYNAMIC
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('E' | 'e') => match cs.next() {
                Some('A' | 'a') => {
                    if matches!(cs.next(), Some('C' | 'c')) && matches!(cs.next(), Some('H' | 'h'))
                    {
                        Keyword::EACH
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('L' | 'l') => match cs.next() {
                    Some('S' | 's') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::ELSE,
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('F' | 'f')) {
                                    Keyword::ELSEIF
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('F' | 'f')) {
                                Keyword::ELSIF
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => Keyword::ELT,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('M' | 'm') => {
                    if matches!(cs.next(), Some('P' | 'p'))
                        && matches!(cs.next(), Some('T' | 't'))
                        && matches!(cs.next(), Some('Y' | 'y'))
                    {
                        Keyword::EMPTY
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('N' | 'n') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('B' | 'b'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::ENABLE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::ENCLOSED
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('R' | 'r') => match cs.next() {
                            Some('Y' | 'y') => match cs.next() {
                                Some('P' | 'p') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        Some('E' | 'e') => {
                                            if matches!(cs.next(), Some('D' | 'd')) {
                                                Keyword::ENCRYPTED
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('I' | 'i') => {
                                            if matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                            {
                                                Keyword::ENCRYPTION
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('D' | 'd') => match cs.next() {
                        None => Keyword::END,
                        Some('S' | 's') => Keyword::ENDS,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('G' | 'g') => match cs.next() {
                        Some('I' | 'i') => match cs.next() {
                            Some('N' | 'n') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    None => Keyword::ENGINE,
                                    Some('S' | 's') => Keyword::ENGINES,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('B' | 'b'))
                                            && matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::ENGINE_ATTRIBUTE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('M' | 'm')) {
                            Keyword::ENUM
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('R' | 'r') => match cs.next() {
                    Some('R' | 'r') => match cs.next() {
                        Some('O' | 'o') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                None => Keyword::ERROR,
                                Some('S' | 's') => Keyword::ERRORS,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('S' | 's') => match cs.next() {
                    Some('C' | 'c') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('P' | 'p') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    None => Keyword::ESCAPE,
                                    Some('D' | 'd') => Keyword::ESCAPED,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('V' | 'v') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('N' | 'n') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                None => Keyword::EVENT,
                                Some('S' | 's') => Keyword::EVENTS,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('R' | 'r') => {
                            if matches!(cs.next(), Some('Y' | 'y')) {
                                Keyword::EVERY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('X' | 'x') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::EXAMINED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('P' | 'p') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    None => Keyword::EXCEPT,
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                        {
                                            Keyword::EXCEPTION
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('H' | 'h') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::EXCHANGE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('L' | 'l') => match cs.next() {
                            Some('U' | 'u') => match cs.next() {
                                Some('D' | 'd') => {
                                    if matches!(cs.next(), Some('E' | 'e')) {
                                        Keyword::EXCLUDE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => {
                                    if matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('V' | 'v'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::EXCLUSIVE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::EXECUTE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => match cs.next() {
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::EXISTS
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('T' | 't') => Keyword::EXIT,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('P' | 'p') => match cs.next() {
                        None => Keyword::EXP,
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::EXPANSION
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::EXPIRE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::EXPLAIN
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    None => Keyword::EXPORT,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                        {
                                            Keyword::EXPORT_SET
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('N' | 'n') => match cs.next() {
                                Some('D' | 'd') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                    {
                                        Keyword::EXTENDED
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => {
                                    if matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                    {
                                        Keyword::EXTENSION
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('T' | 't') => {
                                    if matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('Z' | 'z'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::EXTENT_SIZE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('R' | 'r') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('C' | 'c') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        None => Keyword::EXTRACT,
                                        Some('V' | 'v') => {
                                            if matches!(cs.next(), Some('A' | 'a'))
                                                && matches!(cs.next(), Some('L' | 'l'))
                                                && matches!(cs.next(), Some('U' | 'u'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                            {
                                                Keyword::EXTRACTVALUE
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('F' | 'f') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('L' | 'l') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::FALSE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                        {
                            Keyword::FAMILY
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('T' | 't')) {
                            Keyword::FAST
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::FAULTS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('D' | 'd') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::FEDERATED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('H' | 'h'))
                        {
                            Keyword::FETCH
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('L' | 'l') => match cs.next() {
                            Some('D' | 'd') => match cs.next() {
                                None => Keyword::FIELD,
                                Some('S' | 's') => Keyword::FIELDS,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('L' | 'l') => {
                        if matches!(cs.next(), Some('E' | 'e')) {
                            Keyword::FILE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('_'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('_'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::FIND_IN_SET
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::FIRST
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('X' | 'x') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::FIXED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('L' | 'l') => match cs.next() {
                    Some('O' | 'o') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                None => Keyword::FLOAT,
                                Some('4') => Keyword::FLOAT4,
                                Some('8') => Keyword::FLOAT8,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('R' | 'r')) {
                                Keyword::FLOOR
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('H' | 'h'))
                        {
                            Keyword::FLUSH
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    Some('L' | 'l') => match cs.next() {
                        Some('L' | 'l') => match cs.next() {
                            Some('O' | 'o') => match cs.next() {
                                Some('W' | 'w') => match cs.next() {
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('G' | 'g'))
                                        {
                                            Keyword::FOLLOWING
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => Keyword::FOLLOWS,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('R' | 'r') => match cs.next() {
                        None => Keyword::FOR,
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::FORCE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::FOREIGN
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('M' | 'm') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::FORMAT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::FOUND
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('R' | 'r') => match cs.next() {
                    Some('O' | 'o') => match cs.next() {
                        Some('M' | 'm') => match cs.next() {
                            None => Keyword::FROM,
                            Some('_') => match cs.next() {
                                Some('B' | 'b') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('6'))
                                        && matches!(cs.next(), Some('4'))
                                    {
                                        Keyword::FROM_BASE64
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('D' | 'd') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('Y' | 'y'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                    {
                                        Keyword::FROM_DAYS
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('U' | 'u') => {
                                    if matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('X' | 'x'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('M' | 'm'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::FROM_UNIXTIME
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('U' | 'u') => match cs.next() {
                    Some('L' | 'l') => match cs.next() {
                        Some('L' | 'l') => match cs.next() {
                            None => Keyword::FULL,
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('X' | 'x'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::FULLTEXT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::FUNCTION
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('G' | 'g') => match cs.next() {
                Some('E' | 'e') => match cs.next() {
                    Some('N' | 'n') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('L' | 'l') => Keyword::GENERAL,
                                    Some('T' | 't') => {
                                        if matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::GENERATED
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::GET,
                        Some('_') => {
                            if matches!(cs.next(), Some('F' | 'f'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::GET_FORMAT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('N' | 'n') => Keyword::GIN,
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('T' | 't')) {
                            Keyword::GIST
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('L' | 'l') => {
                    if matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('B' | 'b'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('L' | 'l'))
                    {
                        Keyword::GLOBAL
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('O' | 'o') => {
                    if matches!(cs.next(), Some('T' | 't')) && matches!(cs.next(), Some('O' | 'o'))
                    {
                        Keyword::GOTO
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('R' | 'r') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('N' | 'n') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                None => Keyword::GRANT,
                                Some('S' | 's') => Keyword::GRANTS,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::GREATEST
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => match cs.next() {
                        Some('U' | 'u') => match cs.next() {
                            Some('P' | 'p') => match cs.next() {
                                None => Keyword::GROUP,
                                Some('_') => {
                                    if matches!(cs.next(), Some('C' | 'c'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('C' | 'c'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::GROUP_CONCAT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('H' | 'h') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::HANDLER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('D' | 'd')) {
                            Keyword::HARD
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => match cs.next() {
                        Some('H' | 'h') => match cs.next() {
                            None => Keyword::HASH,
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('S' | 's')) {
                                    Keyword::HASHES
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('V' | 'v') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('G' | 'g'))
                        {
                            Keyword::HAVING
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('L' | 'l') => {
                        if matches!(cs.next(), Some('P' | 'p')) {
                            Keyword::HELP
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('X' | 'x') => Keyword::HEX,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('_'))
                            && matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                        {
                            Keyword::HIGH_PRIORITY
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                        {
                            Keyword::HISTORY
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('N' | 'n') => {
                    if matches!(cs.next(), Some('S' | 's')) && matches!(cs.next(), Some('W' | 'w'))
                    {
                        Keyword::HNSW
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('O' | 'o') => match cs.next() {
                    Some('S' | 's') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            None => Keyword::HOST,
                            Some('S' | 's') => Keyword::HOSTS,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => match cs.next() {
                        Some('R' | 'r') => match cs.next() {
                            None => Keyword::HOUR,
                            Some('_') => match cs.next() {
                                Some('M' | 'm') => match cs.next() {
                                    Some('I' | 'i') => match cs.next() {
                                        Some('C' | 'c') => {
                                            if matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('C' | 'c'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('D' | 'd'))
                                            {
                                                Keyword::HOUR_MICROSECOND
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('N' | 'n') => {
                                            if matches!(cs.next(), Some('U' | 'u'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                            {
                                                Keyword::HOUR_MINUTE
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('S' | 's') => {
                                    if matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('C' | 'c'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                    {
                                        Keyword::HOUR_SECOND
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('I' | 'i') => match cs.next() {
                Some('D' | 'd') => match cs.next() {
                    None => Keyword::ID,
                    Some('E' | 'e') => match cs.next() {
                        Some('N' | 'n') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                Some('I' | 'i') => match cs.next() {
                                    Some('F' | 'f') => {
                                        if matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::IDENTIFIED
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('T' | 't') => {
                                        if matches!(cs.next(), Some('Y' | 'y')) {
                                            Keyword::IDENTITY
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('F' | 'f') => match cs.next() {
                    None => Keyword::IF,
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('L' | 'l'))
                        {
                            Keyword::IFNULL
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('G' | 'g') => match cs.next() {
                    Some('N' | 'n') => match cs.next() {
                        Some('O' | 'o') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    None => Keyword::IGNORE,
                                    Some('D' | 'd') => Keyword::IGNORED,
                                    Some('_') => match cs.next() {
                                        Some('D' | 'd') => {
                                            if matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('M' | 'm'))
                                                && matches!(cs.next(), Some('A' | 'a'))
                                                && matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('D' | 'd'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                            {
                                                Keyword::IGNORE_DOMAIN_IDS
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('S' | 's') => {
                                            if matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('V' | 'v'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('D' | 'd'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                            {
                                                Keyword::IGNORE_SERVER_IDS
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('M' | 'm') => match cs.next() {
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::IMMEDIATE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::IMPORT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('N' | 'n') => match cs.next() {
                    None => Keyword::IN,
                    Some('C' | 'c') => match cs.next() {
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('D' | 'd'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::INCLUDE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('R' | 'r') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::INCREMENT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('D' | 'd') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('X' | 'x') => match cs.next() {
                                None => Keyword::INDEX,
                                Some('E' | 'e') => {
                                    if matches!(cs.next(), Some('S' | 's')) {
                                        Keyword::INDEXES
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            Some('4') => Keyword::INET4,
                            Some('6') => Keyword::INET6,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('F' | 'f') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::INFILE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('H' | 'h') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                Some('I' | 'i') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        None => Keyword::INHERIT,
                                        Some('S' | 's') => Keyword::INHERITS,
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('_'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('Z' | 'z'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::INITIAL_SIZE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::INNER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::INOUT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::INPLACE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('N' | 'n') => {
                                if matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('V' | 'v'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::INSENSITIVE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('R' | 'r') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    None => Keyword::INSERT,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('H' | 'h'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::INSERT_METHOD
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('T' | 't') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('L' | 'l') => {
                                    if matches!(cs.next(), Some('L' | 'l')) {
                                        Keyword::INSTALL
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('N' | 'n') => {
                                    if matches!(cs.next(), Some('T' | 't')) {
                                        Keyword::INSTANT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('R' | 'r') => Keyword::INSTR,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::INT,
                        Some('1') => Keyword::INT1,
                        Some('2') => match cs.next() {
                            None => Keyword::INT2,
                            Some('_') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('P' | 'p'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::INT2_OPS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('3') => Keyword::INT3,
                        Some('4') => match cs.next() {
                            None => Keyword::INT4,
                            Some('_') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('P' | 'p'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::INT4_OPS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('8') => match cs.next() {
                            None => Keyword::INT8,
                            Some('_') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('P' | 'p'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::INT8_OPS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('E' | 'e') => match cs.next() {
                            Some('G' | 'g') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                {
                                    Keyword::INTEGER
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('R' | 'r') => match cs.next() {
                                Some('S' | 's') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        Some('C' | 'c') => match cs.next() {
                                            Some('T' | 't') => match cs.next() {
                                                None => Keyword::INTERSECT,
                                                Some('A' | 'a') => Keyword::INTERSECTA,
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('V' | 'v') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                    {
                                        Keyword::INTERVAL
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('O' | 'o') => Keyword::INTO,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('V' | 'v') => match cs.next() {
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('B' | 'b'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::INVISIBLE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('K' | 'k'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::INVOKER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    None => Keyword::IO,
                    Some('_') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::IO_THREAD
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => {
                    if matches!(cs.next(), Some('C' | 'c')) {
                        Keyword::IPC
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('S' | 's') => match cs.next() {
                    None => Keyword::IS,
                    Some('O' | 'o') => match cs.next() {
                        Some('L' | 'l') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::ISOLATION
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('P' | 'p') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::ISOPEN
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::ISSUER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('T' | 't') => {
                    if matches!(cs.next(), Some('E' | 'e'))
                        && matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('T' | 't'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::ITERATE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('J' | 'j') => match cs.next() {
                Some('O' | 'o') => {
                    if matches!(cs.next(), Some('I' | 'i')) && matches!(cs.next(), Some('N' | 'n'))
                    {
                        Keyword::JOIN
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('S' | 's') => {
                    match cs.next() {
                        Some('O' | 'o') => {
                            match cs.next() {
                                Some('N' | 'n') => {
                                    match cs.next() {
                                        None => Keyword::JSON,
                                        Some('_') => match cs.next() {
                                            Some('A' | 'a') => {
                                                match cs.next() {
                                                    Some('R' | 'r') => {
                                                        match cs.next() {
                                                            Some('R' | 'r') => match cs.next() {
                                                                Some('A' | 'a') => match cs.next() {
                                                                    Some('Y' | 'y') => {
                                                                        match cs.next() {
                                                                                            None => Keyword::JSON_ARRAY,
                                                                                            Some('A' | 'a') => {
                                                                                                if matches!(cs.next(), Some('G' | 'g'))
                                                                                                  && matches!(cs.next(), Some('G' | 'g')) {
                                                                                                    Keyword::JSON_ARRAYAGG
                                                                                                } else {
                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                }
                                                                                            }
                                                                                            Some('_') => {
                                                                                                match cs.next() {
                                                                                                    Some('A' | 'a') => {
                                                                                                        if matches!(cs.next(), Some('P' | 'p'))
                                                                                                          && matches!(cs.next(), Some('P' | 'p'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('N' | 'n'))
                                                                                                          && matches!(cs.next(), Some('D' | 'd')) {
                                                                                                            Keyword::JSON_ARRAY_APPEND
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    Some('I' | 'i') => {
                                                                                                        match cs.next() {
                                                                                                            Some('N' | 'n') => {
                                                                                                                match cs.next() {
                                                                                                                    Some('S' | 's') => {
                                                                                                                        if matches!(cs.next(), Some('E' | 'e'))
                                                                                                                          && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                          && matches!(cs.next(), Some('T' | 't')) {
                                                                                                                            Keyword::JSON_ARRAY_INSERT
                                                                                                                        } else {
                                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                                        }
                                                                                                                    }
                                                                                                                    Some('T' | 't') => {
                                                                                                                        if matches!(cs.next(), Some('E' | 'e'))
                                                                                                                          && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                          && matches!(cs.next(), Some('S' | 's'))
                                                                                                                          && matches!(cs.next(), Some('E' | 'e'))
                                                                                                                          && matches!(cs.next(), Some('C' | 'c'))
                                                                                                                          && matches!(cs.next(), Some('T' | 't')) {
                                                                                                                            Keyword::JSON_ARRAY_INTERSECT
                                                                                                                        } else {
                                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                                        }
                                                                                                                    }
                                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                                }
                                                                                                            }
                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                        }
                                                                                                    }
                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                }
                                                                                            }
                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                        }
                                                                    }
                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                },
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                }
                                            }
                                            Some('C' | 'c') => match cs.next() {
                                                Some('O' | 'o') => match cs.next() {
                                                    Some('M' | 'm') => {
                                                        if matches!(cs.next(), Some('P' | 'p'))
                                                            && matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('C' | 'c'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                        {
                                                            Keyword::JSON_COMPACT
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('N' | 'n') => match cs.next() {
                                                        Some('T' | 't') => match cs.next() {
                                                            Some('A' | 'a') => match cs.next() {
                                                                Some('I' | 'i') => match cs.next() {
                                                                    Some('N' | 'n') => {
                                                                        match cs.next() {
                                                                                                            Some('S' | 's') => {
                                                                                                                match cs.next() {
                                                                                                                    None => Keyword::JSON_CONTAINS,
                                                                                                                    Some('_') => {
                                                                                                                        if matches!(cs.next(), Some('P' | 'p'))
                                                                                                                          && matches!(cs.next(), Some('A' | 'a'))
                                                                                                                          && matches!(cs.next(), Some('T' | 't'))
                                                                                                                          && matches!(cs.next(), Some('H' | 'h')) {
                                                                                                                            Keyword::JSON_CONTAINS_PATH
                                                                                                                        } else {
                                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                                        }
                                                                                                                    }
                                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                                }
                                                                                                            }
                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                        }
                                                                    }
                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                },
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('D' | 'd') => match cs.next() {
                                                Some('E' | 'e') => match cs.next() {
                                                    Some('P' | 'p') => {
                                                        if matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('H' | 'h'))
                                                        {
                                                            Keyword::JSON_DEPTH
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('T' | 't') => {
                                                        if matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('I' | 'i'))
                                                            && matches!(cs.next(), Some('L' | 'l'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                            && matches!(cs.next(), Some('D' | 'd'))
                                                        {
                                                            Keyword::JSON_DETAILED
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('E' | 'e') => match cs.next() {
                                                Some('Q' | 'q') => {
                                                    if matches!(cs.next(), Some('U' | 'u'))
                                                        && matches!(cs.next(), Some('A' | 'a'))
                                                        && matches!(cs.next(), Some('L' | 'l'))
                                                        && matches!(cs.next(), Some('S' | 's'))
                                                    {
                                                        Keyword::JSON_EQUALS
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('X' | 'x') => match cs.next() {
                                                    Some('I' | 'i') => {
                                                        if matches!(cs.next(), Some('S' | 's'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('S' | 's'))
                                                        {
                                                            Keyword::JSON_EXISTS
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('T' | 't') => {
                                                        if matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('C' | 'c'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                        {
                                                            Keyword::JSON_EXTRACT
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('I' | 'i') => {
                                                if matches!(cs.next(), Some('N' | 'n'))
                                                    && matches!(cs.next(), Some('S' | 's'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                {
                                                    Keyword::JSON_INSERT
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('K' | 'k') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('Y' | 'y'))
                                                    && matches!(cs.next(), Some('S' | 's'))
                                                {
                                                    Keyword::JSON_KEYS
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('L' | 'l') => match cs.next() {
                                                Some('E' | 'e') => {
                                                    if matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('G' | 'g'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('H' | 'h'))
                                                    {
                                                        Keyword::JSON_LENGTH
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('O' | 'o') => {
                                                    if matches!(cs.next(), Some('O' | 'o'))
                                                        && matches!(cs.next(), Some('S' | 's'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                    {
                                                        Keyword::JSON_LOOSE
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('M' | 'm') => match cs.next() {
                                                Some('E' | 'e') => match cs.next() {
                                                    Some('R' | 'r') => match cs.next() {
                                                        Some('G' | 'g') => match cs.next() {
                                                            Some('E' | 'e') => match cs.next() {
                                                                None => Keyword::JSON_MERGE,
                                                                Some('_') => match cs.next() {
                                                                    Some('P' | 'p') => {
                                                                        match cs.next() {
                                                                                                            Some('A' | 'a') => {
                                                                                                                if matches!(cs.next(), Some('T' | 't'))
                                                                                                                  && matches!(cs.next(), Some('C' | 'c'))
                                                                                                                  && matches!(cs.next(), Some('H' | 'h')) {
                                                                                                                    Keyword::JSON_MERGE_PATCH
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            Some('R' | 'r') => {
                                                                                                                if matches!(cs.next(), Some('E' | 'e'))
                                                                                                                  && matches!(cs.next(), Some('S' | 's'))
                                                                                                                  && matches!(cs.next(), Some('E' | 'e'))
                                                                                                                  && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                  && matches!(cs.next(), Some('V' | 'v'))
                                                                                                                  && matches!(cs.next(), Some('E' | 'e')) {
                                                                                                                    Keyword::JSON_MERGE_PRESERVE
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                        }
                                                                    }
                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                },
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('N' | 'n') => {
                                                if matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                    && matches!(cs.next(), Some('M' | 'm'))
                                                    && matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('L' | 'l'))
                                                    && matches!(cs.next(), Some('I' | 'i'))
                                                    && matches!(cs.next(), Some('Z' | 'z'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                {
                                                    Keyword::JSON_NORMALIZE
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('O' | 'o') => {
                                                match cs.next() {
                                                    Some('B' | 'b') => match cs.next() {
                                                        Some('J' | 'j') => match cs.next() {
                                                            Some('E' | 'e') => match cs.next() {
                                                                Some('C' | 'c') => match cs.next() {
                                                                    Some('T' | 't') => {
                                                                        match cs.next() {
                                                                                                    None => Keyword::JSON_OBJECT,
                                                                                                    Some('A' | 'a') => {
                                                                                                        if matches!(cs.next(), Some('G' | 'g'))
                                                                                                          && matches!(cs.next(), Some('G' | 'g')) {
                                                                                                            Keyword::JSON_OBJECTAGG
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    Some('_') => {
                                                                                                        match cs.next() {
                                                                                                            Some('F' | 'f') => {
                                                                                                                if matches!(cs.next(), Some('I' | 'i'))
                                                                                                                  && matches!(cs.next(), Some('L' | 'l'))
                                                                                                                  && matches!(cs.next(), Some('T' | 't'))
                                                                                                                  && matches!(cs.next(), Some('E' | 'e'))
                                                                                                                  && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                  && matches!(cs.next(), Some('_'))
                                                                                                                  && matches!(cs.next(), Some('K' | 'k'))
                                                                                                                  && matches!(cs.next(), Some('E' | 'e'))
                                                                                                                  && matches!(cs.next(), Some('Y' | 'y'))
                                                                                                                  && matches!(cs.next(), Some('S' | 's')) {
                                                                                                                    Keyword::JSON_OBJECT_FILTER_KEYS
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            Some('T' | 't') => {
                                                                                                                if matches!(cs.next(), Some('O' | 'o'))
                                                                                                                  && matches!(cs.next(), Some('_'))
                                                                                                                  && matches!(cs.next(), Some('A' | 'a'))
                                                                                                                  && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                  && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                  && matches!(cs.next(), Some('A' | 'a'))
                                                                                                                  && matches!(cs.next(), Some('Y' | 'y')) {
                                                                                                                    Keyword::JSON_OBJECT_TO_ARRAY
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                        }
                                                                                                    }
                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                }
                                                                    }
                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                },
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    Some('V' | 'v') => {
                                                        if matches!(cs.next(), Some('E' | 'e'))
                                                            && matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('L' | 'l'))
                                                            && matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('P' | 'p'))
                                                            && matches!(cs.next(), Some('S' | 's'))
                                                        {
                                                            Keyword::JSON_OVERLAPS
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                }
                                            }
                                            Some('P' | 'p') => {
                                                if matches!(cs.next(), Some('R' | 'r'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('Y' | 'y'))
                                                {
                                                    Keyword::JSON_PRETTY
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('Q' | 'q') => match cs.next() {
                                                Some('U' | 'u') => match cs.next() {
                                                    Some('E' | 'e') => {
                                                        if matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('Y' | 'y'))
                                                        {
                                                            Keyword::JSON_QUERY
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('O' | 'o') => {
                                                        if matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                        {
                                                            Keyword::JSON_QUOTE
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('R' | 'r') => match cs.next() {
                                                Some('E' | 'e') => match cs.next() {
                                                    Some('M' | 'm') => {
                                                        if matches!(cs.next(), Some('O' | 'o'))
                                                            && matches!(cs.next(), Some('V' | 'v'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                        {
                                                            Keyword::JSON_REMOVE
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('P' | 'p') => {
                                                        if matches!(cs.next(), Some('L' | 'l'))
                                                            && matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('C' | 'c'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                        {
                                                            Keyword::JSON_REPLACE
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('S' | 's') => match cs.next() {
                                                Some('C' | 'c') => {
                                                    if matches!(cs.next(), Some('H' | 'h'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('M' | 'm'))
                                                        && matches!(cs.next(), Some('A' | 'a'))
                                                        && matches!(cs.next(), Some('_'))
                                                        && matches!(cs.next(), Some('V' | 'v'))
                                                        && matches!(cs.next(), Some('A' | 'a'))
                                                        && matches!(cs.next(), Some('L' | 'l'))
                                                        && matches!(cs.next(), Some('I' | 'i'))
                                                        && matches!(cs.next(), Some('D' | 'd'))
                                                    {
                                                        Keyword::JSON_SCHEMA_VALID
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('E' | 'e') => match cs.next() {
                                                    Some('A' | 'a') => {
                                                        if matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('C' | 'c'))
                                                            && matches!(cs.next(), Some('H' | 'h'))
                                                        {
                                                            Keyword::JSON_SEARCH
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('T' | 't') => Keyword::JSON_SET,
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('T' | 't') => match cs.next() {
                                                Some('A' | 'a') => {
                                                    if matches!(cs.next(), Some('B' | 'b'))
                                                        && matches!(cs.next(), Some('L' | 'l'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                    {
                                                        Keyword::JSON_TABLE
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('Y' | 'y') => {
                                                    if matches!(cs.next(), Some('P' | 'p'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                    {
                                                        Keyword::JSON_TYPE
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('U' | 'u') => {
                                                if matches!(cs.next(), Some('N' | 'n'))
                                                    && matches!(cs.next(), Some('Q' | 'q'))
                                                    && matches!(cs.next(), Some('U' | 'u'))
                                                    && matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                {
                                                    Keyword::JSON_UNQUOTE
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('V' | 'v') => match cs.next() {
                                                Some('A' | 'a') => match cs.next() {
                                                    Some('L' | 'l') => match cs.next() {
                                                        Some('I' | 'i') => {
                                                            if matches!(cs.next(), Some('D' | 'd'))
                                                            {
                                                                Keyword::JSON_VALID
                                                            } else {
                                                                Keyword::NOT_A_KEYWORD
                                                            }
                                                        }
                                                        Some('U' | 'u') => {
                                                            if matches!(cs.next(), Some('E' | 'e'))
                                                            {
                                                                Keyword::JSON_VALUE
                                                            } else {
                                                                Keyword::NOT_A_KEYWORD
                                                            }
                                                        }
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('K' | 'k') => match cs.next() {
                Some('E' | 'e') => match cs.next() {
                    Some('Y' | 'y') => match cs.next() {
                        None => Keyword::KEY,
                        Some('S' | 's') => Keyword::KEYS,
                        Some('_') => {
                            if matches!(cs.next(), Some('B' | 'b'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('K' | 'k'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('Z' | 'z'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::KEY_BLOCK_SIZE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => {
                    if matches!(cs.next(), Some('L' | 'l')) && matches!(cs.next(), Some('L' | 'l'))
                    {
                        Keyword::KILL
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('L' | 'l') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('G' | 'g') => Keyword::LAG,
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::LANGUAGE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            None => Keyword::LAST,
                            Some('V' | 'v') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                {
                                    Keyword::LASTVAL
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => match cs.next() {
                                Some('D' | 'd') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('Y' | 'y'))
                                    {
                                        Keyword::LAST_DAY
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('V' | 'v') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::LAST_VALUE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('C' | 'c') => {
                    if matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('S' | 's'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::LCASE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('E' | 'e') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('D' | 'd') => match cs.next() {
                            None => Keyword::LEAD,
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('G' | 'g'))
                                {
                                    Keyword::LEADING
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('T' | 't')) {
                                Keyword::LEAST
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('V' | 'v') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                None => Keyword::LEAVE,
                                Some('S' | 's') => Keyword::LEAVES,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('F' | 'f') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            None => Keyword::LEFT,
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('G' | 'g'))
                                {
                                    Keyword::LEFTARG
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('N' | 'n') => match cs.next() {
                        Some('G' | 'g') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                Some('H' | 'h') => match cs.next() {
                                    None => Keyword::LENGTH,
                                    Some('B' | 'b') => Keyword::LENGTHB,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('S' | 's')) {
                            Keyword::LESS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('V' | 'v') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('L' | 'l'))
                        {
                            Keyword::LEVEL
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('K' | 'k') => {
                        if matches!(cs.next(), Some('E' | 'e')) {
                            Keyword::LIKE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::LIMIT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('R' | 'r')) {
                                    Keyword::LINEAR
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => Keyword::LINES,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('T' | 't')) {
                            Keyword::LIST
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('N' | 'n') => Keyword::LN,
                Some('O' | 'o') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('D' | 'd') => match cs.next() {
                            None => Keyword::LOAD,
                            Some('_') => {
                                if matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::LOAD_FILE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('C' | 'c') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('L' | 'l') => match cs.next() {
                                None => Keyword::LOCAL,
                                Some('T' | 't') => match cs.next() {
                                    Some('I' | 'i') => match cs.next() {
                                        Some('M' | 'm') => match cs.next() {
                                            Some('E' | 'e') => match cs.next() {
                                                None => Keyword::LOCALTIME,
                                                Some('S' | 's') => {
                                                    if matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('A' | 'a'))
                                                        && matches!(cs.next(), Some('M' | 'm'))
                                                        && matches!(cs.next(), Some('P' | 'p'))
                                                    {
                                                        Keyword::LOCALTIMESTAMP
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('E' | 'e')) {
                                    Keyword::LOCATE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('K' | 'k') => match cs.next() {
                            None => Keyword::LOCK,
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('D' | 'd')) {
                                    Keyword::LOCKED
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => Keyword::LOCKS,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('G' | 'g') => match cs.next() {
                        None => Keyword::LOG,
                        Some('1') => {
                            if matches!(cs.next(), Some('0')) {
                                Keyword::LOG10
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('2') => Keyword::LOG2,
                        Some('F' | 'f') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::LOGFILE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('N' | 'n')) {
                                Keyword::LOGIN
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => Keyword::LOGS,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('N' | 'n') => match cs.next() {
                        Some('G' | 'g') => match cs.next() {
                            None => Keyword::LONG,
                            Some('B' | 'b') => {
                                if matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('B' | 'b'))
                                {
                                    Keyword::LONGBLOB
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('X' | 'x'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::LONGTEXT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('P' | 'p')) {
                            Keyword::LOOP
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('W' | 'w') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r')) {
                                Keyword::LOWER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('_') => {
                            if matches!(cs.next(), Some('P' | 'p'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                            {
                                Keyword::LOW_PRIORITY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => {
                    if matches!(cs.next(), Some('A' | 'a')) && matches!(cs.next(), Some('D' | 'd'))
                    {
                        Keyword::LPAD
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('T' | 't') => {
                    if matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('I' | 'i'))
                        && matches!(cs.next(), Some('M' | 'm'))
                    {
                        Keyword::LTRIM
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('M' | 'm') => {
                match cs.next() {
                    Some('A' | 'a') => {
                        match cs.next() {
                            Some('K' | 'k') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    Some('D' | 'd') => {
                                        if matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::MAKEDATE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('T' | 't') => {
                                        if matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::MAKETIME
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('_') => {
                                        if matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                        {
                                            Keyword::MAKE_SET
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('S' | 's') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        Some('R' | 'r') => match cs.next() {
                                            None => Keyword::MASTER,
                                            Some('_') => match cs.next() {
                                                Some('C' | 'c') => {
                                                    if matches!(cs.next(), Some('O' | 'o'))
                                                        && matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('C' | 'c'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('_'))
                                                        && matches!(cs.next(), Some('R' | 'r'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('R' | 'r'))
                                                        && matches!(cs.next(), Some('Y' | 'y'))
                                                    {
                                                        Keyword::MASTER_CONNECT_RETRY
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('D' | 'd') => {
                                                    if matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('L' | 'l'))
                                                        && matches!(cs.next(), Some('A' | 'a'))
                                                        && matches!(cs.next(), Some('Y' | 'y'))
                                                    {
                                                        Keyword::MASTER_DELAY
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('G' | 'g') => {
                                                    if matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('I' | 'i'))
                                                        && matches!(cs.next(), Some('D' | 'd'))
                                                        && matches!(cs.next(), Some('_'))
                                                        && matches!(cs.next(), Some('P' | 'p'))
                                                        && matches!(cs.next(), Some('O' | 'o'))
                                                        && matches!(cs.next(), Some('S' | 's'))
                                                    {
                                                        Keyword::MASTER_GTID_POS
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('H' | 'h') => match cs.next() {
                                                    Some('E' | 'e') => {
                                                        if matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('B' | 'b'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                            && matches!(cs.next(), Some('A' | 'a'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                            && matches!(cs.next(), Some('_'))
                                                            && matches!(cs.next(), Some('P' | 'p'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                            && matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('I' | 'i'))
                                                            && matches!(cs.next(), Some('O' | 'o'))
                                                            && matches!(cs.next(), Some('D' | 'd'))
                                                        {
                                                            Keyword::MASTER_HEARTBEAT_PERIOD
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('O' | 'o') => {
                                                        if matches!(cs.next(), Some('S' | 's'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                        {
                                                            Keyword::MASTER_HOST
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                Some('L' | 'l') => {
                                                    match cs.next() {
                                                        Some('O' | 'o') => {
                                                            match cs.next() {
                                                                Some('G' | 'g') => {
                                                                    match cs.next() {
                                                                        Some('_') => {
                                                                            match cs.next() {
                                                                                                    Some('F' | 'f') => {
                                                                                                        if matches!(cs.next(), Some('I' | 'i'))
                                                                                                          && matches!(cs.next(), Some('L' | 'l'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e')) {
                                                                                                            Keyword::MASTER_LOG_FILE
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    Some('P' | 'p') => {
                                                                                                        if matches!(cs.next(), Some('O' | 'o'))
                                                                                                          && matches!(cs.next(), Some('S' | 's')) {
                                                                                                            Keyword::MASTER_LOG_POS
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                }
                                                                        }
                                                                        _ => Keyword::NOT_A_KEYWORD,
                                                                    }
                                                                }
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            }
                                                        }
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    }
                                                }
                                                Some('P' | 'p') => match cs.next() {
                                                    Some('A' | 'a') => {
                                                        if matches!(cs.next(), Some('S' | 's'))
                                                            && matches!(cs.next(), Some('S' | 's'))
                                                            && matches!(cs.next(), Some('W' | 'w'))
                                                            && matches!(cs.next(), Some('O' | 'o'))
                                                            && matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('D' | 'd'))
                                                        {
                                                            Keyword::MASTER_PASSWORD
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('O' | 'o') => {
                                                        if matches!(cs.next(), Some('R' | 'r'))
                                                            && matches!(cs.next(), Some('T' | 't'))
                                                        {
                                                            Keyword::MASTER_PORT
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                Some('S' | 's') => {
                                                    match cs.next() {
                                                        Some('E' | 'e') => {
                                                            if matches!(cs.next(), Some('R' | 'r'))
                                                                && matches!(
                                                                    cs.next(),
                                                                    Some('V' | 'v')
                                                                )
                                                                && matches!(
                                                                    cs.next(),
                                                                    Some('E' | 'e')
                                                                )
                                                                && matches!(
                                                                    cs.next(),
                                                                    Some('R' | 'r')
                                                                )
                                                                && matches!(cs.next(), Some('_'))
                                                                && matches!(
                                                                    cs.next(),
                                                                    Some('I' | 'i')
                                                                )
                                                                && matches!(
                                                                    cs.next(),
                                                                    Some('D' | 'd')
                                                                )
                                                            {
                                                                Keyword::MASTER_SERVER_ID
                                                            } else {
                                                                Keyword::NOT_A_KEYWORD
                                                            }
                                                        }
                                                        Some('S' | 's') => {
                                                            match cs.next() {
                                                                Some('L' | 'l') => {
                                                                    match cs.next() {
                                                                        None => Keyword::MASTER_SSL,
                                                                        Some('_') => {
                                                                            match cs.next() {
                                                                                                    Some('C' | 'c') => {
                                                                                                        match cs.next() {
                                                                                                            Some('A' | 'a') => {
                                                                                                                match cs.next() {
                                                                                                                    None => Keyword::MASTER_SSL_CA,
                                                                                                                    Some('P' | 'p') => {
                                                                                                                        if matches!(cs.next(), Some('A' | 'a'))
                                                                                                                          && matches!(cs.next(), Some('T' | 't'))
                                                                                                                          && matches!(cs.next(), Some('H' | 'h')) {
                                                                                                                            Keyword::MASTER_SSL_CAPATH
                                                                                                                        } else {
                                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                                        }
                                                                                                                    }
                                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                                }
                                                                                                            }
                                                                                                            Some('E' | 'e') => {
                                                                                                                if matches!(cs.next(), Some('R' | 'r'))
                                                                                                                  && matches!(cs.next(), Some('T' | 't')) {
                                                                                                                    Keyword::MASTER_SSL_CERT
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            Some('I' | 'i') => {
                                                                                                                if matches!(cs.next(), Some('P' | 'p'))
                                                                                                                  && matches!(cs.next(), Some('H' | 'h'))
                                                                                                                  && matches!(cs.next(), Some('E' | 'e'))
                                                                                                                  && matches!(cs.next(), Some('R' | 'r')) {
                                                                                                                    Keyword::MASTER_SSL_CIPHER
                                                                                                                } else {
                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                }
                                                                                                            }
                                                                                                            Some('R' | 'r') => {
                                                                                                                match cs.next() {
                                                                                                                    Some('L' | 'l') => {
                                                                                                                        match cs.next() {
                                                                                                                            None => Keyword::MASTER_SSL_CRL,
                                                                                                                            Some('P' | 'p') => {
                                                                                                                                if matches!(cs.next(), Some('A' | 'a'))
                                                                                                                                  && matches!(cs.next(), Some('T' | 't'))
                                                                                                                                  && matches!(cs.next(), Some('H' | 'h')) {
                                                                                                                                    Keyword::MASTER_SSL_CRLPATH
                                                                                                                                } else {
                                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                                }
                                                                                                                            }
                                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                                        }
                                                                                                                    }
                                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                                }
                                                                                                            }
                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                        }
                                                                                                    }
                                                                                                    Some('K' | 'k') => {
                                                                                                        if matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('Y' | 'y')) {
                                                                                                            Keyword::MASTER_SSL_KEY
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    Some('V' | 'v') => {
                                                                                                        if matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('R' | 'r'))
                                                                                                          && matches!(cs.next(), Some('I' | 'i'))
                                                                                                          && matches!(cs.next(), Some('F' | 'f'))
                                                                                                          && matches!(cs.next(), Some('Y' | 'y'))
                                                                                                          && matches!(cs.next(), Some('_'))
                                                                                                          && matches!(cs.next(), Some('S' | 's'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('R' | 'r'))
                                                                                                          && matches!(cs.next(), Some('V' | 'v'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('R' | 'r'))
                                                                                                          && matches!(cs.next(), Some('_'))
                                                                                                          && matches!(cs.next(), Some('C' | 'c'))
                                                                                                          && matches!(cs.next(), Some('E' | 'e'))
                                                                                                          && matches!(cs.next(), Some('R' | 'r'))
                                                                                                          && matches!(cs.next(), Some('T' | 't')) {
                                                                                                            Keyword::MASTER_SSL_VERIFY_SERVER_CERT
                                                                                                        } else {
                                                                                                            Keyword::NOT_A_KEYWORD
                                                                                                        }
                                                                                                    }
                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                }
                                                                        }
                                                                        _ => Keyword::NOT_A_KEYWORD,
                                                                    }
                                                                }
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            }
                                                        }
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    }
                                                }
                                                Some('U' | 'u') => match cs.next() {
                                                    Some('S' | 's') => match cs.next() {
                                                        Some('E' | 'e') => match cs.next() {
                                                            Some('R' | 'r') => Keyword::MASTER_USER,
                                                            Some('_') => {
                                                                if matches!(
                                                                    cs.next(),
                                                                    Some('G' | 'g')
                                                                ) && matches!(
                                                                    cs.next(),
                                                                    Some('T' | 't')
                                                                ) && matches!(
                                                                    cs.next(),
                                                                    Some('I' | 'i')
                                                                ) && matches!(
                                                                    cs.next(),
                                                                    Some('D' | 'd')
                                                                ) {
                                                                    Keyword::MASTER_USE_GTID
                                                                } else {
                                                                    Keyword::NOT_A_KEYWORD
                                                                }
                                                            }
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('T' | 't') => match cs.next() {
                                Some('C' | 'c') => {
                                    if matches!(cs.next(), Some('H' | 'h')) {
                                        Keyword::MATCH
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('E' | 'e') => {
                                    if matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('Z' | 'z'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                    {
                                        Keyword::MATERIALIZED
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('X' | 'x') => match cs.next() {
                                None => Keyword::MAX,
                                Some('V' | 'v') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::MAXVALUE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('_') => match cs.next() {
                                    Some('C' | 'c') => {
                                        if matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('P' | 'p'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('H' | 'h'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                        {
                                            Keyword::MAX_CONNECTIONS_PER_HOUR
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('Q' | 'q') => {
                                        if matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('P' | 'p'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('H' | 'h'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                        {
                                            Keyword::MAX_QUERIES_PER_HOUR
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('R' | 'r') => {
                                        if matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('W' | 'w'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                        {
                                            Keyword::MAX_ROWS
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => match cs.next() {
                                        Some('I' | 'i') => {
                                            if matches!(cs.next(), Some('Z' | 'z'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                            {
                                                Keyword::MAX_SIZE
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('T' | 't') => {
                                            if matches!(cs.next(), Some('A' | 'a'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('M' | 'm'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('M' | 'm'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                            {
                                                Keyword::MAX_STATEMENT_TIME
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    Some('U' | 'u') => match cs.next() {
                                        Some('P' | 'p') => {
                                            if matches!(cs.next(), Some('D' | 'd'))
                                                && matches!(cs.next(), Some('A' | 'a'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('P' | 'p'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('H' | 'h'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('U' | 'u'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                            {
                                                Keyword::MAX_UPDATES_PER_HOUR
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('S' | 's') => {
                                            if matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('C' | 'c'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('C' | 'c'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                            {
                                                Keyword::MAX_USER_CONNECTIONS
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        }
                    }
                    Some('E' | 'e') => match cs.next() {
                        Some('D' | 'd') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('U' | 'u') => match cs.next() {
                                    Some('M' | 'm') => match cs.next() {
                                        None => Keyword::MEDIUM,
                                        Some('B' | 'b') => {
                                            if matches!(cs.next(), Some('L' | 'l'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('B' | 'b'))
                                            {
                                                Keyword::MEDIUMBLOB
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('I' | 'i') => {
                                            if matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                            {
                                                Keyword::MEDIUMINT
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('T' | 't') => {
                                            if matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('X' | 'x'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                            {
                                                Keyword::MEDIUMTEXT
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('M' | 'm') => match cs.next() {
                            Some('B' | 'b') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                {
                                    Keyword::MEMBER
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('O' | 'o') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('Y' | 'y'))
                                {
                                    Keyword::MEMORY
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('R' | 'r') => match cs.next() {
                            Some('G' | 'g') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    None => Keyword::MERGE,
                                    Some('S' | 's') => Keyword::MERGES,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('X' | 'x'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::MESSAGE_TEXT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => match cs.next() {
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::MICROSECOND
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('D' | 'd') => match cs.next() {
                            None => Keyword::MID,
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::MIDDLEINT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('G' | 'g') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::MIGRATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('N' | 'n') => match cs.next() {
                            None => Keyword::MIN,
                            Some('U' | 'u') => match cs.next() {
                                Some('S' | 's') => Keyword::MINUS,
                                Some('T' | 't') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        None => Keyword::MINUTE,
                                        Some('_') => match cs.next() {
                                            Some('M' | 'm') => {
                                                if matches!(cs.next(), Some('I' | 'i'))
                                                    && matches!(cs.next(), Some('C' | 'c'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                    && matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('S' | 's'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('C' | 'c'))
                                                    && matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('N' | 'n'))
                                                    && matches!(cs.next(), Some('D' | 'd'))
                                                {
                                                    Keyword::MINUTE_MICROSECOND
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('S' | 's') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('C' | 'c'))
                                                    && matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('N' | 'n'))
                                                    && matches!(cs.next(), Some('D' | 'd'))
                                                {
                                                    Keyword::MINUTE_SECOND
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('V' | 'v') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::MINVALUE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('W' | 'w'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::MIN_ROWS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => match cs.next() {
                        Some('D' | 'd') => match cs.next() {
                            None => Keyword::MOD,
                            Some('E' | 'e') => Keyword::MODE,
                            Some('I' | 'i') => match cs.next() {
                                Some('F' | 'f') => match cs.next() {
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                        {
                                            Keyword::MODIFIES
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('Y' | 'y') => Keyword::MODIFY,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('N' | 'n') => match cs.next() {
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                {
                                    Keyword::MONITOR
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => match cs.next() {
                                Some('H' | 'h') => match cs.next() {
                                    None => Keyword::MONTH,
                                    Some('N' | 'n') => {
                                        if matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::MONTHNAME
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('X' | 'x'))
                        {
                            Keyword::MUTEX
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('Y' | 'y') => match cs.next() {
                        Some('S' | 's') => match cs.next() {
                            Some('Q' | 'q') => match cs.next() {
                                Some('L' | 'l') => match cs.next() {
                                    None => Keyword::MYSQL,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                        {
                                            Keyword::MYSQL_ERRNO
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                }
            }
            Some('N' | 'n') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('M' | 'm') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::NAME,
                            Some('S' | 's') => Keyword::NAMES,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                            {
                                Keyword::NATIONAL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('U' | 'u') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('L' | 'l') => match cs.next() {
                                        None => Keyword::NATURAL,
                                        Some('_') => {
                                            if matches!(cs.next(), Some('S' | 's'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('K' | 'k'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('Y' | 'y'))
                                            {
                                                Keyword::NATURAL_SORT_KEY
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('C' | 'c') => {
                    if matches!(cs.next(), Some('H' | 'h'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('R' | 'r'))
                    {
                        Keyword::NCHAR
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('E' | 'e') => match cs.next() {
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::NEGATOR
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::NESTED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('V' | 'v') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::NEVER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('W' | 'w') => Keyword::NEW,
                    Some('X' | 'x') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            None => Keyword::NEXT,
                            Some('V' | 'v') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                {
                                    Keyword::NEXTVAL
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    None => Keyword::NO,
                    Some('B' | 'b') => {
                        if matches!(cs.next(), Some('Y' | 'y'))
                            && matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::NOBYPASSRLS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('H' | 'h'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::NOCACHE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('R' | 'r') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        Some('E' | 'e') => match cs.next() {
                                            Some('D' | 'd') => {
                                                if matches!(cs.next(), Some('B' | 'b')) {
                                                    Keyword::NOCREATEDB
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('R' | 'r') => {
                                                if matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('L' | 'l'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                {
                                                    Keyword::NOCREATEROLE
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('Y' | 'y') => {
                            if matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::NOCYCLE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('D' | 'd') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('P' | 'p'))
                        {
                            Keyword::NODEGROUP
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::NOINHERIT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::NOLOGIN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('M' | 'm') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('X' | 'x'))
                                && matches!(cs.next(), Some('V' | 'v'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::NOMAXVALUE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('V' | 'v'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::NOMINVALUE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('E' | 'e')) {
                            Keyword::NONE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::NOREPLICATION
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::NOSUPERUSER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::NOT,
                        Some('F' | 'f') => {
                            if matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::NOTFOUND
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('H' | 'h') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                            {
                                Keyword::NOTHING
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::NOTICE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('W' | 'w') => match cs.next() {
                        None => Keyword::NOW,
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::NOWAIT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('_') => match cs.next() {
                        Some('W' | 'w') => match cs.next() {
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::NO_WAIT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('R' | 'r') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('B' | 'b'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('G' | 'g'))
                                {
                                    Keyword::NO_WRITE_TO_BINLOG
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('U' | 'u') => match cs.next() {
                    Some('L' | 'l') => match cs.next() {
                        Some('L' | 'l') => match cs.next() {
                            None => Keyword::NULL,
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('F' | 'f')) {
                                    Keyword::NULLIF
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => Keyword::NULLS,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('M' | 'm') => match cs.next() {
                        Some('B' | 'b') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::NUMBER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('C' | 'c'))
                            {
                                Keyword::NUMERIC
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('V' | 'v') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::NVARCHAR
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => match cs.next() {
                        None => Keyword::NVL,
                        Some('2') => Keyword::NVL2,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('O' | 'o') => match cs.next() {
                Some('C' | 'c') => match cs.next() {
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::OCT,
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('H' | 'h'))
                            {
                                Keyword::OCTET_LENGTH
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('F' | 'f') => match cs.next() {
                    None => Keyword::OF,
                    Some('F' | 'f') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::OFFSET
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('L' | 'l') => {
                    if matches!(cs.next(), Some('D' | 'd'))
                        && matches!(cs.next(), Some('_'))
                        && matches!(cs.next(), Some('P' | 'p'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('S' | 's'))
                        && matches!(cs.next(), Some('S' | 's'))
                        && matches!(cs.next(), Some('W' | 'w'))
                        && matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('D' | 'd'))
                    {
                        Keyword::OLD_PASSWORD
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('N' | 'n') => match cs.next() {
                    None => Keyword::ON,
                    Some('E' | 'e') => Keyword::ONE,
                    Some('L' | 'l') => match cs.next() {
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::ONLINE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('Y' | 'y') => Keyword::ONLY,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('N' | 'n') => Keyword::OPEN,
                        Some('R' | 'r') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::OPERATOR
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        Some('I' | 'i') => match cs.next() {
                            Some('M' | 'm') => match cs.next() {
                                Some('I' | 'i') => match cs.next() {
                                    Some('Z' | 'z') => match cs.next() {
                                        Some('E' | 'e') => match cs.next() {
                                            None => Keyword::OPTIMIZE,
                                            Some('R' | 'r') => match cs.next() {
                                                None => Keyword::OPTIMIZER,
                                                Some('_') => {
                                                    if matches!(cs.next(), Some('C' | 'c'))
                                                        && matches!(cs.next(), Some('O' | 'o'))
                                                        && matches!(cs.next(), Some('S' | 's'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('S' | 's'))
                                                    {
                                                        Keyword::OPTIMIZER_COSTS
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('O' | 'o') => match cs.next() {
                                Some('N' | 'n') => match cs.next() {
                                    None => Keyword::OPTION,
                                    Some('A' | 'a') => {
                                        if matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('Y' | 'y'))
                                        {
                                            Keyword::OPTIONALLY
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => Keyword::OPTIONS,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('R' | 'r') => match cs.next() {
                    None => Keyword::OR,
                    Some('D' | 'd') => match cs.next() {
                        None => Keyword::ORD,
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r')) {
                                Keyword::ORDER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                            {
                                Keyword::ORDINALITY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('T' | 't') => {
                    if matches!(cs.next(), Some('H' | 'h'))
                        && matches!(cs.next(), Some('E' | 'e'))
                        && matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('S' | 's'))
                    {
                        Keyword::OTHERS
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('U' | 'u') => match cs.next() {
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::OUT,
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r')) {
                                Keyword::OUTER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('F' | 'f') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::OUTFILE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('V' | 'v') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('R' | 'r') => match cs.next() {
                            None => Keyword::OVER,
                            Some('L' | 'l') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('P' | 'p'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::OVERLAPS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('W' | 'w') => match cs.next() {
                    Some('N' | 'n') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('D' | 'd') => Keyword::OWNED,
                            Some('R' | 'r') => Keyword::OWNER,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('P' | 'p') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('C' | 'c') => match cs.next() {
                        Some('K' | 'k') => match cs.next() {
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('G' | 'g'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::PACKAGE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => {
                                if matches!(cs.next(), Some('K' | 'k'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('Y' | 'y'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::PACK_KEYS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('G' | 'g') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::PAGE,
                            Some('_') => {
                                if matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('H' | 'h'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('K' | 'k'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('M' | 'm'))
                                {
                                    Keyword::PAGE_CHECKSUM
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('R' | 'r') => match cs.next() {
                        Some('S' | 's') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                Some('R' | 'r') => Keyword::PARSER,
                                Some('_') => {
                                    if matches!(cs.next(), Some('V' | 'v'))
                                        && matches!(cs.next(), Some('C' | 'c'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('X' | 'x'))
                                        && matches!(cs.next(), Some('P' | 'p'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                    {
                                        Keyword::PARSE_VCOL_EXPR
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('T' | 't') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('A' | 'a') => {
                                    if matches!(cs.next(), Some('L' | 'l')) {
                                        Keyword::PARTIAL
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('T' | 't') => match cs.next() {
                                    Some('I' | 'i') => match cs.next() {
                                        Some('O' | 'o') => match cs.next() {
                                            Some('N' | 'n') => match cs.next() {
                                                None => Keyword::PARTITION,
                                                Some('I' | 'i') => {
                                                    if matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('G' | 'g'))
                                                    {
                                                        Keyword::PARTITIONING
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('S' | 's') => Keyword::PARTITIONS,
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('W' | 'w'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::PASSWORD
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('H' | 'h')) {
                            Keyword::PATH
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('R' | 'r') => match cs.next() {
                        Some('I' | 'i') => match cs.next() {
                            Some('O' | 'o') => match cs.next() {
                                Some('D' | 'd') => match cs.next() {
                                    None => Keyword::PERIOD,
                                    Some('_') => match cs.next() {
                                        Some('A' | 'a') => {
                                            if matches!(cs.next(), Some('D' | 'd'))
                                                && matches!(cs.next(), Some('D' | 'd'))
                                            {
                                                Keyword::PERIOD_ADD
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('D' | 'd') => {
                                            if matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('F' | 'f'))
                                                && matches!(cs.next(), Some('F' | 'f'))
                                            {
                                                Keyword::PERIOD_DIFF
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::PERSISTENT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('H' | 'h') => {
                    if matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('S' | 's'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::PHASE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('I' | 'i') => Keyword::PI,
                Some('L' | 'l') => match cs.next() {
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('Q' | 'q'))
                            && matches!(cs.next(), Some('L' | 'l'))
                        {
                            Keyword::PLPGSQL
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('U' | 'u') => match cs.next() {
                        Some('G' | 'g') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('N' | 'n') => match cs.next() {
                                    None => Keyword::PLUGIN,
                                    Some('S' | 's') => Keyword::PLUGINS,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    Some('R' | 'r') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            None => Keyword::PORT,
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                {
                                    Keyword::PORTION
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::POSITION
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('W' | 'w') => match cs.next() {
                        None => Keyword::POW,
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r')) {
                                Keyword::POWER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('R' | 'r') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('C' | 'c') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                Some('D' | 'd') => match cs.next() {
                                    Some('E' | 'e') => {
                                        if matches!(cs.next(), Some('S' | 's')) {
                                            Keyword::PRECEDES
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('G' | 'g'))
                                        {
                                            Keyword::PRECEDING
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                {
                                    Keyword::PRECISION
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('P' | 'p') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::PREPARE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('V' | 'v'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::PRESERVE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('V' | 'v') => match cs.next() {
                            None => Keyword::PREV,
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::PREVIOUS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => match cs.next() {
                        Some('M' | 'm') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                            {
                                Keyword::PRIMARY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('V' | 'v') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::PRIVILEGES
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => match cs.next() {
                        Some('C' | 'c') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                Some('D' | 'd') => {
                                    if matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::PROCEDURE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => match cs.next() {
                                    Some('S' | 's') => match cs.next() {
                                        None => Keyword::PROCESS,
                                        Some('L' | 'l') => {
                                            if matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                            {
                                                Keyword::PROCESSLIST
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('F' | 'f') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('L' | 'l') => match cs.next() {
                                    Some('E' | 'e') => match cs.next() {
                                        None => Keyword::PROFILE,
                                        Some('S' | 's') => Keyword::PROFILES,
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('X' | 'x') => {
                            if matches!(cs.next(), Some('Y' | 'y')) {
                                Keyword::PROXY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('U' | 'u') => {
                    if matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('G' | 'g'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::PURGE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('Q' | 'q') => match cs.next() {
                Some('U' | 'u') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::QUARTER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('Y' | 'y'))
                        {
                            Keyword::QUERY
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('K' | 'k'))
                        {
                            Keyword::QUICK
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::QUOTE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('R' | 'r') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('D' | 'd') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::RADIANS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::RAISE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => match cs.next() {
                        Some('D' | 'd') => Keyword::RAND,
                        Some('G' | 'g') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::RANGE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('W' | 'w') => Keyword::RAW,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('D' | 'd') => match cs.next() {
                            None => Keyword::READ,
                            Some('S' | 's') => Keyword::READS,
                            Some('_') => match cs.next() {
                                Some('O' | 'o') => {
                                    if matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('Y' | 'y'))
                                    {
                                        Keyword::READ_ONLY
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('W' | 'w') => {
                                    if matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::READ_WRITE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('L' | 'l') => Keyword::REAL,
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('B' | 'b') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::REBUILD
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('V' | 'v'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::RECOVER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('U' | 'u') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('V' | 'v'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::RECURSIVE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('D' | 'd') => match cs.next() {
                        Some('O' | 'o') => match cs.next() {
                            Some('F' | 'f') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::REDOFILE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => {
                                if matches!(cs.next(), Some('B' | 'b'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('Z' | 'z'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::REDO_BUFFER_SIZE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('U' | 'u') => {
                            if matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('D' | 'd'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::REDUNDANT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('F' | 'f') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::REFERENCES
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('_') => {
                            if matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::REF_SYSTEM_ID
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('X' | 'x'))
                            && matches!(cs.next(), Some('P' | 'p'))
                        {
                            Keyword::REGEXP
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('Y' | 'y') => match cs.next() {
                                None => Keyword::RELAY,
                                Some('L' | 'l') => {
                                    if matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('G' | 'g'))
                                    {
                                        Keyword::RELAYLOG
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('_') => match cs.next() {
                                    Some('L' | 'l') => match cs.next() {
                                        Some('O' | 'o') => match cs.next() {
                                            Some('G' | 'g') => match cs.next() {
                                                Some('_') => match cs.next() {
                                                    Some('F' | 'f') => {
                                                        if matches!(cs.next(), Some('I' | 'i'))
                                                            && matches!(cs.next(), Some('L' | 'l'))
                                                            && matches!(cs.next(), Some('E' | 'e'))
                                                        {
                                                            Keyword::RELAY_LOG_FILE
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    Some('P' | 'p') => {
                                                        if matches!(cs.next(), Some('O' | 'o'))
                                                            && matches!(cs.next(), Some('S' | 's'))
                                                        {
                                                            Keyword::RELAY_LOG_POS
                                                        } else {
                                                            Keyword::NOT_A_KEYWORD
                                                        }
                                                    }
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    Some('T' | 't') => {
                                        if matches!(cs.next(), Some('H' | 'h'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::RELAY_THREAD
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::RELEASE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::RELOAD
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('V' | 'v'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::REMOVE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::RENAME
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('Z' | 'z'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::REORGANIZE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('P' | 'p') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::REPAIR
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('E' | 'e') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    None => Keyword::REPEAT,
                                    Some('A' | 'a') => {
                                        if matches!(cs.next(), Some('B' | 'b'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::REPEATABLE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('L' | 'l') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('C' | 'c') => {
                                    if matches!(cs.next(), Some('E' | 'e')) {
                                        Keyword::REPLACE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('Y' | 'y') => Keyword::REPLAY,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('I' | 'i') => match cs.next() {
                                Some('C' | 'c') => match cs.next() {
                                    Some('A' | 'a') => match cs.next() {
                                        None => Keyword::REPLICA,
                                        Some('S' | 's') => Keyword::REPLICAS,
                                        Some('T' | 't') => {
                                            if matches!(cs.next(), Some('I' | 'i'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                            {
                                                Keyword::REPLICATION
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('_') => {
                                            if matches!(cs.next(), Some('P' | 'p'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                            {
                                                Keyword::REPLICA_POS
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('Q' | 'q') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::REQUIRE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('T' | 't')) {
                                Keyword::RESET
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                            {
                                Keyword::RESIGNAL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('T' | 't') => match cs.next() {
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::RESTART
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('O' | 'o') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::RESTORE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('R' | 'r') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::RESTRICT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('U' | 'u') => {
                            if matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::RESUME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        Some('U' | 'u') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                Some('N' | 'n') => match cs.next() {
                                    None => Keyword::RETURN,
                                    Some('E' | 'e') => {
                                        if matches!(cs.next(), Some('D' | 'd'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('Q' | 'q'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::RETURNED_SQLSTATE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('G' | 'g'))
                                        {
                                            Keyword::RETURNING
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => Keyword::RETURNS,
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::REUSE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('V' | 'v') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::REVERSE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('K' | 'k'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::REVOKE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('G' | 'g') => match cs.next() {
                        Some('H' | 'h') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                None => Keyword::RIGHT,
                                Some('A' | 'a') => {
                                    if matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('G' | 'g'))
                                    {
                                        Keyword::RIGHTARG
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('L' | 'l') => {
                    if matches!(cs.next(), Some('I' | 'i'))
                        && matches!(cs.next(), Some('K' | 'k'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::RLIKE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('O' | 'o') => match cs.next() {
                    Some('L' | 'l') => match cs.next() {
                        Some('E' | 'e') => Keyword::ROLE,
                        Some('L' | 'l') => match cs.next() {
                            Some('B' | 'b') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('K' | 'k'))
                                {
                                    Keyword::ROLLBACK
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('U' | 'u') => {
                                if matches!(cs.next(), Some('P' | 'p')) {
                                    Keyword::ROLLUP
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => match cs.next() {
                        Some('N' | 'n') => {
                            if matches!(cs.next(), Some('D' | 'd')) {
                                Keyword::ROUND
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('T' | 't') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::ROUTINE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('W' | 'w') => match cs.next() {
                        None => Keyword::ROW,
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::ROWCOUNT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('N' | 'n') => {
                            if matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('M' | 'm'))
                            {
                                Keyword::ROWNUM
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => Keyword::ROWS,
                        Some('T' | 't') => {
                            if matches!(cs.next(), Some('Y' | 'y'))
                                && matches!(cs.next(), Some('P' | 'p'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::ROWTYPE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('_') => match cs.next() {
                            Some('C' | 'c') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::ROW_COUNT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('F' | 'f') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('M' | 'm'))
                                    && matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::ROW_FORMAT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => {
                    if matches!(cs.next(), Some('A' | 'a')) && matches!(cs.next(), Some('D' | 'd'))
                    {
                        Keyword::RPAD
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('T' | 't') => match cs.next() {
                    Some('R' | 'r') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::RTREE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('M' | 'm')) {
                                Keyword::RTRIM
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('S' | 's') => match cs.next() {
                Some('A' | 'a') => {
                    if matches!(cs.next(), Some('V' | 'v'))
                        && matches!(cs.next(), Some('E' | 'e'))
                        && matches!(cs.next(), Some('P' | 'p'))
                        && matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('I' | 'i'))
                        && matches!(cs.next(), Some('N' | 'n'))
                        && matches!(cs.next(), Some('T' | 't'))
                    {
                        Keyword::SAVEPOINT
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('C' | 'c') => match cs.next() {
                    Some('H' | 'h') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::SCHEDULE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('M' | 'm') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    None => Keyword::SCHEMA,
                                    Some('S' | 's') => Keyword::SCHEMAS,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::SCHEMA_NAME
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('H' | 'h'))
                        {
                            Keyword::SEARCH
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        Some('O' | 'o') => match cs.next() {
                            Some('N' | 'n') => match cs.next() {
                                Some('D' | 'd') => match cs.next() {
                                    None => Keyword::SECOND,
                                    Some('A' | 'a') => match cs.next() {
                                        Some('R' | 'r') => match cs.next() {
                                            Some('Y' | 'y') => match cs.next() {
                                                None => Keyword::SECONDARY,
                                                Some('_') => match cs.next() {
                                                    Some('E' | 'e') => match cs.next() {
                                                        Some('N' | 'n') => match cs.next() {
                                                            Some('G' | 'g') => match cs.next() {
                                                                Some('I' | 'i') => match cs.next() {
                                                                    Some('N' | 'n') => {
                                                                        match cs.next() {
                                                                                                                                    Some('E' | 'e') => {
                                                                                                                                        match cs.next() {
                                                                                                                                            None => Keyword::SECONDARY_ENGINE,
                                                                                                                                            Some('_') => {
                                                                                                                                                if matches!(cs.next(), Some('A' | 'a'))
                                                                                                                                                  && matches!(cs.next(), Some('T' | 't'))
                                                                                                                                                  && matches!(cs.next(), Some('T' | 't'))
                                                                                                                                                  && matches!(cs.next(), Some('R' | 'r'))
                                                                                                                                                  && matches!(cs.next(), Some('I' | 'i'))
                                                                                                                                                  && matches!(cs.next(), Some('B' | 'b'))
                                                                                                                                                  && matches!(cs.next(), Some('U' | 'u'))
                                                                                                                                                  && matches!(cs.next(), Some('T' | 't'))
                                                                                                                                                  && matches!(cs.next(), Some('E' | 'e')) {
                                                                                                                                                    Keyword::SECONDARY_ENGINE_ATTRIBUTE
                                                                                                                                                } else {
                                                                                                                                                    Keyword::NOT_A_KEYWORD
                                                                                                                                                }
                                                                                                                                            }
                                                                                                                                            _ => Keyword::NOT_A_KEYWORD,
                                                                                                                                        }
                                                                                                                                    }
                                                                                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                                                                                }
                                                                    }
                                                                    _ => Keyword::NOT_A_KEYWORD,
                                                                },
                                                                _ => Keyword::NOT_A_KEYWORD,
                                                            },
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    Some('_') => {
                                        if matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                        {
                                            Keyword::SECOND_MICROSECOND
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('U' | 'u') => {
                            if matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                            {
                                Keyword::SECURITY
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('_') => {
                            if matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::SEC_TO_TIME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('L' | 'l') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::SELECT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('V' | 'v'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::SENSITIVE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::SEPARATOR
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('Q' | 'q') => {
                        if matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::SEQUENCE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => match cs.next() {
                        Some('I' | 'i') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('L' | 'l') => match cs.next() {
                                    None => Keyword::SERIAL,
                                    Some('I' | 'i') => {
                                        if matches!(cs.next(), Some('Z' | 'z'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('B' | 'b'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::SERIALIZABLE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('V' | 'v') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::SERVER
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => match cs.next() {
                        Some('S' | 's') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('O' | 'o') => match cs.next() {
                                    Some('N' | 'n') => match cs.next() {
                                        None => Keyword::SESSION,
                                        Some('_') => {
                                            if matches!(cs.next(), Some('U' | 'u'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                            {
                                                Keyword::SESSION_USER
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('T' | 't') => match cs.next() {
                        None => Keyword::SET,
                        Some('V' | 'v') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                            {
                                Keyword::SETVAL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('F' | 'f') => {
                    if matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('M' | 'm'))
                        && matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('T' | 't'))
                    {
                        Keyword::SFORMAT
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('H' | 'h') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('R' | 'r') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                None => Keyword::SHARE,
                                Some('D' | 'd') => Keyword::SHARED,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('W' | 'w')) {
                            Keyword::SHOW
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('U' | 'u') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('W' | 'w'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::SHUTDOWN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('G' | 'g') => match cs.next() {
                        Some('N' | 'n') => match cs.next() {
                            None => Keyword::SIGN,
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('L' | 'l')) {
                                    Keyword::SIGNAL
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('D' | 'd')) {
                                    Keyword::SIGNED
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::SIMPLE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => Keyword::SIN,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('K' | 'k') => {
                    if matches!(cs.next(), Some('I' | 'i')) && matches!(cs.next(), Some('P' | 'p'))
                    {
                        Keyword::SKIP
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('L' | 'l') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('V' | 'v') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                None => Keyword::SLAVE,
                                Some('S' | 's') => Keyword::SLAVES,
                                Some('_') => {
                                    if matches!(cs.next(), Some('P' | 'p'))
                                        && matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                    {
                                        Keyword::SLAVE_POS
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('P' | 'p'))
                        {
                            Keyword::SLEEP
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => {
                        if matches!(cs.next(), Some('W' | 'w')) {
                            Keyword::SLOW
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('M' | 'm') => {
                    if matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('L' | 'l'))
                        && matches!(cs.next(), Some('L' | 'l'))
                        && matches!(cs.next(), Some('I' | 'i'))
                        && matches!(cs.next(), Some('N' | 'n'))
                        && matches!(cs.next(), Some('T' | 't'))
                    {
                        Keyword::SMALLINT
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('N' | 'n') => {
                    if matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('P' | 'p'))
                        && matches!(cs.next(), Some('S' | 's'))
                        && matches!(cs.next(), Some('H' | 'h'))
                        && matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('T' | 't'))
                    {
                        Keyword::SNAPSHOT
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('O' | 'o') => match cs.next() {
                    Some('C' | 'c') => {
                        if matches!(cs.next(), Some('K' | 'k'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('T' | 't'))
                        {
                            Keyword::SOCKET
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('F' | 'f') => {
                        if matches!(cs.next(), Some('T' | 't')) {
                            Keyword::SOFT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('M' | 'm') => {
                        if matches!(cs.next(), Some('E' | 'e')) {
                            Keyword::SOME
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::SONAME
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('U' | 'u') => match cs.next() {
                        Some('N' | 'n') => match cs.next() {
                            Some('D' | 'd') => match cs.next() {
                                Some('E' | 'e') => {
                                    if matches!(cs.next(), Some('X' | 'x')) {
                                        Keyword::SOUNDEX
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => Keyword::SOUNDS,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('R' | 'r') => {
                            if matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::SOURCE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::SPACE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('T' | 't') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                            {
                                Keyword::SPATIAL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('F' | 'f'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('C' | 'c'))
                        {
                            Keyword::SPECIFIC
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('Q' | 'q') => match cs.next() {
                    Some('L' | 'l') => match cs.next() {
                        None => Keyword::SQL,
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('X' | 'x'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('P' | 'p'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::SQLEXCEPTION
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::SQLSTATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('W' | 'w') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                            {
                                Keyword::SQLWARNING
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('_') => match cs.next() {
                            Some('B' | 'b') => match cs.next() {
                                Some('I' | 'i') => {
                                    if matches!(cs.next(), Some('G' | 'g'))
                                        && matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::SQL_BIG_RESULT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('U' | 'u') => {
                                    if matches!(cs.next(), Some('F' | 'f'))
                                        && matches!(cs.next(), Some('F' | 'f'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('U' | 'u'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::SQL_BUFFER_RESULT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('C' | 'c') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('C' | 'c') => {
                                        if matches!(cs.next(), Some('H' | 'h'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::SQL_CACHE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('L' | 'l') => {
                                        if matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('F' | 'f'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('D' | 'd'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('W' | 'w'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                        {
                                            Keyword::SQL_CALC_FOUND_ROWS
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('N' | 'n') => {
                                if matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('H' | 'h'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::SQL_NO_CACHE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => {
                                if matches!(cs.next(), Some('M' | 'm'))
                                    && matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::SQL_SMALL_RESULT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => match cs.next() {
                                Some('H' | 'h') => {
                                    if matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                    {
                                        Keyword::SQL_THREAD
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => match cs.next() {
                                    Some('I' | 'i') => match cs.next() {
                                        Some('_') => match cs.next() {
                                            Some('D' | 'd') => {
                                                if matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('Y' | 'y'))
                                                {
                                                    Keyword::SQL_TSI_DAY
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('H' | 'h') => {
                                                if matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('U' | 'u'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                {
                                                    Keyword::SQL_TSI_HOUR
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('M' | 'm') => match cs.next() {
                                                Some('I' | 'i') => {
                                                    if matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('U' | 'u'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                    {
                                                        Keyword::SQL_TSI_MINUTE
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('O' | 'o') => {
                                                    if matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('T' | 't'))
                                                        && matches!(cs.next(), Some('H' | 'h'))
                                                    {
                                                        Keyword::SQL_TSI_MONTH
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            Some('Q' | 'q') => {
                                                if matches!(cs.next(), Some('U' | 'u'))
                                                    && matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                    && matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                {
                                                    Keyword::SQL_TSI_QUARTER
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('S' | 's') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('C' | 'c'))
                                                    && matches!(cs.next(), Some('O' | 'o'))
                                                    && matches!(cs.next(), Some('N' | 'n'))
                                                    && matches!(cs.next(), Some('D' | 'd'))
                                                {
                                                    Keyword::SQL_TSI_SECOND
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('W' | 'w') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('K' | 'k'))
                                                {
                                                    Keyword::SQL_TSI_WEEK
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            Some('Y' | 'y') => {
                                                if matches!(cs.next(), Some('E' | 'e'))
                                                    && matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('R' | 'r'))
                                                {
                                                    Keyword::SQL_TSI_YEAR
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('T' | 't')) {
                            Keyword::SQRT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('S' | 's') => {
                    if matches!(cs.next(), Some('L' | 'l')) {
                        Keyword::SSL
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('T' | 't') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('G' | 'g') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::STAGE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('R' | 'r') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                None => Keyword::START,
                                Some('I' | 'i') => {
                                    if matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('G' | 'g'))
                                    {
                                        Keyword::STARTING
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('S' | 's') => match cs.next() {
                                    None => Keyword::STARTS,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('W' | 'w'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('H' | 'h'))
                                        {
                                            Keyword::STARTS_WITH
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('T' | 't') => match cs.next() {
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('M' | 'm'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::STATEMENT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => match cs.next() {
                                Some('_') => match cs.next() {
                                    Some('A' | 'a') => {
                                        if matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('O' | 'o'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                        {
                                            Keyword::STATS_AUTO_RECALC
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('P' | 'p') => {
                                        if matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('R' | 'r'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('N' | 'n'))
                                            && matches!(cs.next(), Some('T' | 't'))
                                        {
                                            Keyword::STATS_PERSISTENT
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('S' | 's') => {
                                        if matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('P' | 'p'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('_'))
                                            && matches!(cs.next(), Some('P' | 'p'))
                                            && matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('G' | 'g'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                        {
                                            Keyword::STATS_SAMPLE_PAGES
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('U' | 'u') => {
                                if matches!(cs.next(), Some('S' | 's')) {
                                    Keyword::STATUS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('D' | 'd') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::STDIN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('O' | 'o') => match cs.next() {
                        Some('P' | 'p') => Keyword::STOP,
                        Some('R' | 'r') => match cs.next() {
                            Some('A' | 'a') => {
                                if matches!(cs.next(), Some('G' | 'g'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::STORAGE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('E' | 'e') => {
                                if matches!(cs.next(), Some('D' | 'd')) {
                                    Keyword::STORED
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('R' | 'r') => match cs.next() {
                        Some('A' | 'a') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('H' | 'h'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('J' | 'j'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::STRAIGHT_JOIN
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('P' | 'p'))
                            {
                                Keyword::STRCMP
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('F' | 'f') => {
                            if matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::STRFTIME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => match cs.next() {
                            Some('C' | 'c') => {
                                if matches!(cs.next(), Some('T' | 't')) {
                                    Keyword::STRICT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('N' | 'n') => {
                                if matches!(cs.next(), Some('G' | 'g')) {
                                    Keyword::STRING
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('_') => {
                            if matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('D' | 'd'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::STR_TO_DATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('U' | 'u') => match cs.next() {
                    Some('B' | 'b') => match cs.next() {
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('G' | 'g'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                            {
                                Keyword::SUBCLASS_ORIGIN
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('D' | 'd') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::SUBDATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('J' | 'j') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('T' | 't'))
                            {
                                Keyword::SUBJECT
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('P' | 'p') => match cs.next() {
                            Some('A' | 'a') => match cs.next() {
                                Some('R' | 'r') => match cs.next() {
                                    Some('T' | 't') => match cs.next() {
                                        Some('I' | 'i') => match cs.next() {
                                            Some('T' | 't') => match cs.next() {
                                                Some('I' | 'i') => match cs.next() {
                                                    Some('O' | 'o') => match cs.next() {
                                                        Some('N' | 'n') => match cs.next() {
                                                            None => Keyword::SUBPARTITION,
                                                            Some('S' | 's') => {
                                                                Keyword::SUBPARTITIONS
                                                            }
                                                            _ => Keyword::NOT_A_KEYWORD,
                                                        },
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('S' | 's') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                Some('R' | 'r') => match cs.next() {
                                    None => Keyword::SUBSTR,
                                    Some('I' | 'i') => match cs.next() {
                                        Some('N' | 'n') => match cs.next() {
                                            Some('G' | 'g') => match cs.next() {
                                                None => Keyword::SUBSTRING,
                                                Some('_') => {
                                                    if matches!(cs.next(), Some('I' | 'i'))
                                                        && matches!(cs.next(), Some('N' | 'n'))
                                                        && matches!(cs.next(), Some('D' | 'd'))
                                                        && matches!(cs.next(), Some('E' | 'e'))
                                                        && matches!(cs.next(), Some('X' | 'x'))
                                                    {
                                                        Keyword::SUBSTRING_INDEX
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('T' | 't') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::SUBTIME
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('M' | 'm') => Keyword::SUM,
                    Some('P' | 'p') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            Some('R' | 'r') => match cs.next() {
                                None => Keyword::SUPER,
                                Some('U' | 'u') => {
                                    if matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                    {
                                        Keyword::SUPERUSER
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::SUSPEND
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('W' | 'w') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::SWAPS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::SWITCHES
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('Y' | 'y') => match cs.next() {
                    Some('S' | 's') => match cs.next() {
                        Some('D' | 'd') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::SYSDATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('D' | 'd')) {
                                Keyword::SYSID
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('T' | 't') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                Some('M' | 'm') => match cs.next() {
                                    None => Keyword::SYSTEM,
                                    Some('_') => {
                                        if matches!(cs.next(), Some('T' | 't'))
                                            && matches!(cs.next(), Some('I' | 'i'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::SYSTEM_TIME
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('T' | 't') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('B' | 'b') => match cs.next() {
                        Some('L' | 'l') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                None => Keyword::TABLE,
                                Some('S' | 's') => match cs.next() {
                                    None => Keyword::TABLES,
                                    Some('P' | 'p') => {
                                        if matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::TABLESPACE
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                Some('_') => match cs.next() {
                                    Some('C' | 'c') => {
                                        if matches!(cs.next(), Some('H' | 'h'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                            && matches!(cs.next(), Some('C' | 'c'))
                                            && matches!(cs.next(), Some('K' | 'k'))
                                            && matches!(cs.next(), Some('S' | 's'))
                                            && matches!(cs.next(), Some('U' | 'u'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                        {
                                            Keyword::TABLE_CHECKSUM
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    Some('N' | 'n') => {
                                        if matches!(cs.next(), Some('A' | 'a'))
                                            && matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('E' | 'e'))
                                        {
                                            Keyword::TABLE_NAME
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('N' | 'n') => Keyword::TAN,
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('M' | 'm') => match cs.next() {
                        Some('P' | 'p') => match cs.next() {
                            Some('O' | 'o') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('Y' | 'y'))
                                {
                                    Keyword::TEMPORARY
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('B' | 'b'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::TEMPTABLE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('M' | 'm'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::TERMINATED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('X' | 'x') => match cs.next() {
                        Some('T' | 't') => match cs.next() {
                            None => Keyword::TEXT,
                            Some('_') => {
                                if matches!(cs.next(), Some('P' | 'p'))
                                    && matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('P' | 'p'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::TEXT_PATTERN_OPS
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('H' | 'h') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('N' | 'n')) {
                            Keyword::THAN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('N' | 'n')) {
                            Keyword::THEN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::THREADS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('S' | 's')) {
                            Keyword::TIES
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('M' | 'm') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::TIME,
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                {
                                    Keyword::TIMEDIFF
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('S' | 's') => match cs.next() {
                                Some('T' | 't') => match cs.next() {
                                    Some('A' | 'a') => match cs.next() {
                                        Some('M' | 'm') => match cs.next() {
                                            Some('P' | 'p') => match cs.next() {
                                                None => Keyword::TIMESTAMP,
                                                Some('A' | 'a') => {
                                                    if matches!(cs.next(), Some('D' | 'd'))
                                                        && matches!(cs.next(), Some('D' | 'd'))
                                                    {
                                                        Keyword::TIMESTAMPADD
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('D' | 'd') => {
                                                    if matches!(cs.next(), Some('I' | 'i'))
                                                        && matches!(cs.next(), Some('F' | 'f'))
                                                        && matches!(cs.next(), Some('F' | 'f'))
                                                    {
                                                        Keyword::TIMESTAMPDIFF
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                Some('T' | 't') => {
                                                    if matches!(cs.next(), Some('Z' | 'z')) {
                                                        Keyword::TIMESTAMPTZ
                                                    } else {
                                                        Keyword::NOT_A_KEYWORD
                                                    }
                                                }
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            Some('_') => match cs.next() {
                                Some('F' | 'f') => {
                                    if matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('M' | 'm'))
                                        && matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                    {
                                        Keyword::TIME_FORMAT
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('T' | 't') => {
                                    if matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('C' | 'c'))
                                    {
                                        Keyword::TIME_TO_SEC
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('Z' | 'z') => {
                                    if matches!(cs.next(), Some('O' | 'o'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                    {
                                        Keyword::TIME_ZONE
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('N' | 'n') => match cs.next() {
                        Some('Y' | 'y') => match cs.next() {
                            Some('B' | 'b') => {
                                if matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('B' | 'b'))
                                {
                                    Keyword::TINYBLOB
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::TINYINT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('X' | 'x'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::TINYTEXT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => match cs.next() {
                    None => Keyword::TO,
                    Some('_') => match cs.next() {
                        Some('B' | 'b') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('6'))
                                && matches!(cs.next(), Some('4'))
                            {
                                Keyword::TO_BASE64
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('H' | 'h'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('R' | 'r'))
                            {
                                Keyword::TO_CHAR
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('D' | 'd') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('Y' | 'y'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::TO_DAYS
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('S' | 's') => {
                            if matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('D' | 'd'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::TO_SECONDS
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('R' | 'r') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                            {
                                Keyword::TRAILING
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('N' | 'n') => match cs.next() {
                            Some('S' | 's') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('C' | 'c') => match cs.next() {
                                        Some('T' | 't') => match cs.next() {
                                            Some('I' | 'i') => match cs.next() {
                                                Some('O' | 'o') => match cs.next() {
                                                    Some('N' | 'n') => match cs.next() {
                                                        None => Keyword::TRANSACTION,
                                                        Some('A' | 'a') => {
                                                            if matches!(cs.next(), Some('L' | 'l'))
                                                            {
                                                                Keyword::TRANSACTIONAL
                                                            } else {
                                                                Keyword::NOT_A_KEYWORD
                                                            }
                                                        }
                                                        _ => Keyword::NOT_A_KEYWORD,
                                                    },
                                                    _ => Keyword::NOT_A_KEYWORD,
                                                },
                                                _ => Keyword::NOT_A_KEYWORD,
                                            },
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => match cs.next() {
                        Some('G' | 'g') => match cs.next() {
                            Some('G' | 'g') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    Some('R' | 'r') => match cs.next() {
                                        None => Keyword::TRIGGER,
                                        Some('S' | 's') => Keyword::TRIGGERS,
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('U' | 'u') => match cs.next() {
                        Some('E' | 'e') => Keyword::TRUE,
                        Some('N' | 'n') => {
                            if matches!(cs.next(), Some('C' | 'c'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::TRUNCATE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('Y' | 'y') => match cs.next() {
                    Some('P' | 'p') => match cs.next() {
                        Some('E' | 'e') => match cs.next() {
                            None => Keyword::TYPE,
                            Some('S' | 's') => Keyword::TYPES,
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('U' | 'u') => match cs.next() {
                Some('C' | 'c') => {
                    if matches!(cs.next(), Some('A' | 'a'))
                        && matches!(cs.next(), Some('S' | 's'))
                        && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::UCASE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('N' | 'n') => match cs.next() {
                    Some('B' | 'b') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::UNBOUNDED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('C' | 'c') => match cs.next() {
                        Some('O' | 'o') => match cs.next() {
                            Some('M' | 'm') => match cs.next() {
                                Some('M' | 'm') => {
                                    if matches!(cs.next(), Some('I' | 'i'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                    {
                                        Keyword::UNCOMMITTED
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('P' | 'p') => {
                                    if matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('S' | 's'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('D' | 'd'))
                                        && matches!(cs.next(), Some('_'))
                                        && matches!(cs.next(), Some('L' | 'l'))
                                        && matches!(cs.next(), Some('E' | 'e'))
                                        && matches!(cs.next(), Some('N' | 'n'))
                                        && matches!(cs.next(), Some('G' | 'g'))
                                        && matches!(cs.next(), Some('T' | 't'))
                                        && matches!(cs.next(), Some('H' | 'h'))
                                    {
                                        Keyword::UNCOMPRESSED_LENGTH
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('D' | 'd') => match cs.next() {
                        Some('E' | 'e') => {
                            if matches!(cs.next(), Some('F' | 'f'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('D' | 'd'))
                            {
                                Keyword::UNDEFINED
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => match cs.next() {
                            None => Keyword::UNDO,
                            Some('F' | 'f') => {
                                if matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('L' | 'l'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::UNDOFILE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => {
                                if matches!(cs.next(), Some('B' | 'b'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('_'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('I' | 'i'))
                                    && matches!(cs.next(), Some('Z' | 'z'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::UNDO_BUFFER_SIZE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('H' | 'h') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('X' | 'x'))
                        {
                            Keyword::UNHEX
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => match cs.next() {
                        Some('C' | 'c') => {
                            if matches!(cs.next(), Some('O' | 'o'))
                                && matches!(cs.next(), Some('D' | 'd'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::UNICODE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('N' | 'n') => {
                            if matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('L' | 'l'))
                            {
                                Keyword::UNINSTALL
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('O' | 'o') => {
                            if matches!(cs.next(), Some('N' | 'n')) {
                                Keyword::UNION
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('Q' | 'q') => {
                            if matches!(cs.next(), Some('U' | 'u'))
                                && matches!(cs.next(), Some('E' | 'e'))
                            {
                                Keyword::UNIQUE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('X' | 'x') => {
                            if matches!(cs.next(), Some('_'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('S' | 's'))
                                && matches!(cs.next(), Some('T' | 't'))
                                && matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('M' | 'm'))
                                && matches!(cs.next(), Some('P' | 'p'))
                            {
                                Keyword::UNIX_TIMESTAMP
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('K' | 'k') => {
                        if matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('W' | 'w'))
                            && matches!(cs.next(), Some('N' | 'n'))
                        {
                            Keyword::UNKNOWN
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('L' | 'l') => {
                        if matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('C' | 'c'))
                            && matches!(cs.next(), Some('K' | 'k'))
                        {
                            Keyword::UNLOCK
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('D' | 'd'))
                        {
                            Keyword::UNSIGNED
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('L' | 'l'))
                        {
                            Keyword::UNTIL
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('P' | 'p') => match cs.next() {
                    Some('D' | 'd') => match cs.next() {
                        Some('A' | 'a') => match cs.next() {
                            Some('T' | 't') => match cs.next() {
                                Some('E' | 'e') => match cs.next() {
                                    None => Keyword::UPDATE,
                                    Some('X' | 'x') => {
                                        if matches!(cs.next(), Some('M' | 'm'))
                                            && matches!(cs.next(), Some('L' | 'l'))
                                        {
                                            Keyword::UPDATEXML
                                        } else {
                                            Keyword::NOT_A_KEYWORD
                                        }
                                    }
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('G' | 'g') => {
                        if matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::UPGRADE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('P' | 'p') => {
                        if matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::UPPER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('S' | 's') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::USAGE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('E' | 'e') => match cs.next() {
                        None => Keyword::USE,
                        Some('R' | 'r') => match cs.next() {
                            None => Keyword::USER,
                            Some('_') => {
                                if matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                    && matches!(cs.next(), Some('C' | 'c'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('S' | 's'))
                                {
                                    Keyword::USER_RESOURCES
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('_') => {
                            if matches!(cs.next(), Some('F' | 'f'))
                                && matches!(cs.next(), Some('R' | 'r'))
                                && matches!(cs.next(), Some('M' | 'm'))
                            {
                                Keyword::USE_FRM
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('G' | 'g'))
                        {
                            Keyword::USING
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('T' | 't') => match cs.next() {
                    Some('C' | 'c') => match cs.next() {
                        Some('_') => match cs.next() {
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                {
                                    Keyword::UTC_DATE
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('T' | 't') => match cs.next() {
                                Some('I' | 'i') => match cs.next() {
                                    Some('M' | 'm') => match cs.next() {
                                        Some('E' | 'e') => match cs.next() {
                                            None => Keyword::UTC_TIME,
                                            Some('S' | 's') => {
                                                if matches!(cs.next(), Some('T' | 't'))
                                                    && matches!(cs.next(), Some('A' | 'a'))
                                                    && matches!(cs.next(), Some('M' | 'm'))
                                                    && matches!(cs.next(), Some('P' | 'p'))
                                                {
                                                    Keyword::UTC_TIMESTAMP
                                                } else {
                                                    Keyword::NOT_A_KEYWORD
                                                }
                                            }
                                            _ => Keyword::NOT_A_KEYWORD,
                                        },
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('V' | 'v') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('L' | 'l') => match cs.next() {
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('D' | 'd')) {
                                Keyword::VALID
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('U' | 'u') => match cs.next() {
                            Some('E' | 'e') => match cs.next() {
                                None => Keyword::VALUE,
                                Some('S' | 's') => Keyword::VALUES,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('R' | 'r') => match cs.next() {
                        Some('B' | 'b') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('N' | 'n') => {
                                    if matches!(cs.next(), Some('A' | 'a'))
                                        && matches!(cs.next(), Some('R' | 'r'))
                                        && matches!(cs.next(), Some('Y' | 'y'))
                                    {
                                        Keyword::VARBINARY
                                    } else {
                                        Keyword::NOT_A_KEYWORD
                                    }
                                }
                                Some('T' | 't') => Keyword::VARBIT,
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('C' | 'c') => match cs.next() {
                            Some('H' | 'h') => match cs.next() {
                                Some('A' | 'a') => match cs.next() {
                                    Some('R' | 'r') => match cs.next() {
                                        None => Keyword::VARCHAR,
                                        Some('2') => Keyword::VARCHAR2,
                                        Some('A' | 'a') => {
                                            if matches!(cs.next(), Some('C' | 'c'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                            {
                                                Keyword::VARCHARACTER
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        Some('_') => {
                                            if matches!(cs.next(), Some('P' | 'p'))
                                                && matches!(cs.next(), Some('A' | 'a'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('T' | 't'))
                                                && matches!(cs.next(), Some('E' | 'e'))
                                                && matches!(cs.next(), Some('R' | 'r'))
                                                && matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('_'))
                                                && matches!(cs.next(), Some('O' | 'o'))
                                                && matches!(cs.next(), Some('P' | 'p'))
                                                && matches!(cs.next(), Some('S' | 's'))
                                            {
                                                Keyword::VARCHAR_PATTERN_OPS
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        Some('I' | 'i') => {
                            if matches!(cs.next(), Some('A' | 'a'))
                                && matches!(cs.next(), Some('B' | 'b'))
                                && matches!(cs.next(), Some('L' | 'l'))
                                && matches!(cs.next(), Some('E' | 'e'))
                                && matches!(cs.next(), Some('S' | 's'))
                            {
                                Keyword::VARIABLES
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        Some('Y' | 'y') => {
                            if matches!(cs.next(), Some('I' | 'i'))
                                && matches!(cs.next(), Some('N' | 'n'))
                                && matches!(cs.next(), Some('G' | 'g'))
                            {
                                Keyword::VARYING
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('R' | 'r') => match cs.next() {
                        Some('S' | 's') => match cs.next() {
                            Some('I' | 'i') => match cs.next() {
                                Some('O' | 'o') => match cs.next() {
                                    Some('N' | 'n') => match cs.next() {
                                        None => Keyword::VERSION,
                                        Some('I' | 'i') => {
                                            if matches!(cs.next(), Some('N' | 'n'))
                                                && matches!(cs.next(), Some('G' | 'g'))
                                            {
                                                Keyword::VERSIONING
                                            } else {
                                                Keyword::NOT_A_KEYWORD
                                            }
                                        }
                                        _ => Keyword::NOT_A_KEYWORD,
                                    },
                                    _ => Keyword::NOT_A_KEYWORD,
                                },
                                _ => Keyword::NOT_A_KEYWORD,
                            },
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('A' | 'a') => Keyword::VIA,
                    Some('E' | 'e') => {
                        if matches!(cs.next(), Some('W' | 'w')) {
                            Keyword::VIEW
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('U' | 'u'))
                            && matches!(cs.next(), Some('A' | 'a'))
                            && matches!(cs.next(), Some('L' | 'l'))
                        {
                            Keyword::VIRTUAL
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('S' | 's') => {
                        if matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('B' | 'b'))
                            && matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::VISIBLE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('W' | 'w') => match cs.next() {
                Some('A' | 'a') => match cs.next() {
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('T' | 't')) {
                            Keyword::WAIT
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('R' | 'r') => {
                        if matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('S' | 's'))
                        {
                            Keyword::WARNINGS
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('E' | 'e') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('K' | 'k') => match cs.next() {
                            None => Keyword::WEEK,
                            Some('D' | 'd') => {
                                if matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('Y' | 'y'))
                                {
                                    Keyword::WEEKDAY
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('O' | 'o') => {
                                if matches!(cs.next(), Some('F' | 'f'))
                                    && matches!(cs.next(), Some('Y' | 'y'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('A' | 'a'))
                                    && matches!(cs.next(), Some('R' | 'r'))
                                {
                                    Keyword::WEEKOFYEAR
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('G' | 'g'))
                            && matches!(cs.next(), Some('H' | 'h'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('_'))
                            && matches!(cs.next(), Some('S' | 's'))
                            && matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('R' | 'r'))
                            && matches!(cs.next(), Some('I' | 'i'))
                            && matches!(cs.next(), Some('N' | 'n'))
                            && matches!(cs.next(), Some('G' | 'g'))
                        {
                            Keyword::WEIGHT_STRING
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('H' | 'h') => match cs.next() {
                    Some('E' | 'e') => match cs.next() {
                        Some('N' | 'n') => Keyword::WHEN,
                        Some('R' | 'r') => {
                            if matches!(cs.next(), Some('E' | 'e')) {
                                Keyword::WHERE
                            } else {
                                Keyword::NOT_A_KEYWORD
                            }
                        }
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('L' | 'l'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::WHILE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('I' | 'i') => match cs.next() {
                    Some('N' | 'n') => {
                        if matches!(cs.next(), Some('D' | 'd'))
                            && matches!(cs.next(), Some('O' | 'o'))
                            && matches!(cs.next(), Some('W' | 'w'))
                        {
                            Keyword::WINDOW
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('T' | 't') => match cs.next() {
                        Some('H' | 'h') => match cs.next() {
                            None => Keyword::WITH,
                            Some('I' | 'i') => {
                                if matches!(cs.next(), Some('N' | 'n')) {
                                    Keyword::WITHIN
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('O' | 'o') => {
                                if matches!(cs.next(), Some('U' | 'u'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                {
                                    Keyword::WITHOUT
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                Some('O' | 'o') => {
                    if matches!(cs.next(), Some('R' | 'r')) && matches!(cs.next(), Some('K' | 'k'))
                    {
                        Keyword::WORK
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('R' | 'r') => match cs.next() {
                    Some('A' | 'a') => {
                        if matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('P' | 'p'))
                            && matches!(cs.next(), Some('E' | 'e'))
                            && matches!(cs.next(), Some('R' | 'r'))
                        {
                            Keyword::WRAPPER
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    Some('I' | 'i') => {
                        if matches!(cs.next(), Some('T' | 't'))
                            && matches!(cs.next(), Some('E' | 'e'))
                        {
                            Keyword::WRITE
                        } else {
                            Keyword::NOT_A_KEYWORD
                        }
                    }
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('X' | 'x') => match cs.next() {
                Some('5') => {
                    if matches!(cs.next(), Some('0')) && matches!(cs.next(), Some('9')) {
                        Keyword::X509
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('A' | 'a') => Keyword::XA,
                Some('M' | 'm') => {
                    if matches!(cs.next(), Some('L' | 'l')) {
                        Keyword::XML
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('O' | 'o') => {
                    if matches!(cs.next(), Some('R' | 'r')) {
                        Keyword::XOR
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('Y' | 'y') => match cs.next() {
                Some('E' | 'e') => match cs.next() {
                    Some('A' | 'a') => match cs.next() {
                        Some('R' | 'r') => match cs.next() {
                            None => Keyword::YEAR,
                            Some('W' | 'w') => {
                                if matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('E' | 'e'))
                                    && matches!(cs.next(), Some('K' | 'k'))
                                {
                                    Keyword::YEARWEEK
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            Some('_') => {
                                if matches!(cs.next(), Some('M' | 'm'))
                                    && matches!(cs.next(), Some('O' | 'o'))
                                    && matches!(cs.next(), Some('N' | 'n'))
                                    && matches!(cs.next(), Some('T' | 't'))
                                    && matches!(cs.next(), Some('H' | 'h'))
                                {
                                    Keyword::YEAR_MONTH
                                } else {
                                    Keyword::NOT_A_KEYWORD
                                }
                            }
                            _ => Keyword::NOT_A_KEYWORD,
                        },
                        _ => Keyword::NOT_A_KEYWORD,
                    },
                    _ => Keyword::NOT_A_KEYWORD,
                },
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('Z' | 'z') => match cs.next() {
                Some('E' | 'e') => {
                    if matches!(cs.next(), Some('R' | 'r'))
                        && matches!(cs.next(), Some('O' | 'o'))
                        && matches!(cs.next(), Some('F' | 'f'))
                        && matches!(cs.next(), Some('I' | 'i'))
                        && matches!(cs.next(), Some('L' | 'l'))
                        && matches!(cs.next(), Some('L' | 'l'))
                    {
                        Keyword::ZEROFILL
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                Some('O' | 'o') => {
                    if matches!(cs.next(), Some('N' | 'n')) && matches!(cs.next(), Some('E' | 'e'))
                    {
                        Keyword::ZONE
                    } else {
                        Keyword::NOT_A_KEYWORD
                    }
                }
                _ => Keyword::NOT_A_KEYWORD,
            },
            Some('_') => {
                if matches!(cs.next(), Some('L' | 'l'))
                    && matches!(cs.next(), Some('I' | 'i'))
                    && matches!(cs.next(), Some('S' | 's'))
                    && matches!(cs.next(), Some('T' | 't'))
                    && matches!(cs.next(), Some('_'))
                {
                    Keyword::_LIST_
                } else {
                    Keyword::NOT_A_KEYWORD
                }
            }
            _ => Keyword::NOT_A_KEYWORD,
        }
    }
}

impl Keyword {
    pub fn name(&self) -> &'static str {
        match self {
            Keyword::NOT_A_KEYWORD => "NOT_A_KEYWORD",
            Keyword::QUOTED_IDENTIFIER => "QUOTED_IDENTIFIER",
            Keyword::ABS => "ABS",
            Keyword::ACCESSIBLE => "ACCESSIBLE",
            Keyword::ACCOUNT => "ACCOUNT",
            Keyword::ACOS => "ACOS",
            Keyword::ACTION => "ACTION",
            Keyword::ADD => "ADD",
            Keyword::ADDDATE => "ADDDATE",
            Keyword::ADDTIME => "ADDTIME",
            Keyword::ADD_MONTHS => "ADD_MONTHS",
            Keyword::ADMIN => "ADMIN",
            Keyword::AFTER => "AFTER",
            Keyword::AGAINST => "AGAINST",
            Keyword::AGGREGATE => "AGGREGATE",
            Keyword::ALGORITHM => "ALGORITHM",
            Keyword::ALL => "ALL",
            Keyword::ALTER => "ALTER",
            Keyword::ALWAYS => "ALWAYS",
            Keyword::ANALYZE => "ANALYZE",
            Keyword::AND => "AND",
            Keyword::ANY => "ANY",
            Keyword::AS => "AS",
            Keyword::ASC => "ASC",
            Keyword::ASCII => "ASCII",
            Keyword::ASENSITIVE => "ASENSITIVE",
            Keyword::ASIN => "ASIN",
            Keyword::AT => "AT",
            Keyword::ATAN => "ATAN",
            Keyword::ATAN2 => "ATAN2",
            Keyword::ATOMIC => "ATOMIC",
            Keyword::AUTHORIZATION => "AUTHORIZATION",
            Keyword::AUTHORS => "AUTHORS",
            Keyword::AUTO => "AUTO",
            Keyword::AUTOEXTEND_SIZE => "AUTOEXTEND_SIZE",
            Keyword::AUTO_INCREMENT => "AUTO_INCREMENT",
            Keyword::AVG => "AVG",
            Keyword::AVG_ROW_LENGTH => "AVG_ROW_LENGTH",
            Keyword::BACKUP => "BACKUP",
            Keyword::BEFORE => "BEFORE",
            Keyword::BEGIN => "BEGIN",
            Keyword::BETWEEN => "BETWEEN",
            Keyword::BIGINT => "BIGINT",
            Keyword::BIN => "BIN",
            Keyword::BINARY => "BINARY",
            Keyword::BINLOG => "BINLOG",
            Keyword::BIT => "BIT",
            Keyword::BIT_LENGTH => "BIT_LENGTH",
            Keyword::BLOB => "BLOB",
            Keyword::BLOCK => "BLOCK",
            Keyword::BLOOM => "BLOOM",
            Keyword::BODY => "BODY",
            Keyword::BOOL => "BOOL",
            Keyword::BOOLEAN => "BOOLEAN",
            Keyword::BOTH => "BOTH",
            Keyword::BPCHAR_PATTERN_OPS => "BPCHAR_PATTERN_OPS",
            Keyword::BRIN => "BRIN",
            Keyword::BTREE => "BTREE",
            Keyword::BY => "BY",
            Keyword::BYPASSRLS => "BYPASSRLS",
            Keyword::BYTE => "BYTE",
            Keyword::BYTEA => "BYTEA",
            Keyword::CACHE => "CACHE",
            Keyword::CALL => "CALL",
            Keyword::CASCADE => "CASCADE",
            Keyword::CASCADED => "CASCADED",
            Keyword::CASE => "CASE",
            Keyword::CAST => "CAST",
            Keyword::CATALOG_NAME => "CATALOG_NAME",
            Keyword::CEIL => "CEIL",
            Keyword::CEILING => "CEILING",
            Keyword::CHAIN => "CHAIN",
            Keyword::CHANGE => "CHANGE",
            Keyword::CHANGED => "CHANGED",
            Keyword::CHANNEL => "CHANNEL",
            Keyword::CHAR => "CHAR",
            Keyword::CHARACTER => "CHARACTER",
            Keyword::CHARACTER_LENGTH => "CHARACTER_LENGTH",
            Keyword::CHARSET => "CHARSET",
            Keyword::CHAR_LENGTH => "CHAR_LENGTH",
            Keyword::CHECK => "CHECK",
            Keyword::CHECKPOINT => "CHECKPOINT",
            Keyword::CHECKSUM => "CHECKSUM",
            Keyword::CHR => "CHR",
            Keyword::CIPHER => "CIPHER",
            Keyword::CLASS => "CLASS",
            Keyword::CLASS_ORIGIN => "CLASS_ORIGIN",
            Keyword::CLIENT => "CLIENT",
            Keyword::CLOB => "CLOB",
            Keyword::CLOSE => "CLOSE",
            Keyword::COALESCE => "COALESCE",
            Keyword::CODE => "CODE",
            Keyword::COLLATE => "COLLATE",
            Keyword::COLLATION => "COLLATION",
            Keyword::COLUMN => "COLUMN",
            Keyword::COLUMNS => "COLUMNS",
            Keyword::COLUMN_ADD => "COLUMN_ADD",
            Keyword::COLUMN_CHECK => "COLUMN_CHECK",
            Keyword::COLUMN_CREATE => "COLUMN_CREATE",
            Keyword::COLUMN_DELETE => "COLUMN_DELETE",
            Keyword::COLUMN_GET => "COLUMN_GET",
            Keyword::COLUMN_NAME => "COLUMN_NAME",
            Keyword::COMMENT => "COMMENT",
            Keyword::COMMIT => "COMMIT",
            Keyword::COMMITTED => "COMMITTED",
            Keyword::COMMUTATOR => "COMMUTATOR",
            Keyword::COMPACT => "COMPACT",
            Keyword::COMPLETION => "COMPLETION",
            Keyword::COMPRESSED => "COMPRESSED",
            Keyword::COMPRESSION => "COMPRESSION",
            Keyword::CONCAT => "CONCAT",
            Keyword::CONCAT_WS => "CONCAT_WS",
            Keyword::CONCURRENT => "CONCURRENT",
            Keyword::CONCURRENTLY => "CONCURRENTLY",
            Keyword::CONDITION => "CONDITION",
            Keyword::CONFLICT => "CONFLICT",
            Keyword::CONNECTION => "CONNECTION",
            Keyword::CONSISTENT => "CONSISTENT",
            Keyword::CONSTRAINT => "CONSTRAINT",
            Keyword::CONSTRAINT_CATALOG => "CONSTRAINT_CATALOG",
            Keyword::CONSTRAINT_NAME => "CONSTRAINT_NAME",
            Keyword::CONSTRAINT_SCHEMA => "CONSTRAINT_SCHEMA",
            Keyword::CONTAINS => "CONTAINS",
            Keyword::CONTEXT => "CONTEXT",
            Keyword::CONTINUE => "CONTINUE",
            Keyword::CONTRIBUTORS => "CONTRIBUTORS",
            Keyword::CONV => "CONV",
            Keyword::CONVERT => "CONVERT",
            Keyword::CONVERT_TZ => "CONVERT_TZ",
            Keyword::COPY => "COPY",
            Keyword::COS => "COS",
            Keyword::COSTS => "COSTS",
            Keyword::COT => "COT",
            Keyword::COUNT => "COUNT",
            Keyword::CPU => "CPU",
            Keyword::CRC32 => "CRC32",
            Keyword::CRC32C => "CRC32C",
            Keyword::CREATE => "CREATE",
            Keyword::CREATEDB => "CREATEDB",
            Keyword::CREATEROLE => "CREATEROLE",
            Keyword::CROSS => "CROSS",
            Keyword::CUBE => "CUBE",
            Keyword::CURDATE => "CURDATE",
            Keyword::CURRENT => "CURRENT",
            Keyword::CURRENT_DATE => "CURRENT_DATE",
            Keyword::CURRENT_POS => "CURRENT_POS",
            Keyword::CURRENT_ROLE => "CURRENT_ROLE",
            Keyword::CURRENT_TIME => "CURRENT_TIME",
            Keyword::CURRENT_TIMESTAMP => "CURRENT_TIMESTAMP",
            Keyword::CURRENT_USER => "CURRENT_USER",
            Keyword::CURSOR => "CURSOR",
            Keyword::CURSOR_NAME => "CURSOR_NAME",
            Keyword::CURTIME => "CURTIME",
            Keyword::CYCLE => "CYCLE",
            Keyword::DATA => "DATA",
            Keyword::DATABASE => "DATABASE",
            Keyword::DATABASES => "DATABASES",
            Keyword::DATAFILE => "DATAFILE",
            Keyword::DATE => "DATE",
            Keyword::DATEDIFF => "DATEDIFF",
            Keyword::DATETIME => "DATETIME",
            Keyword::DATE_ADD => "DATE_ADD",
            Keyword::DATE_FORMAT => "DATE_FORMAT",
            Keyword::DATE_SUB => "DATE_SUB",
            Keyword::DAY => "DAY",
            Keyword::DAYNAME => "DAYNAME",
            Keyword::DAYOFMONTH => "DAYOFMONTH",
            Keyword::DAYOFWEEK => "DAYOFWEEK",
            Keyword::DAYOFYEAR => "DAYOFYEAR",
            Keyword::DAY_HOUR => "DAY_HOUR",
            Keyword::DAY_MICROSECOND => "DAY_MICROSECOND",
            Keyword::DAY_MINUTE => "DAY_MINUTE",
            Keyword::DAY_SECOND => "DAY_SECOND",
            Keyword::DEALLOCATE => "DEALLOCATE",
            Keyword::DEC => "DEC",
            Keyword::DECIMAL => "DECIMAL",
            Keyword::DECLARE => "DECLARE",
            Keyword::DEFAULT => "DEFAULT",
            Keyword::DEFINER => "DEFINER",
            Keyword::DEGREES => "DEGREES",
            Keyword::DELAYED => "DELAYED",
            Keyword::DELAY_KEY_WRITE => "DELAY_KEY_WRITE",
            Keyword::DELETE => "DELETE",
            Keyword::DELETE_DOMAIN_ID => "DELETE_DOMAIN_ID",
            Keyword::DELIMITER => "DELIMITER",
            Keyword::DESC => "DESC",
            Keyword::DESCRIBE => "DESCRIBE",
            Keyword::DES_KEY_FILE => "DES_KEY_FILE",
            Keyword::DETERMINISTIC => "DETERMINISTIC",
            Keyword::DIAGNOSTICS => "DIAGNOSTICS",
            Keyword::DIRECTORY => "DIRECTORY",
            Keyword::DISABLE => "DISABLE",
            Keyword::DISCARD => "DISCARD",
            Keyword::DISK => "DISK",
            Keyword::DISTINCT => "DISTINCT",
            Keyword::DISTINCTROW => "DISTINCTROW",
            Keyword::DIV => "DIV",
            Keyword::DO => "DO",
            Keyword::DOMAIN => "DOMAIN",
            Keyword::DOUBLE => "DOUBLE",
            Keyword::DO_DOMAIN_IDS => "DO_DOMAIN_IDS",
            Keyword::DROP => "DROP",
            Keyword::DUAL => "DUAL",
            Keyword::DUMPFILE => "DUMPFILE",
            Keyword::DUPLICATE => "DUPLICATE",
            Keyword::DYNAMIC => "DYNAMIC",
            Keyword::EACH => "EACH",
            Keyword::ELSE => "ELSE",
            Keyword::ELSEIF => "ELSEIF",
            Keyword::ELSIF => "ELSIF",
            Keyword::ELT => "ELT",
            Keyword::EMPTY => "EMPTY",
            Keyword::ENABLE => "ENABLE",
            Keyword::ENCLOSED => "ENCLOSED",
            Keyword::ENCRYPTED => "ENCRYPTED",
            Keyword::ENCRYPTION => "ENCRYPTION",
            Keyword::END => "END",
            Keyword::ENDS => "ENDS",
            Keyword::ENGINE => "ENGINE",
            Keyword::ENGINES => "ENGINES",
            Keyword::ENGINE_ATTRIBUTE => "ENGINE_ATTRIBUTE",
            Keyword::ENUM => "ENUM",
            Keyword::ERROR => "ERROR",
            Keyword::ERRORS => "ERRORS",
            Keyword::ESCAPE => "ESCAPE",
            Keyword::ESCAPED => "ESCAPED",
            Keyword::EVENT => "EVENT",
            Keyword::EVENTS => "EVENTS",
            Keyword::EVERY => "EVERY",
            Keyword::EXAMINED => "EXAMINED",
            Keyword::EXCEPT => "EXCEPT",
            Keyword::EXCEPTION => "EXCEPTION",
            Keyword::EXCHANGE => "EXCHANGE",
            Keyword::EXCLUDE => "EXCLUDE",
            Keyword::EXCLUSIVE => "EXCLUSIVE",
            Keyword::EXECUTE => "EXECUTE",
            Keyword::EXISTS => "EXISTS",
            Keyword::EXIT => "EXIT",
            Keyword::EXP => "EXP",
            Keyword::EXPANSION => "EXPANSION",
            Keyword::EXPIRE => "EXPIRE",
            Keyword::EXPLAIN => "EXPLAIN",
            Keyword::EXPORT => "EXPORT",
            Keyword::EXPORT_SET => "EXPORT_SET",
            Keyword::EXTENDED => "EXTENDED",
            Keyword::EXTENSION => "EXTENSION",
            Keyword::EXTENT_SIZE => "EXTENT_SIZE",
            Keyword::EXTRACT => "EXTRACT",
            Keyword::EXTRACTVALUE => "EXTRACTVALUE",
            Keyword::FALSE => "FALSE",
            Keyword::FAMILY => "FAMILY",
            Keyword::FAST => "FAST",
            Keyword::FAULTS => "FAULTS",
            Keyword::FEDERATED => "FEDERATED",
            Keyword::FETCH => "FETCH",
            Keyword::FIELD => "FIELD",
            Keyword::FIELDS => "FIELDS",
            Keyword::FILE => "FILE",
            Keyword::FIND_IN_SET => "FIND_IN_SET",
            Keyword::FIRST => "FIRST",
            Keyword::FIXED => "FIXED",
            Keyword::FLOAT => "FLOAT",
            Keyword::FLOAT4 => "FLOAT4",
            Keyword::FLOAT8 => "FLOAT8",
            Keyword::FLOOR => "FLOOR",
            Keyword::FLUSH => "FLUSH",
            Keyword::FOLLOWING => "FOLLOWING",
            Keyword::FOLLOWS => "FOLLOWS",
            Keyword::FOR => "FOR",
            Keyword::FORCE => "FORCE",
            Keyword::FOREIGN => "FOREIGN",
            Keyword::FORMAT => "FORMAT",
            Keyword::FOUND => "FOUND",
            Keyword::FROM => "FROM",
            Keyword::FROM_BASE64 => "FROM_BASE64",
            Keyword::FROM_DAYS => "FROM_DAYS",
            Keyword::FROM_UNIXTIME => "FROM_UNIXTIME",
            Keyword::FULL => "FULL",
            Keyword::FULLTEXT => "FULLTEXT",
            Keyword::FUNCTION => "FUNCTION",
            Keyword::GENERAL => "GENERAL",
            Keyword::GENERATED => "GENERATED",
            Keyword::GET => "GET",
            Keyword::GET_FORMAT => "GET_FORMAT",
            Keyword::GIN => "GIN",
            Keyword::GIST => "GIST",
            Keyword::GLOBAL => "GLOBAL",
            Keyword::GOTO => "GOTO",
            Keyword::GRANT => "GRANT",
            Keyword::GRANTS => "GRANTS",
            Keyword::GREATEST => "GREATEST",
            Keyword::GROUP => "GROUP",
            Keyword::GROUP_CONCAT => "GROUP_CONCAT",
            Keyword::HANDLER => "HANDLER",
            Keyword::HARD => "HARD",
            Keyword::HASH => "HASH",
            Keyword::HASHES => "HASHES",
            Keyword::HAVING => "HAVING",
            Keyword::HELP => "HELP",
            Keyword::HEX => "HEX",
            Keyword::HIGH_PRIORITY => "HIGH_PRIORITY",
            Keyword::HISTORY => "HISTORY",
            Keyword::HNSW => "HNSW",
            Keyword::HOST => "HOST",
            Keyword::HOSTS => "HOSTS",
            Keyword::HOUR => "HOUR",
            Keyword::HOUR_MICROSECOND => "HOUR_MICROSECOND",
            Keyword::HOUR_MINUTE => "HOUR_MINUTE",
            Keyword::HOUR_SECOND => "HOUR_SECOND",
            Keyword::ID => "ID",
            Keyword::IDENTIFIED => "IDENTIFIED",
            Keyword::IDENTITY => "IDENTITY",
            Keyword::IF => "IF",
            Keyword::IFNULL => "IFNULL",
            Keyword::IGNORE => "IGNORE",
            Keyword::IGNORED => "IGNORED",
            Keyword::IGNORE_DOMAIN_IDS => "IGNORE_DOMAIN_IDS",
            Keyword::IGNORE_SERVER_IDS => "IGNORE_SERVER_IDS",
            Keyword::IMMEDIATE => "IMMEDIATE",
            Keyword::IMPORT => "IMPORT",
            Keyword::IN => "IN",
            Keyword::INCLUDE => "INCLUDE",
            Keyword::INCREMENT => "INCREMENT",
            Keyword::INDEX => "INDEX",
            Keyword::INDEXES => "INDEXES",
            Keyword::INET4 => "INET4",
            Keyword::INET6 => "INET6",
            Keyword::INFILE => "INFILE",
            Keyword::INHERIT => "INHERIT",
            Keyword::INHERITS => "INHERITS",
            Keyword::INITIAL_SIZE => "INITIAL_SIZE",
            Keyword::INNER => "INNER",
            Keyword::INOUT => "INOUT",
            Keyword::INPLACE => "INPLACE",
            Keyword::INSENSITIVE => "INSENSITIVE",
            Keyword::INSERT => "INSERT",
            Keyword::INSERT_METHOD => "INSERT_METHOD",
            Keyword::INSTALL => "INSTALL",
            Keyword::INSTANT => "INSTANT",
            Keyword::INSTR => "INSTR",
            Keyword::INT => "INT",
            Keyword::INT1 => "INT1",
            Keyword::INT2 => "INT2",
            Keyword::INT2_OPS => "INT2_OPS",
            Keyword::INT3 => "INT3",
            Keyword::INT4 => "INT4",
            Keyword::INT4_OPS => "INT4_OPS",
            Keyword::INT8 => "INT8",
            Keyword::INT8_OPS => "INT8_OPS",
            Keyword::INTEGER => "INTEGER",
            Keyword::INTERSECT => "INTERSECT",
            Keyword::INTERSECTA => "INTERSECTA",
            Keyword::INTERVAL => "INTERVAL",
            Keyword::INTO => "INTO",
            Keyword::INVISIBLE => "INVISIBLE",
            Keyword::INVOKER => "INVOKER",
            Keyword::IO => "IO",
            Keyword::IO_THREAD => "IO_THREAD",
            Keyword::IPC => "IPC",
            Keyword::IS => "IS",
            Keyword::ISOLATION => "ISOLATION",
            Keyword::ISOPEN => "ISOPEN",
            Keyword::ISSUER => "ISSUER",
            Keyword::ITERATE => "ITERATE",
            Keyword::JOIN => "JOIN",
            Keyword::JSON => "JSON",
            Keyword::JSON_ARRAY => "JSON_ARRAY",
            Keyword::JSON_ARRAYAGG => "JSON_ARRAYAGG",
            Keyword::JSON_ARRAY_APPEND => "JSON_ARRAY_APPEND",
            Keyword::JSON_ARRAY_INSERT => "JSON_ARRAY_INSERT",
            Keyword::JSON_ARRAY_INTERSECT => "JSON_ARRAY_INTERSECT",
            Keyword::JSON_COMPACT => "JSON_COMPACT",
            Keyword::JSON_CONTAINS => "JSON_CONTAINS",
            Keyword::JSON_CONTAINS_PATH => "JSON_CONTAINS_PATH",
            Keyword::JSON_DEPTH => "JSON_DEPTH",
            Keyword::JSON_DETAILED => "JSON_DETAILED",
            Keyword::JSON_EQUALS => "JSON_EQUALS",
            Keyword::JSON_EXISTS => "JSON_EXISTS",
            Keyword::JSON_EXTRACT => "JSON_EXTRACT",
            Keyword::JSON_INSERT => "JSON_INSERT",
            Keyword::JSON_KEYS => "JSON_KEYS",
            Keyword::JSON_LENGTH => "JSON_LENGTH",
            Keyword::JSON_LOOSE => "JSON_LOOSE",
            Keyword::JSON_MERGE => "JSON_MERGE",
            Keyword::JSON_MERGE_PATCH => "JSON_MERGE_PATCH",
            Keyword::JSON_MERGE_PRESERVE => "JSON_MERGE_PRESERVE",
            Keyword::JSON_NORMALIZE => "JSON_NORMALIZE",
            Keyword::JSON_OBJECT => "JSON_OBJECT",
            Keyword::JSON_OBJECTAGG => "JSON_OBJECTAGG",
            Keyword::JSON_OBJECT_FILTER_KEYS => "JSON_OBJECT_FILTER_KEYS",
            Keyword::JSON_OBJECT_TO_ARRAY => "JSON_OBJECT_TO_ARRAY",
            Keyword::JSON_OVERLAPS => "JSON_OVERLAPS",
            Keyword::JSON_PRETTY => "JSON_PRETTY",
            Keyword::JSON_QUERY => "JSON_QUERY",
            Keyword::JSON_QUOTE => "JSON_QUOTE",
            Keyword::JSON_REMOVE => "JSON_REMOVE",
            Keyword::JSON_REPLACE => "JSON_REPLACE",
            Keyword::JSON_SCHEMA_VALID => "JSON_SCHEMA_VALID",
            Keyword::JSON_SEARCH => "JSON_SEARCH",
            Keyword::JSON_SET => "JSON_SET",
            Keyword::JSON_TABLE => "JSON_TABLE",
            Keyword::JSON_TYPE => "JSON_TYPE",
            Keyword::JSON_UNQUOTE => "JSON_UNQUOTE",
            Keyword::JSON_VALID => "JSON_VALID",
            Keyword::JSON_VALUE => "JSON_VALUE",
            Keyword::KEY => "KEY",
            Keyword::KEYS => "KEYS",
            Keyword::KEY_BLOCK_SIZE => "KEY_BLOCK_SIZE",
            Keyword::KILL => "KILL",
            Keyword::LAG => "LAG",
            Keyword::LANGUAGE => "LANGUAGE",
            Keyword::LAST => "LAST",
            Keyword::LASTVAL => "LASTVAL",
            Keyword::LAST_DAY => "LAST_DAY",
            Keyword::LAST_VALUE => "LAST_VALUE",
            Keyword::LCASE => "LCASE",
            Keyword::LEAD => "LEAD",
            Keyword::LEADING => "LEADING",
            Keyword::LEAST => "LEAST",
            Keyword::LEAVE => "LEAVE",
            Keyword::LEAVES => "LEAVES",
            Keyword::LEFT => "LEFT",
            Keyword::LEFTARG => "LEFTARG",
            Keyword::LENGTH => "LENGTH",
            Keyword::LENGTHB => "LENGTHB",
            Keyword::LESS => "LESS",
            Keyword::LEVEL => "LEVEL",
            Keyword::LIKE => "LIKE",
            Keyword::LIMIT => "LIMIT",
            Keyword::LINEAR => "LINEAR",
            Keyword::LINES => "LINES",
            Keyword::LIST => "LIST",
            Keyword::LN => "LN",
            Keyword::LOAD => "LOAD",
            Keyword::LOAD_FILE => "LOAD_FILE",
            Keyword::LOCAL => "LOCAL",
            Keyword::LOCALTIME => "LOCALTIME",
            Keyword::LOCALTIMESTAMP => "LOCALTIMESTAMP",
            Keyword::LOCATE => "LOCATE",
            Keyword::LOCK => "LOCK",
            Keyword::LOCKED => "LOCKED",
            Keyword::LOCKS => "LOCKS",
            Keyword::LOG => "LOG",
            Keyword::LOG10 => "LOG10",
            Keyword::LOG2 => "LOG2",
            Keyword::LOGFILE => "LOGFILE",
            Keyword::LOGIN => "LOGIN",
            Keyword::LOGS => "LOGS",
            Keyword::LONG => "LONG",
            Keyword::LONGBLOB => "LONGBLOB",
            Keyword::LONGTEXT => "LONGTEXT",
            Keyword::LOOP => "LOOP",
            Keyword::LOWER => "LOWER",
            Keyword::LOW_PRIORITY => "LOW_PRIORITY",
            Keyword::LPAD => "LPAD",
            Keyword::LTRIM => "LTRIM",
            Keyword::MAKEDATE => "MAKEDATE",
            Keyword::MAKETIME => "MAKETIME",
            Keyword::MAKE_SET => "MAKE_SET",
            Keyword::MASTER => "MASTER",
            Keyword::MASTER_CONNECT_RETRY => "MASTER_CONNECT_RETRY",
            Keyword::MASTER_DELAY => "MASTER_DELAY",
            Keyword::MASTER_GTID_POS => "MASTER_GTID_POS",
            Keyword::MASTER_HEARTBEAT_PERIOD => "MASTER_HEARTBEAT_PERIOD",
            Keyword::MASTER_HOST => "MASTER_HOST",
            Keyword::MASTER_LOG_FILE => "MASTER_LOG_FILE",
            Keyword::MASTER_LOG_POS => "MASTER_LOG_POS",
            Keyword::MASTER_PASSWORD => "MASTER_PASSWORD",
            Keyword::MASTER_PORT => "MASTER_PORT",
            Keyword::MASTER_SERVER_ID => "MASTER_SERVER_ID",
            Keyword::MASTER_SSL => "MASTER_SSL",
            Keyword::MASTER_SSL_CA => "MASTER_SSL_CA",
            Keyword::MASTER_SSL_CAPATH => "MASTER_SSL_CAPATH",
            Keyword::MASTER_SSL_CERT => "MASTER_SSL_CERT",
            Keyword::MASTER_SSL_CIPHER => "MASTER_SSL_CIPHER",
            Keyword::MASTER_SSL_CRL => "MASTER_SSL_CRL",
            Keyword::MASTER_SSL_CRLPATH => "MASTER_SSL_CRLPATH",
            Keyword::MASTER_SSL_KEY => "MASTER_SSL_KEY",
            Keyword::MASTER_SSL_VERIFY_SERVER_CERT => "MASTER_SSL_VERIFY_SERVER_CERT",
            Keyword::MASTER_USER => "MASTER_USER",
            Keyword::MASTER_USE_GTID => "MASTER_USE_GTID",
            Keyword::MATCH => "MATCH",
            Keyword::MATERIALIZED => "MATERIALIZED",
            Keyword::MAX => "MAX",
            Keyword::MAXVALUE => "MAXVALUE",
            Keyword::MAX_CONNECTIONS_PER_HOUR => "MAX_CONNECTIONS_PER_HOUR",
            Keyword::MAX_QUERIES_PER_HOUR => "MAX_QUERIES_PER_HOUR",
            Keyword::MAX_ROWS => "MAX_ROWS",
            Keyword::MAX_SIZE => "MAX_SIZE",
            Keyword::MAX_STATEMENT_TIME => "MAX_STATEMENT_TIME",
            Keyword::MAX_UPDATES_PER_HOUR => "MAX_UPDATES_PER_HOUR",
            Keyword::MAX_USER_CONNECTIONS => "MAX_USER_CONNECTIONS",
            Keyword::MEDIUM => "MEDIUM",
            Keyword::MEDIUMBLOB => "MEDIUMBLOB",
            Keyword::MEDIUMINT => "MEDIUMINT",
            Keyword::MEDIUMTEXT => "MEDIUMTEXT",
            Keyword::MEMBER => "MEMBER",
            Keyword::MEMORY => "MEMORY",
            Keyword::MERGE => "MERGE",
            Keyword::MERGES => "MERGES",
            Keyword::MESSAGE_TEXT => "MESSAGE_TEXT",
            Keyword::MICROSECOND => "MICROSECOND",
            Keyword::MID => "MID",
            Keyword::MIDDLEINT => "MIDDLEINT",
            Keyword::MIGRATE => "MIGRATE",
            Keyword::MIN => "MIN",
            Keyword::MINUS => "MINUS",
            Keyword::MINUTE => "MINUTE",
            Keyword::MINUTE_MICROSECOND => "MINUTE_MICROSECOND",
            Keyword::MINUTE_SECOND => "MINUTE_SECOND",
            Keyword::MINVALUE => "MINVALUE",
            Keyword::MIN_ROWS => "MIN_ROWS",
            Keyword::MOD => "MOD",
            Keyword::MODE => "MODE",
            Keyword::MODIFIES => "MODIFIES",
            Keyword::MODIFY => "MODIFY",
            Keyword::MONITOR => "MONITOR",
            Keyword::MONTH => "MONTH",
            Keyword::MONTHNAME => "MONTHNAME",
            Keyword::MUTEX => "MUTEX",
            Keyword::MYSQL => "MYSQL",
            Keyword::MYSQL_ERRNO => "MYSQL_ERRNO",
            Keyword::NAME => "NAME",
            Keyword::NAMES => "NAMES",
            Keyword::NATIONAL => "NATIONAL",
            Keyword::NATURAL => "NATURAL",
            Keyword::NATURAL_SORT_KEY => "NATURAL_SORT_KEY",
            Keyword::NCHAR => "NCHAR",
            Keyword::NEGATOR => "NEGATOR",
            Keyword::NESTED => "NESTED",
            Keyword::NEVER => "NEVER",
            Keyword::NEW => "NEW",
            Keyword::NEXT => "NEXT",
            Keyword::NEXTVAL => "NEXTVAL",
            Keyword::NO => "NO",
            Keyword::NOBYPASSRLS => "NOBYPASSRLS",
            Keyword::NOCACHE => "NOCACHE",
            Keyword::NOCREATEDB => "NOCREATEDB",
            Keyword::NOCREATEROLE => "NOCREATEROLE",
            Keyword::NOCYCLE => "NOCYCLE",
            Keyword::NODEGROUP => "NODEGROUP",
            Keyword::NOINHERIT => "NOINHERIT",
            Keyword::NOLOGIN => "NOLOGIN",
            Keyword::NOMAXVALUE => "NOMAXVALUE",
            Keyword::NOMINVALUE => "NOMINVALUE",
            Keyword::NONE => "NONE",
            Keyword::NOREPLICATION => "NOREPLICATION",
            Keyword::NOSUPERUSER => "NOSUPERUSER",
            Keyword::NOT => "NOT",
            Keyword::NOTFOUND => "NOTFOUND",
            Keyword::NOTHING => "NOTHING",
            Keyword::NOTICE => "NOTICE",
            Keyword::NOW => "NOW",
            Keyword::NOWAIT => "NOWAIT",
            Keyword::NO_WAIT => "NO_WAIT",
            Keyword::NO_WRITE_TO_BINLOG => "NO_WRITE_TO_BINLOG",
            Keyword::NULL => "NULL",
            Keyword::NULLIF => "NULLIF",
            Keyword::NULLS => "NULLS",
            Keyword::NUMBER => "NUMBER",
            Keyword::NUMERIC => "NUMERIC",
            Keyword::NVARCHAR => "NVARCHAR",
            Keyword::NVL => "NVL",
            Keyword::NVL2 => "NVL2",
            Keyword::OCT => "OCT",
            Keyword::OCTET_LENGTH => "OCTET_LENGTH",
            Keyword::OF => "OF",
            Keyword::OFFSET => "OFFSET",
            Keyword::OLD_PASSWORD => "OLD_PASSWORD",
            Keyword::ON => "ON",
            Keyword::ONE => "ONE",
            Keyword::ONLINE => "ONLINE",
            Keyword::ONLY => "ONLY",
            Keyword::OPEN => "OPEN",
            Keyword::OPERATOR => "OPERATOR",
            Keyword::OPTIMIZE => "OPTIMIZE",
            Keyword::OPTIMIZER => "OPTIMIZER",
            Keyword::OPTIMIZER_COSTS => "OPTIMIZER_COSTS",
            Keyword::OPTION => "OPTION",
            Keyword::OPTIONALLY => "OPTIONALLY",
            Keyword::OPTIONS => "OPTIONS",
            Keyword::OR => "OR",
            Keyword::ORD => "ORD",
            Keyword::ORDER => "ORDER",
            Keyword::ORDINALITY => "ORDINALITY",
            Keyword::OTHERS => "OTHERS",
            Keyword::OUT => "OUT",
            Keyword::OUTER => "OUTER",
            Keyword::OUTFILE => "OUTFILE",
            Keyword::OVER => "OVER",
            Keyword::OVERLAPS => "OVERLAPS",
            Keyword::OWNED => "OWNED",
            Keyword::OWNER => "OWNER",
            Keyword::PACKAGE => "PACKAGE",
            Keyword::PACK_KEYS => "PACK_KEYS",
            Keyword::PAGE => "PAGE",
            Keyword::PAGE_CHECKSUM => "PAGE_CHECKSUM",
            Keyword::PARSER => "PARSER",
            Keyword::PARSE_VCOL_EXPR => "PARSE_VCOL_EXPR",
            Keyword::PARTIAL => "PARTIAL",
            Keyword::PARTITION => "PARTITION",
            Keyword::PARTITIONING => "PARTITIONING",
            Keyword::PARTITIONS => "PARTITIONS",
            Keyword::PASSWORD => "PASSWORD",
            Keyword::PATH => "PATH",
            Keyword::PERIOD => "PERIOD",
            Keyword::PERIOD_ADD => "PERIOD_ADD",
            Keyword::PERIOD_DIFF => "PERIOD_DIFF",
            Keyword::PERSISTENT => "PERSISTENT",
            Keyword::PHASE => "PHASE",
            Keyword::PI => "PI",
            Keyword::PLPGSQL => "PLPGSQL",
            Keyword::PLUGIN => "PLUGIN",
            Keyword::PLUGINS => "PLUGINS",
            Keyword::PORT => "PORT",
            Keyword::PORTION => "PORTION",
            Keyword::POSITION => "POSITION",
            Keyword::POW => "POW",
            Keyword::POWER => "POWER",
            Keyword::PRECEDES => "PRECEDES",
            Keyword::PRECEDING => "PRECEDING",
            Keyword::PRECISION => "PRECISION",
            Keyword::PREPARE => "PREPARE",
            Keyword::PRESERVE => "PRESERVE",
            Keyword::PREV => "PREV",
            Keyword::PREVIOUS => "PREVIOUS",
            Keyword::PRIMARY => "PRIMARY",
            Keyword::PRIVILEGES => "PRIVILEGES",
            Keyword::PROCEDURE => "PROCEDURE",
            Keyword::PROCESS => "PROCESS",
            Keyword::PROCESSLIST => "PROCESSLIST",
            Keyword::PROFILE => "PROFILE",
            Keyword::PROFILES => "PROFILES",
            Keyword::PROXY => "PROXY",
            Keyword::PURGE => "PURGE",
            Keyword::QUARTER => "QUARTER",
            Keyword::QUERY => "QUERY",
            Keyword::QUICK => "QUICK",
            Keyword::QUOTE => "QUOTE",
            Keyword::RADIANS => "RADIANS",
            Keyword::RAISE => "RAISE",
            Keyword::RAND => "RAND",
            Keyword::RANGE => "RANGE",
            Keyword::RAW => "RAW",
            Keyword::READ => "READ",
            Keyword::READS => "READS",
            Keyword::READ_ONLY => "READ_ONLY",
            Keyword::READ_WRITE => "READ_WRITE",
            Keyword::REAL => "REAL",
            Keyword::REBUILD => "REBUILD",
            Keyword::RECOVER => "RECOVER",
            Keyword::RECURSIVE => "RECURSIVE",
            Keyword::REDOFILE => "REDOFILE",
            Keyword::REDO_BUFFER_SIZE => "REDO_BUFFER_SIZE",
            Keyword::REDUNDANT => "REDUNDANT",
            Keyword::REFERENCES => "REFERENCES",
            Keyword::REF_SYSTEM_ID => "REF_SYSTEM_ID",
            Keyword::REGEXP => "REGEXP",
            Keyword::RELAY => "RELAY",
            Keyword::RELAYLOG => "RELAYLOG",
            Keyword::RELAY_LOG_FILE => "RELAY_LOG_FILE",
            Keyword::RELAY_LOG_POS => "RELAY_LOG_POS",
            Keyword::RELAY_THREAD => "RELAY_THREAD",
            Keyword::RELEASE => "RELEASE",
            Keyword::RELOAD => "RELOAD",
            Keyword::REMOVE => "REMOVE",
            Keyword::RENAME => "RENAME",
            Keyword::REORGANIZE => "REORGANIZE",
            Keyword::REPAIR => "REPAIR",
            Keyword::REPEAT => "REPEAT",
            Keyword::REPEATABLE => "REPEATABLE",
            Keyword::REPLACE => "REPLACE",
            Keyword::REPLAY => "REPLAY",
            Keyword::REPLICA => "REPLICA",
            Keyword::REPLICAS => "REPLICAS",
            Keyword::REPLICATION => "REPLICATION",
            Keyword::REPLICA_POS => "REPLICA_POS",
            Keyword::REQUIRE => "REQUIRE",
            Keyword::RESET => "RESET",
            Keyword::RESIGNAL => "RESIGNAL",
            Keyword::RESTART => "RESTART",
            Keyword::RESTORE => "RESTORE",
            Keyword::RESTRICT => "RESTRICT",
            Keyword::RESUME => "RESUME",
            Keyword::RETURN => "RETURN",
            Keyword::RETURNED_SQLSTATE => "RETURNED_SQLSTATE",
            Keyword::RETURNING => "RETURNING",
            Keyword::RETURNS => "RETURNS",
            Keyword::REUSE => "REUSE",
            Keyword::REVERSE => "REVERSE",
            Keyword::REVOKE => "REVOKE",
            Keyword::RIGHT => "RIGHT",
            Keyword::RIGHTARG => "RIGHTARG",
            Keyword::RLIKE => "RLIKE",
            Keyword::ROLE => "ROLE",
            Keyword::ROLLBACK => "ROLLBACK",
            Keyword::ROLLUP => "ROLLUP",
            Keyword::ROUND => "ROUND",
            Keyword::ROUTINE => "ROUTINE",
            Keyword::ROW => "ROW",
            Keyword::ROWCOUNT => "ROWCOUNT",
            Keyword::ROWNUM => "ROWNUM",
            Keyword::ROWS => "ROWS",
            Keyword::ROWTYPE => "ROWTYPE",
            Keyword::ROW_COUNT => "ROW_COUNT",
            Keyword::ROW_FORMAT => "ROW_FORMAT",
            Keyword::RPAD => "RPAD",
            Keyword::RTREE => "RTREE",
            Keyword::RTRIM => "RTRIM",
            Keyword::SAVEPOINT => "SAVEPOINT",
            Keyword::SCHEDULE => "SCHEDULE",
            Keyword::SCHEMA => "SCHEMA",
            Keyword::SCHEMAS => "SCHEMAS",
            Keyword::SCHEMA_NAME => "SCHEMA_NAME",
            Keyword::SEARCH => "SEARCH",
            Keyword::SECOND => "SECOND",
            Keyword::SECONDARY => "SECONDARY",
            Keyword::SECONDARY_ENGINE => "SECONDARY_ENGINE",
            Keyword::SECONDARY_ENGINE_ATTRIBUTE => "SECONDARY_ENGINE_ATTRIBUTE",
            Keyword::SECOND_MICROSECOND => "SECOND_MICROSECOND",
            Keyword::SECURITY => "SECURITY",
            Keyword::SEC_TO_TIME => "SEC_TO_TIME",
            Keyword::SELECT => "SELECT",
            Keyword::SENSITIVE => "SENSITIVE",
            Keyword::SEPARATOR => "SEPARATOR",
            Keyword::SEQUENCE => "SEQUENCE",
            Keyword::SERIAL => "SERIAL",
            Keyword::SERIALIZABLE => "SERIALIZABLE",
            Keyword::SERVER => "SERVER",
            Keyword::SESSION => "SESSION",
            Keyword::SESSION_USER => "SESSION_USER",
            Keyword::SET => "SET",
            Keyword::SETVAL => "SETVAL",
            Keyword::SFORMAT => "SFORMAT",
            Keyword::SHARE => "SHARE",
            Keyword::SHARED => "SHARED",
            Keyword::SHOW => "SHOW",
            Keyword::SHUTDOWN => "SHUTDOWN",
            Keyword::SIGN => "SIGN",
            Keyword::SIGNAL => "SIGNAL",
            Keyword::SIGNED => "SIGNED",
            Keyword::SIMPLE => "SIMPLE",
            Keyword::SIN => "SIN",
            Keyword::SKIP => "SKIP",
            Keyword::SLAVE => "SLAVE",
            Keyword::SLAVES => "SLAVES",
            Keyword::SLAVE_POS => "SLAVE_POS",
            Keyword::SLEEP => "SLEEP",
            Keyword::SLOW => "SLOW",
            Keyword::SMALLINT => "SMALLINT",
            Keyword::SNAPSHOT => "SNAPSHOT",
            Keyword::SOCKET => "SOCKET",
            Keyword::SOFT => "SOFT",
            Keyword::SOME => "SOME",
            Keyword::SONAME => "SONAME",
            Keyword::SOUNDEX => "SOUNDEX",
            Keyword::SOUNDS => "SOUNDS",
            Keyword::SOURCE => "SOURCE",
            Keyword::SPACE => "SPACE",
            Keyword::SPATIAL => "SPATIAL",
            Keyword::SPECIFIC => "SPECIFIC",
            Keyword::SQL => "SQL",
            Keyword::SQLEXCEPTION => "SQLEXCEPTION",
            Keyword::SQLSTATE => "SQLSTATE",
            Keyword::SQLWARNING => "SQLWARNING",
            Keyword::SQL_BIG_RESULT => "SQL_BIG_RESULT",
            Keyword::SQL_BUFFER_RESULT => "SQL_BUFFER_RESULT",
            Keyword::SQL_CACHE => "SQL_CACHE",
            Keyword::SQL_CALC_FOUND_ROWS => "SQL_CALC_FOUND_ROWS",
            Keyword::SQL_NO_CACHE => "SQL_NO_CACHE",
            Keyword::SQL_SMALL_RESULT => "SQL_SMALL_RESULT",
            Keyword::SQL_THREAD => "SQL_THREAD",
            Keyword::SQL_TSI_DAY => "SQL_TSI_DAY",
            Keyword::SQL_TSI_HOUR => "SQL_TSI_HOUR",
            Keyword::SQL_TSI_MINUTE => "SQL_TSI_MINUTE",
            Keyword::SQL_TSI_MONTH => "SQL_TSI_MONTH",
            Keyword::SQL_TSI_QUARTER => "SQL_TSI_QUARTER",
            Keyword::SQL_TSI_SECOND => "SQL_TSI_SECOND",
            Keyword::SQL_TSI_WEEK => "SQL_TSI_WEEK",
            Keyword::SQL_TSI_YEAR => "SQL_TSI_YEAR",
            Keyword::SQRT => "SQRT",
            Keyword::SSL => "SSL",
            Keyword::STAGE => "STAGE",
            Keyword::START => "START",
            Keyword::STARTING => "STARTING",
            Keyword::STARTS => "STARTS",
            Keyword::STARTS_WITH => "STARTS_WITH",
            Keyword::STATEMENT => "STATEMENT",
            Keyword::STATS_AUTO_RECALC => "STATS_AUTO_RECALC",
            Keyword::STATS_PERSISTENT => "STATS_PERSISTENT",
            Keyword::STATS_SAMPLE_PAGES => "STATS_SAMPLE_PAGES",
            Keyword::STATUS => "STATUS",
            Keyword::STDIN => "STDIN",
            Keyword::STOP => "STOP",
            Keyword::STORAGE => "STORAGE",
            Keyword::STORED => "STORED",
            Keyword::STRAIGHT_JOIN => "STRAIGHT_JOIN",
            Keyword::STRCMP => "STRCMP",
            Keyword::STRFTIME => "STRFTIME",
            Keyword::STRICT => "STRICT",
            Keyword::STRING => "STRING",
            Keyword::STR_TO_DATE => "STR_TO_DATE",
            Keyword::SUBCLASS_ORIGIN => "SUBCLASS_ORIGIN",
            Keyword::SUBDATE => "SUBDATE",
            Keyword::SUBJECT => "SUBJECT",
            Keyword::SUBPARTITION => "SUBPARTITION",
            Keyword::SUBPARTITIONS => "SUBPARTITIONS",
            Keyword::SUBSTR => "SUBSTR",
            Keyword::SUBSTRING => "SUBSTRING",
            Keyword::SUBSTRING_INDEX => "SUBSTRING_INDEX",
            Keyword::SUBTIME => "SUBTIME",
            Keyword::SUM => "SUM",
            Keyword::SUPER => "SUPER",
            Keyword::SUPERUSER => "SUPERUSER",
            Keyword::SUSPEND => "SUSPEND",
            Keyword::SWAPS => "SWAPS",
            Keyword::SWITCHES => "SWITCHES",
            Keyword::SYSDATE => "SYSDATE",
            Keyword::SYSID => "SYSID",
            Keyword::SYSTEM => "SYSTEM",
            Keyword::SYSTEM_TIME => "SYSTEM_TIME",
            Keyword::TABLE => "TABLE",
            Keyword::TABLES => "TABLES",
            Keyword::TABLESPACE => "TABLESPACE",
            Keyword::TABLE_CHECKSUM => "TABLE_CHECKSUM",
            Keyword::TABLE_NAME => "TABLE_NAME",
            Keyword::TAN => "TAN",
            Keyword::TEMPORARY => "TEMPORARY",
            Keyword::TEMPTABLE => "TEMPTABLE",
            Keyword::TERMINATED => "TERMINATED",
            Keyword::TEXT => "TEXT",
            Keyword::TEXT_PATTERN_OPS => "TEXT_PATTERN_OPS",
            Keyword::THAN => "THAN",
            Keyword::THEN => "THEN",
            Keyword::THREADS => "THREADS",
            Keyword::TIES => "TIES",
            Keyword::TIME => "TIME",
            Keyword::TIMEDIFF => "TIMEDIFF",
            Keyword::TIMESTAMP => "TIMESTAMP",
            Keyword::TIMESTAMPADD => "TIMESTAMPADD",
            Keyword::TIMESTAMPDIFF => "TIMESTAMPDIFF",
            Keyword::TIMESTAMPTZ => "TIMESTAMPTZ",
            Keyword::TIME_FORMAT => "TIME_FORMAT",
            Keyword::TIME_TO_SEC => "TIME_TO_SEC",
            Keyword::TIME_ZONE => "TIME_ZONE",
            Keyword::TINYBLOB => "TINYBLOB",
            Keyword::TINYINT => "TINYINT",
            Keyword::TINYTEXT => "TINYTEXT",
            Keyword::TO => "TO",
            Keyword::TO_BASE64 => "TO_BASE64",
            Keyword::TO_CHAR => "TO_CHAR",
            Keyword::TO_DAYS => "TO_DAYS",
            Keyword::TO_SECONDS => "TO_SECONDS",
            Keyword::TRAILING => "TRAILING",
            Keyword::TRANSACTION => "TRANSACTION",
            Keyword::TRANSACTIONAL => "TRANSACTIONAL",
            Keyword::TRIGGER => "TRIGGER",
            Keyword::TRIGGERS => "TRIGGERS",
            Keyword::TRUE => "TRUE",
            Keyword::TRUNCATE => "TRUNCATE",
            Keyword::TYPE => "TYPE",
            Keyword::TYPES => "TYPES",
            Keyword::UCASE => "UCASE",
            Keyword::UNBOUNDED => "UNBOUNDED",
            Keyword::UNCOMMITTED => "UNCOMMITTED",
            Keyword::UNCOMPRESSED_LENGTH => "UNCOMPRESSED_LENGTH",
            Keyword::UNDEFINED => "UNDEFINED",
            Keyword::UNDO => "UNDO",
            Keyword::UNDOFILE => "UNDOFILE",
            Keyword::UNDO_BUFFER_SIZE => "UNDO_BUFFER_SIZE",
            Keyword::UNHEX => "UNHEX",
            Keyword::UNICODE => "UNICODE",
            Keyword::UNINSTALL => "UNINSTALL",
            Keyword::UNION => "UNION",
            Keyword::UNIQUE => "UNIQUE",
            Keyword::UNIX_TIMESTAMP => "UNIX_TIMESTAMP",
            Keyword::UNKNOWN => "UNKNOWN",
            Keyword::UNLOCK => "UNLOCK",
            Keyword::UNSIGNED => "UNSIGNED",
            Keyword::UNTIL => "UNTIL",
            Keyword::UPDATE => "UPDATE",
            Keyword::UPDATEXML => "UPDATEXML",
            Keyword::UPGRADE => "UPGRADE",
            Keyword::UPPER => "UPPER",
            Keyword::USAGE => "USAGE",
            Keyword::USE => "USE",
            Keyword::USER => "USER",
            Keyword::USER_RESOURCES => "USER_RESOURCES",
            Keyword::USE_FRM => "USE_FRM",
            Keyword::USING => "USING",
            Keyword::UTC_DATE => "UTC_DATE",
            Keyword::UTC_TIME => "UTC_TIME",
            Keyword::UTC_TIMESTAMP => "UTC_TIMESTAMP",
            Keyword::VALID => "VALID",
            Keyword::VALUE => "VALUE",
            Keyword::VALUES => "VALUES",
            Keyword::VARBINARY => "VARBINARY",
            Keyword::VARBIT => "VARBIT",
            Keyword::VARCHAR => "VARCHAR",
            Keyword::VARCHAR2 => "VARCHAR2",
            Keyword::VARCHARACTER => "VARCHARACTER",
            Keyword::VARCHAR_PATTERN_OPS => "VARCHAR_PATTERN_OPS",
            Keyword::VARIABLES => "VARIABLES",
            Keyword::VARYING => "VARYING",
            Keyword::VERSION => "VERSION",
            Keyword::VERSIONING => "VERSIONING",
            Keyword::VIA => "VIA",
            Keyword::VIEW => "VIEW",
            Keyword::VIRTUAL => "VIRTUAL",
            Keyword::VISIBLE => "VISIBLE",
            Keyword::WAIT => "WAIT",
            Keyword::WARNINGS => "WARNINGS",
            Keyword::WEEK => "WEEK",
            Keyword::WEEKDAY => "WEEKDAY",
            Keyword::WEEKOFYEAR => "WEEKOFYEAR",
            Keyword::WEIGHT_STRING => "WEIGHT_STRING",
            Keyword::WHEN => "WHEN",
            Keyword::WHERE => "WHERE",
            Keyword::WHILE => "WHILE",
            Keyword::WINDOW => "WINDOW",
            Keyword::WITH => "WITH",
            Keyword::WITHIN => "WITHIN",
            Keyword::WITHOUT => "WITHOUT",
            Keyword::WORK => "WORK",
            Keyword::WRAPPER => "WRAPPER",
            Keyword::WRITE => "WRITE",
            Keyword::X509 => "X509",
            Keyword::XA => "XA",
            Keyword::XML => "XML",
            Keyword::XOR => "XOR",
            Keyword::YEAR => "YEAR",
            Keyword::YEARWEEK => "YEARWEEK",
            Keyword::YEAR_MONTH => "YEAR_MONTH",
            Keyword::ZEROFILL => "ZEROFILL",
            Keyword::ZONE => "ZONE",
            Keyword::_LIST_ => "_LIST_",
        }
    }

    pub const fn reserved(&self) -> bool {
        matches!(
            self,
            Keyword::ACCESSIBLE
                | Keyword::ADD
                | Keyword::ALL
                | Keyword::ALTER
                | Keyword::ANALYZE
                | Keyword::AND
                | Keyword::AS
                | Keyword::ASC
                | Keyword::ASENSITIVE
                | Keyword::BEFORE
                | Keyword::BETWEEN
                | Keyword::BIGINT
                | Keyword::BINARY
                | Keyword::BLOB
                | Keyword::BOTH
                | Keyword::BY
                | Keyword::CALL
                | Keyword::CASCADE
                | Keyword::CASE
                | Keyword::CHANGE
                | Keyword::CHAR
                | Keyword::CHARACTER
                | Keyword::CHECK
                | Keyword::COLLATE
                | Keyword::COLUMN
                | Keyword::COMMENT
                | Keyword::CONDITION
                | Keyword::CONSTRAINT
                | Keyword::CONTINUE
                | Keyword::CONVERT
                | Keyword::CREATE
                | Keyword::CROSS
                | Keyword::CURRENT_DATE
                | Keyword::CURRENT_ROLE
                | Keyword::CURRENT_TIME
                | Keyword::CURRENT_TIMESTAMP
                | Keyword::CURRENT_USER
                | Keyword::CURSOR
                | Keyword::DATABASE
                | Keyword::DATABASES
                | Keyword::DAY_HOUR
                | Keyword::DAY_MICROSECOND
                | Keyword::DAY_MINUTE
                | Keyword::DAY_SECOND
                | Keyword::DEC
                | Keyword::DECIMAL
                | Keyword::DECLARE
                | Keyword::DELAYED
                | Keyword::DELETE
                | Keyword::DELETE_DOMAIN_ID
                | Keyword::DESC
                | Keyword::DESCRIBE
                | Keyword::DETERMINISTIC
                | Keyword::DISTINCT
                | Keyword::DISTINCTROW
                | Keyword::DIV
                | Keyword::DO_DOMAIN_IDS
                | Keyword::DOUBLE
                | Keyword::DROP
                | Keyword::DUAL
                | Keyword::EACH
                | Keyword::ELSE
                | Keyword::ELSEIF
                | Keyword::ENCLOSED
                | Keyword::END
                | Keyword::ESCAPED
                | Keyword::EXCEPT
                | Keyword::EXISTS
                | Keyword::EXIT
                | Keyword::EXPLAIN
                | Keyword::FALSE
                | Keyword::FETCH
                | Keyword::FLOAT
                | Keyword::FLOAT4
                | Keyword::FLOAT8
                | Keyword::FOR
                | Keyword::FORCE
                | Keyword::FOREIGN
                | Keyword::FROM
                | Keyword::FULLTEXT
                | Keyword::GENERAL
                | Keyword::GRANT
                | Keyword::GROUP
                | Keyword::HAVING
                | Keyword::HIGH_PRIORITY
                | Keyword::HOUR_MICROSECOND
                | Keyword::HOUR_MINUTE
                | Keyword::HOUR_SECOND
                | Keyword::IF
                | Keyword::IGNORE
                | Keyword::IGNORE_DOMAIN_IDS
                | Keyword::IGNORE_SERVER_IDS
                | Keyword::IN
                | Keyword::INFILE
                | Keyword::INNER
                | Keyword::INOUT
                | Keyword::INSENSITIVE
                | Keyword::INSERT
                | Keyword::INT
                | Keyword::INT1
                | Keyword::INT2
                | Keyword::INT3
                | Keyword::INT4
                | Keyword::INT8
                | Keyword::INTEGER
                | Keyword::INTERSECTA
                | Keyword::INTERVAL
                | Keyword::INTO
                | Keyword::IS
                | Keyword::ITERATE
                | Keyword::JOIN
                | Keyword::KEY
                | Keyword::KEYS
                | Keyword::KILL
                | Keyword::LEADING
                | Keyword::LEAVE
                | Keyword::LEFT
                | Keyword::LIKE
                | Keyword::LIMIT
                | Keyword::LINEAR
                | Keyword::LINES
                | Keyword::LOAD
                | Keyword::LOCALTIME
                | Keyword::LOCALTIMESTAMP
                | Keyword::LOCK
                | Keyword::LONG
                | Keyword::LONGBLOB
                | Keyword::LONGTEXT
                | Keyword::LOOP
                | Keyword::LOW_PRIORITY
                | Keyword::MASTER_HEARTBEAT_PERIOD
                | Keyword::MASTER_SSL_VERIFY_SERVER_CERT
                | Keyword::MATCH
                | Keyword::MAXVALUE
                | Keyword::MEDIUMBLOB
                | Keyword::MEDIUMINT
                | Keyword::MEDIUMTEXT
                | Keyword::MIDDLEINT
                | Keyword::MINUTE_MICROSECOND
                | Keyword::MINUTE_SECOND
                | Keyword::MOD
                | Keyword::MODIFIES
                | Keyword::NATURAL
                | Keyword::NO_WRITE_TO_BINLOG
                | Keyword::NOT
                | Keyword::NULL
                | Keyword::NUMERIC
                | Keyword::OFFSET
                | Keyword::ON
                | Keyword::OPTIMIZE
                | Keyword::OPTION
                | Keyword::OPTIONALLY
                | Keyword::OR
                | Keyword::ORDER
                | Keyword::OUT
                | Keyword::OUTER
                | Keyword::OUTFILE
                | Keyword::OVER
                | Keyword::PAGE_CHECKSUM
                | Keyword::PARSE_VCOL_EXPR
                | Keyword::PARTITION
                | Keyword::POSITION
                | Keyword::PRECISION
                | Keyword::PRIMARY
                | Keyword::PROCEDURE
                | Keyword::PURGE
                | Keyword::RANGE
                | Keyword::READ
                | Keyword::READ_WRITE
                | Keyword::READS
                | Keyword::REAL
                | Keyword::RECURSIVE
                | Keyword::REF_SYSTEM_ID
                | Keyword::REFERENCES
                | Keyword::REGEXP
                | Keyword::RENAME
                | Keyword::REPEAT
                | Keyword::REPLACE
                | Keyword::REQUIRE
                | Keyword::RESIGNAL
                | Keyword::RESTRICT
                | Keyword::RETURN
                | Keyword::RETURNING
                | Keyword::REVOKE
                | Keyword::RIGHT
                | Keyword::RLIKE
                | Keyword::ROWS
                | Keyword::SCHEMA
                | Keyword::SCHEMAS
                | Keyword::SECOND_MICROSECOND
                | Keyword::SELECT
                | Keyword::SENSITIVE
                | Keyword::SEPARATOR
                | Keyword::SET
                | Keyword::SHOW
                | Keyword::SIGNAL
                | Keyword::SLOW
                | Keyword::SMALLINT
                | Keyword::SPATIAL
                | Keyword::SPECIFIC
                | Keyword::SQL
                | Keyword::SQL_BIG_RESULT
                | Keyword::SQL_CALC_FOUND_ROWS
                | Keyword::SQL_SMALL_RESULT
                | Keyword::SQLEXCEPTION
                | Keyword::SQLSTATE
                | Keyword::SQLWARNING
                | Keyword::SSL
                | Keyword::STARTING
                | Keyword::STATS_AUTO_RECALC
                | Keyword::STATS_PERSISTENT
                | Keyword::STATS_SAMPLE_PAGES
                | Keyword::STRAIGHT_JOIN
                | Keyword::TABLE
                | Keyword::TERMINATED
                | Keyword::THEN
                | Keyword::TINYBLOB
                | Keyword::TINYINT
                | Keyword::TINYTEXT
                | Keyword::TO
                | Keyword::TRAILING
                | Keyword::TRIGGER
                | Keyword::TRUE
                | Keyword::UNDO
                | Keyword::UNION
                | Keyword::UNIQUE
                | Keyword::UNLOCK
                | Keyword::UNSIGNED
                | Keyword::UPDATE
                | Keyword::USAGE
                | Keyword::USE
                | Keyword::USING
                | Keyword::UTC_DATE
                | Keyword::UTC_TIME
                | Keyword::UTC_TIMESTAMP
                | Keyword::VALUES
                | Keyword::VARBINARY
                | Keyword::VARCHAR
                | Keyword::VARCHARACTER
                | Keyword::VARYING
                | Keyword::WHEN
                | Keyword::WHERE
                | Keyword::WHILE
                | Keyword::WINDOW
                | Keyword::WITH
                | Keyword::WRITE
                | Keyword::XOR
                | Keyword::YEAR_MONTH
                | Keyword::ZEROFILL
        )
    }

    pub const fn expr_ident(&self) -> bool {
        matches!(
            self,
            Keyword::CURRENT_DATE
                | Keyword::CURRENT_TIME
                | Keyword::CURRENT_TIMESTAMP
                | Keyword::IF
                | Keyword::LOCALTIME
                | Keyword::LOCALTIMESTAMP
                | Keyword::REPLACE
                | Keyword::RIGHT
                | Keyword::UTC_DATE
                | Keyword::UTC_TIME
                | Keyword::UTC_TIMESTAMP
                | Keyword::VALUES
        ) || !self.reserved()
    }
}
