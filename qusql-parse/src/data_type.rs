    ...
    loop {
        ...
        match (&parser.token, ctx) {
            ...
            (Token::Ident(_, Keyword::GENERATED), Column) => {
                if parser.options.dialect.is_postgresql() {
                    let generated_always_as_span = parser.consume_keywords(&[
                        Keyword::GENERATED,
                        Keyword::ALWAYS,
                        Keyword::AS,
                    ])?;
                    if matches!(parser.token, Token::LParen) {
                        ...
                        properties.push(DataTypeProperty::GeneratedAlways(
                            generated_always_as_span.join_span(&identity_span),
                        ));
                    }
                } else {
                    properties.push(DataTypeProperty::GeneratedAlways(
