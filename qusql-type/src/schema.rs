    ...
    match expr {
        qusql_parse::Expression::String(s) => {
            ...
            if let Cow::Borrowed(borrowed) = &s.value {
                let span_offset =
                    borrowed.as_ptr() as usize - inner_src.as_ptr() as usize;
                let mut nested_issues = Issues::new(borrowed);
                let stmts =
                    parse_statements(borrowed, &mut nested_issues, &options.parse_options);
    ...
    match statement {
        ...
        qusql_parse::Statement::CreateFunction(f) => {
            ...
            let body = f
                ...
                .and_then(|s| {
                    try_parse_body(src, s, issues, &options.parse_options)
                });
